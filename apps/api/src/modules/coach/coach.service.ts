...
export class CoachService {
  ...
  async getPerformanceAdvice(userId: string) {
    ...
    const aiPrompt = `أنت مُدرّب تداول خبير في منصة "رؤى". حلل أداء المتداول بناءً على الإحصائيات التالية وسجل الصفقات. قدم 3-5 نصائح محددة وقابلة للتنفيذ لتحسين الأداء. ركز على إدارة المخاطر، الانضباط، حجم الصفقات، واختيار الأصول. اذكر نقاط القوة والضعف. اجعل النصائح بالعربية ومباشرة.
...
خطة_تحسين: [خطة التحسين الموصى بها]`;
  ...
  async askCoach(userId: string, question: string, contextAdviceId?: string) {
    ...
    const aiPrompt = `أنت مُدرّب تداول خبير في منصة "رؤى". المتداول يسألك سؤالاً حول أدائه. أجب بالعربية بشكل مهني ومفيد ومباشر.
