...
const indicators_service_1 = require("../analytics/indicators.service");
...
let ScannerService = ScannerService_1 = class ScannerService {
    constructor(aggregator, indicators, aiOrchestrator, redis) {
        this.aggregator = aggregator;
        this.indicators = indicators;
        this.aiOrchestrator = aiOrchestrator;
        this.redis = redis;
        this.logger = new common_1.Logger(ScannerService_1.name);
        this.SCANNER_AI_DAILY_COST_CAP_USD = 3.00;
        this.REDIS_SCANNER_AI_COST_KEY = 'scanner:ai:daily_cost';
        this.REDIS_SCANNER_AI_COST_DATE_KEY = 'scanner:ai:daily_cost_date';
        this.logger.log('🔍 Advanced Scanner Service initialized (with $3/day AI cost cap)');
    }
    async invalidateCache(timeframe = '1h', category) {
        const keys = [
    ...
    async fullScan(timeframe = '1h', category) {
        ...
        const symbols = scanner_types_1.SCANNER_SYMBOLS.filter(s => !category || category === scanner_types_1.MarketCategory.ALL || s.category === category);
        const items = [];
        const results = await Promise.allSettled(symbols.map(s => this._scanSymbol(s.symbol, s.name, s.category, timeframe)));
        ...
        for (let i = 0; i < symbols.length; i += BATCH_SIZE) {
            ...
            const results = await Promise.allSettled(batch.map(async (s) => {
                try {
                    ...
                    const marketInfo = (0, market_hours_util_1.isMarketOpen)(s.symbol);
                    return {
                        symbol: s.symbol,
                        name: s.name,
                        category: s.category,
    ...
    async multiTimeframeAnalysis(symbol) {
        ...
        const timeframes = [scanner_types_1.TimeFrame.M15, scanner_types_1.TimeFrame.H1, scanner_types_1.TimeFrame.H4, scanner_types_1.TimeFrame.D1];
        const weights = {
            [scanner_types_1.TimeFrame.D1]: 2.0,
            [scanner_types_1.TimeFrame.H4]: 1.5,
            [scanner_types_1.TimeFrame.H1]: 1.0,
            [scanner_types_1.TimeFrame.M15]: 0.5,
        };
        const tfAnalyses = [];
        for (const tf of timeframes) {
            try {
                ...
                const stoch = this._stochastic(candles.map(c => c.high), candles.map(c => c.low), candles.map(c => c.close));
                tfAnalyses.push({
                    timeframe: tf,
    ...
    async deepAnalysis(symbol) {
        ...
        const symbolInfo = scanner_types_1.SCANNER_SYMBOLS.find(s => s.symbol === symbol);
        const category = symbolInfo?.category ?? scanner_types_1.MarketCategory.ALL;
        const name = symbolInfo?.name ?? symbol;
        const marketInfo = (0, market_hours_util_1.isMarketOpen)(symbol);
        ...
        if (candles.length >= 30) {
            ...
            fibonacciResult = this._fibonacci(Math.max(...highs.slice(-52)), Math.min(...lows.slice(-52)));
            ...
            smartScore = this._calculateSmartScore(technical, stochResult, adxResult, ichimokuResult, obvResult, cciResult, sarResult, quote, volumeProfileResult, divergenceResult);
        }
        ...
        if (direction === scanner_types_1.SignalDirection.BUY || direction === scanner_types_1.SignalDirection.STRONG_BUY) {
            takeProfit = entryPrice + atrValue * 2;
            stopLoss = entryPrice - atrValue * 1;
            riskRewardRatio = 2;
        }
        else if (direction === scanner_types_1.SignalDirection.SELL || direction === scanner_types_1.SignalDirection.STRONG_SELL) {
            takeProfit = entryPrice - atrValue * 2;
            stopLoss = entryPrice + atrValue * 1;
            riskRewardRatio = 2;
        }
        ...
        try {
            ...
            if (aiCached) {
                ...
                if (todayCost >= this.SCANNER_AI_DAILY_COST_CAP_USD) {
                    ...
                    aiSentiment = technicalScore > 20 ? 'POSITIVE' : technicalScore < -20 ? 'NEGATIVE' : 'NEUTRAL';
                    riskLevel = Math.abs(quote.changePercent) > 3 ? 'HIGH' : Math.abs(quote.changePercent) > 1.5 ? 'MEDIUM' : 'LOW';
                    await this._addScannerAICost(0.02);
    ...
    async marketOverview() {
        ...
        const bullishCount = items.filter(i => i.direction === scanner_types_1.SignalDirection.BUY || i.direction === scanner_types_1.SignalDirection.STRONG_BUY).length;
        const bearishCount = items.filter(i => i.direction === scanner_types_1.SignalDirection.SELL || i.direction === scanner_types_1.SignalDirection.STRONG_SELL).length;
        const neutralCount = items.filter(i => i.direction === scanner_types_1.SignalDirection.NEUTRAL).length;
        const topGainers = heatmap.slice(0, 5);
        const topLosers = heatmap.slice(-5).reverse();
        const strongestSignals = items
            .filter(i => i.direction !== scanner_types_1.SignalDirection.NEUTRAL && i.confidence >= 60)
            .slice(0, 5);
    ...
    _stochastic(highs, lows, closes, kPeriod = 14, dPeriod = 3) {
    ...
    _adx(highs, lows, closes, period = 14) {
        ...
        let trendDirection;
        if (latestPlusDi > latestMinusDi + 5)
            trendDirection = 'BULLISH';
        else if (latestMinusDi > latestPlusDi + 5)
    ...
    _vwap(highs, lows, closes, volumes) {
        ...
        const deviation = cumVolume > 0
            ? Math.sqrt(closes.reduce((sum, c, i) => {
                const tp = (highs[i] + lows[i] + c) / 3;
                return sum + volumes[i] * Math.pow(tp - vwapValue, 2);
            }, 0) / cumVolume)
            : 0;
        let position;
        if (lastPrice > vwapValue * 1.001)
            position = 'ABOVE';
        else if (lastPrice < vwapValue * 0.999)
            position = 'BELOW';
        else
            position = 'AT';
        return {
    ...
    _supportResistance(closes, windowSize = 5) {
        if (closes.length < windowSize * 2 + 1)
            return [];
    ...
    _ichimoku(highs, lows, closes, tenkanPeriod = 9, kijunPeriod = 26, senkouBPeriod = 52) {
        if (closes.length < senkouBPeriod)
            return null;
        const midPoint = (data, period, endIdx) => {
        ...
        let cloudColor;
        if (senkouSpanA > senkouSpanB * 1.001)
            cloudColor = 'BULLISH';
        else if (senkouSpanB > senkouSpanA * 1.001)
            cloudColor = 'BEARISH';
        else
            cloudColor = 'NEUTRAL';
        ...
        if (currentPrice > cloudTop)
            ...
            priceVsCloud = 'INSIDE';
        let tkCross;
        if (tenkanSenValue > kijunSenValue * 1.001)
            tkCross = 'BULLISH';
        else if (kijunSenValue > tenkanSenValue * 1.001)
            tkCross = 'BEARISH';
        else
            tkCross = 'NEUTRAL';
        return {
    ...
    _cci(highs, lows, closes, period = 20) {
    ...
    _parabolicSar(highs, lows, af = 0.02, maxAf = 0.2) {
        if (highs.length < 5)
            return null;
        let isRising = highs[1] > highs[0];
    ...
    _detectDivergence(closes, indicatorValues, type = 'rsi') {
        if (closes.length < 10 || indicatorValues.length < 10)
            return null;
        ...
        const findTroughs = (data, minDistance = 3) => {
            const troughs = [];
            for (let i = minDistance; i < data.length - minDistance; i++) {
                let isTrough = true;
                for (let j = 1; j <= minDistance; j++) {
    ...
    _volumeProfile(highs, lows, closes, volumes, bins = 10) {
    ...
    _calculateSmartScore(technical, stoch, adx, ichimoku, obv, cci, sar, quote, volumeProfile, divergence) {
        const rsiValue = technical.rsi?.values?.[technical.rsi.values.length - 1] ?? null;
        let trendStrength = 0;
        let trendDir = 0;
        if (adx) {
            if (adx.trendStrength === 'VERY_STRONG')
                trendStrength += 30;
            else if (adx.trendStrength === 'STRONG')
                trendStrength += 22;
            else if (adx.trendStrength === 'WEAK')
                trendStrength += 10;
            if (adx.trendDirection === 'BULLISH')
                trendDir += 1;
            else if (adx.trendDirection === 'BEARISH')
                trendDir -= 1;
        }
        else {
            trendStrength += 10;
        ...
        if (ichimoku) {
            ...
            if (ichimoku.priceVsCloud === 'ABOVE' || ichimoku.priceVsCloud === 'BELOW')
                ichimokuStrength = 30;
            else if (ichimoku.priceVsCloud === 'INSIDE')
                ichimokuStrength = 18;
            if (ichimoku.cloudColor === 'BULLISH' && ichimoku.priceVsCloud === 'ABOVE')
                trendDir += 1;
            else if (ichimoku.cloudColor === 'BEARISH' && ichimoku.priceVsCloud === 'BELOW')
                trendDir -= 1;
            else if (ichimoku.cloudColor === 'BULLISH' && ichimoku.priceVsCloud === 'INSIDE')
                trendDir += 0.5;
            else if (ichimoku.cloudColor === 'BEARISH' && ichimoku.priceVsCloud === 'INSIDE')
                trendDir -= 0.5;
            if (ichimoku.tkCross === 'BULLISH') {
                trendDir += 0.5;
                ichimokuStrength = Math.min(30, ichimokuStrength + 3);
            }
        ...
        if (cci) {
            ...
            else if (cci.interpretation === 'OVERBOUGHT') {
                momentumStrength += 22;
                momentumDir -= 0.5;
            }
            else {
        ...
        if (technical.atr) {
            if (technical.atr.volatilityLevel === 'HIGH')
                volatilityScore += 35;
            else if (technical.atr.volatilityLevel === 'NORMAL')
                volatilityScore += 20;
            else
                volatilityScore += 10;
        }
        ...
        if (obv) {
            ...
            if (obv.divergence === 'BULLISH_DIVERGENCE') {
                volumeStrength = Math.min(40, volumeStrength + 5);
                volumeDir += 0.5;
            }
            else if (obv.divergence === 'BEARISH_DIVERGENCE') {
                volumeStrength = Math.min(40, volumeStrength + 5);
                volumeDir -= 0.5;
            }
        }
        ...
        if (quoteVolume > 0) {
            ...
            if (changePct > 3) {
                volumeStrength += 30;
                volumeDir += (quote.changePercent ?? 0) > 0 ? 0.5 : -0.5;
            }
            else if (changePct > 1.5) {
                volumeStrength += 22;
                volumeDir += (quote.changePercent ?? 0) > 0 ? 0.3 : -0.3;
            }
        ...
        let action;
        if (compositeScore >= 50 && confidence >= 70)
            action = 'STRONG_BUY';
        else if (compositeScore >= 20)
            action = 'BUY';
        else if (compositeScore <= -50 && confidence >= 70)
            action = 'STRONG_SELL';
        else if (compositeScore <= -20)
            action = 'SELL';
        else
    ...
    async _scanSymbol(symbol, name, category, timeframe) {
        try {
            const marketInfo = (0, market_hours_util_1.isMarketOpen)(symbol);
            if (!marketInfo.open && category !== scanner_types_1.MarketCategory.CRYPTO) {
                return null;
            }
            const [quote, candles] = await Promise.all([
                this.aggregator.getAggregatedQuote(symbol).catch(() => null),
            ...
            if (candles.length >= 35) {
                ...
                smartScore = this._calculateSmartScore(analysis, stochResult, adxResult, ichimokuResult, obvResult, cciResult, sarResult, quote, volumeProfileResult, divergenceResult);
                ...
                if (macdSignal === 'BULLISH_CROSSOVER') {
                    ...
                    reasonsAr.push('MACD صعودي');
                }
                else if (macdSignal === 'BEARISH_CROSSOVER') {
                    reasons.push('MACD Bearish');
                    reasonsAr.push('MACD هبوطي');
                }
                if (bollingerPosition === 'BELOW_LOWER') {
                    reasons.push('BB Lower');
                    reasonsAr.push('بولنجر سفلي');
                }
                else if (bollingerPosition === 'ABOVE_UPPER') {
                    reasons.push('BB Upper');
                    reasonsAr.push('بولنجر علوي');
                }
                if (stochResult?.interpretation === 'OVERSOLD') {
                    reasons.push('Stoch Oversold');
                    reasonsAr.push('ستوكاستيك بيعي');
                }
                else if (stochResult?.interpretation === 'OVERBOUGHT') {
                    reasons.push('Stoch Overbought');
                    reasonsAr.push('ستوكاستيك شرائي');
                }
                if (adxResult?.trendStrength === 'STRONG' || adxResult?.trendStrength === 'VERY_STRONG') {
                ...
                if (cciResult?.interpretation === 'OVERSOLD') {
                    reasons.push('CCI Oversold');
                    reasonsAr.push('CCI تشبع بيعي');
                }
                else if (cciResult?.interpretation === 'OVERBOUGHT') {
                    reasons.push('CCI Overbought');
                    reasonsAr.push('CCI تشبع شرائي');
                }
                if (divergenceResult?.type === 'BULLISH') {
                    reasons.push('Bullish RSI Divergence');
                    reasonsAr.push('تباعد صعودي RSI');
    ...
    _classifyDirection(scoreOrChange, isTechnicalScore = false) {
        const s = isTechnicalScore ? scoreOrChange : this._quickScore({ changePercent: scoreOrChange });
        if (s >= 50)
            return scanner_types_1.SignalDirection.STRONG_BUY;
        if (s >= 15)
            return scanner_types_1.SignalDirection.BUY;
        if (s <= -50)
            return scanner_types_1.SignalDirection.STRONG_SELL;
        if (s <= -15)
            return scanner_types_1.SignalDirection.SELL;
        return scanner_types_1.SignalDirection.NEUTRAL;
    }
    _scoreToDirection(score) {
        if (score >= 50)
            return scanner_types_1.SignalDirection.STRONG_BUY;
        if (score >= 15)
            return scanner_types_1.SignalDirection.BUY;
    ...
    _classifySignalClass(technical, stoch, adx) {
        if (!technical)
            return scanner_types_1.SignalClass.WATCH;
        ...
        if (technical.atr?.volatilityLevel === 'HIGH' && score > 40) {
            return scanner_types_1.SignalClass.BREAKOUT;
        }
        if (technical.rsi?.interpretation === 'OVERBOUGHT' || technical.rsi?.interpretation === 'OVERSOLD' ||
            ...
            return scanner_types_1.SignalClass.REVERSION;
        }
        if (isStrongTrend && score > 25) {
            return scanner_types_1.SignalClass.TREND;
        }
        if (technical.atr?.volatilityLevel === 'LOW' && score < 20) {
            return scanner_types_1.SignalClass.CONSOLIDATION;
        }
        return score > 15 ? scanner_types_1.SignalClass.TREND : scanner_types_1.SignalClass.WATCH;
    }
    ...
    _generateExecutionHintAr(alignment, score, timeframes) {
        const alignedCount = timeframes.filter(t => (score > 0 && t.technicalScore > 0) || (score < 0 && t.technicalScore < 0)).length;
    ...
    async _getScannerAIDailyCost() {
        try {
            const today = new Date().toISOString().slice(0, 10);
            const storedDate = await this.redis.get(this.REDIS_SCANNER_AI_COST_DATE_KEY);
            if (storedDate !== today) {
                await this.redis.set(this.REDIS_SCANNER_AI_COST_KEY, '0', 86400000);
    ...
    async _addScannerAICost(estimatedCostUsd) {
        try {
