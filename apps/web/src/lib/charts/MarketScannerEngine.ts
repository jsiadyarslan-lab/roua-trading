...
export const SECTOR_LABELS_AR: Record<AssetSector, string> = {
  defi: 'DeFi — اللامركزية',
  layer1: 'الطبقة الأولى — Layer 1',
  layer2: 'الطبقة الثانية — Layer 2',
  meme: 'عملات الميم',
  ai: 'الذكاء الاصطناعي',
  gaming: 'الألعاب',
  rwa: 'الأصول الحقيقية',
  cefi: 'التمويل المركزي',
...
function getSignalLabelAr(source: string): string {
  const labels: Record<string, string> = {
    ...
    rsi_bullish: 'RSI صاعد',
    rsi_bearish: 'RSI هابط',
...
function computeQuickCorrelations(assets: AssetScanResult[]): AssetCorrelation[] {
  ...
  for (let i = 0; i < assets.length; i++) {
    for (let j = i + 1; j < assets.length; j++) {
      ...
      if (correlation > 0.7) { strengthLabel = 'Strong Positive'; strengthLabelAr = 'ارتباط إيجابي قوي'; }
      else if (correlation > 0.3) { strengthLabel = 'Moderate Positive'; strengthLabelAr = 'ارتباط إيجابي متوسط'; }
      else if (correlation > -0.3) { strengthLabel = 'Weak'; strengthLabelAr = 'ارتباط ضعيف'; }
      else if (correlation > -0.7) { strengthLabel = 'Moderate Negative'; strengthLabelAr = 'ارتباط سلبي متوسط'; }
      else { strengthLabel = 'Strong Negative'; strengthLabelAr = 'ارتباط سلبي قوي'; }
