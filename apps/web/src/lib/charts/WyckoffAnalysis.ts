...
export function detectWyckoff(candles: CandleData[]): WyckoffResult {
 if (candles.length < 30) return { phase: 'Unknown', labelAr: ' ', confidence: 0, events: [], bias: 'neutral' };
 ...
 events.push({ type: 'SC', labelAr: ' ', time: slice[slice.length-1].time, price: minP });
 } else if (slopeNorm > 0.001 && posInRange > 0.3 && posInRange < 0.7) {
 phase = 'Markup'; bias = 'bullish'; confidence = 0.70;
 events.push({ type: 'SOS', labelAr: 'strength', time: slice[slice.length-1].time, price: current });
 } else if (posInRange > 0.7 && slopeNorm > -0.001 && volTrend > 1.1) {
 phase = 'Distribution'; bias = 'bearish'; confidence = 0.65;
 events.push({ type: 'BC', labelAr: ' ', time: slice[slice.length-1].time, price: maxP });
 } else if (slopeNorm < -0.001 && posInRange < 0.5) {
 phase = 'Markdown'; bias = 'bearish'; confidence = 0.68;
 events.push({ type: 'LPSY', labelAr: 'weakness', time: slice[slice.length-1].time, price: current });
 }

 const labels: Record<WyckoffPhase, string> = {
 'Accumulation': 'how much', 'Markup': '', 'Distribution': '', 'Markdown': '', 'Unknown': ' '
 };
