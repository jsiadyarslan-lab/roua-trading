 ...
 */
export function closePaperTrade(tradeId: string, currentPrice: number, reasonAr: string = ' '): PaperTrade | null {
...
export function autoEvaluatePaperTrades(currentPrice: number, candles?: CandleData[]): PaperTrade[] {
 ...
 closePaperTrade(trade.id, currentPrice, ' stop loss');
 closed = true;
 ...
 const reasonAr = i === trade.takeProfits.length - 1 ? ' goal ' : ` goal ${i + 1}`;
