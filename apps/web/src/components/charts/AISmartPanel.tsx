    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 340, maxHeight: 560, background: C.bg, borderRadius: 'var(--radius-lg)', border: `1px solid ${C.border}`, overflow: 'hidden', fontFamily: "var(--font-ar)", boxShadow: '0 24px 64px rgba(0,0,0,0.7)', direction: 'inherit' }}>
      ...
      <div data-drag-handle="true" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 10px', borderBottom: `1px solid ${C.border}`, background: 'rgba(255,255,255,0.025)', cursor: 'grab', userSelect: 'none', flexShrink: 0 }}>
        <div data-drag-handle="true" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span data-drag-handle="true" style={{ fontSize: 17 }}>🧠</span>
          <div data-drag-handle="true">
            <div style={{ color: C.text, fontSize: 11, fontWeight: 700 }}>{t('title')}</div>
            <div style={{ color: C.mut, fontSize: 11, fontFamily: "var(--font-mono)" }}>{symbol}</div>
          </div>
        ...
        <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
          ...
          {streamMode && (
            <div style={{ padding: '1px 5px', borderRadius: 'var(--radius-xs)', fontSize: 11, fontWeight: 700, fontFamily: "var(--font-mono)", background: `${C.cyan}18`, color: C.cyan, border: `1px solid ${C.cyan}30`, display: 'flex', alignItems: 'center', gap: 3 }}>
          ...
          <button onClick={() => { runRef.current = false; abortRef.current?.abort(); analyze(); }} disabled={loading} title={t('refresh')} style={{ background: 'none', border: `1px solid ${C.border}`, borderRadius: 'var(--radius-sm)', color: loading ? C.mut : C.cyan, width: 22, height: 22, cursor: 'pointer', fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', outline: 'none' }}>⟳</button>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: C.mut, fontSize: 17, cursor: 'pointer', outline: 'none', width: 22, height: 22, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(42px, 1fr))', gap:2, padding:'3px 6px', borderBottom:`1px solid ${C.border}`, flexShrink:0 }}>
        {([['S/R','sr','#4ade80'],['📈','trend','#facc15'],['HM','harmonic','#B388FF'],['FVG','fvg','#00D4FF'],['BOS','bos','#FFB800'],['GEO','geo','#B388FF'],['EW','ew','#93c5fd'],['WY','wyckoff','#fb923c'],['VP','vp','#FFB800'],['ENT','entry','#00D4FF'],['MTF','mtf','#06b6d4'],['LIQ','liq','#F472B6'],['TRD','trade','#a3e635']] as [string,keyof typeof overlays,string][]).map(([lbl,key,col])=>(
          <button key={key} onClick={()=>{ toggleOverlay(key); }} title={[['S/R','sr'],['📈','trend'],['HM','harmonic'],['FVG','fvg'],['BOS','bos'],['GEO','geo'],['EW','ew'],['WY','wyckoff'],['VP','vp'],['ENT','entry'],['MTF','mtf'],['LIQ','liq'],['TRD','trade']].find(x=>x[1]===key)?.[0] || key}
            style={{ padding:'2px 0', borderRadius: 'var(--radius-xs)', fontSize: 11, fontWeight:700, cursor:'pointer', outline:'none', fontFamily:'inherit',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', borderBottom: `1px solid ${C.border}`, borderTop: `1px solid ${C.border}`, flexShrink: 0 }}>
        {([['signal', t('tabSignal')], ['patterns', t('tabPatterns')], ['wyckoff', 'WY'], ['elliott', 'EW'], ['levels', t('tabLevels')], ['smc', t('tabSmc')], ['mtf', 'MTF'], ['alerts', '🚨'], ['trades', '💰'], ['advanced', t('tabAdvanced')], ['adaptive', '🧠'], ['rules', '📐'], ['paper', '📝'], ['scanner', '🔍'], ['council', '🤖'], ['backtest', '⏪'], ['confluence', '🎯'], ['explain', '❓'], ['correlate', '🔗'], ['predict', '🔮'], ['intelligence', '🔬'], ['scenario', '🎲'], ['spring', '🌀'], ['journal', '📋']] as [Tab, string][]).map(([k, l]) => (
          <button key={k} onClick={() => setTab(k)} style={{ padding: '3px 2px', background: tab===k?'rgba(34,211,238,0.08)':'none', border: 'none', borderBottom: `2px solid ${tab === k ? C.cyan : 'transparent'}`, color: tab === k ? C.cyan : C.dim, fontSize: 11, cursor: 'pointer', outline: 'none', fontFamily: 'inherit', transition: 'all 0.15s', fontWeight: tab===k?700:400, whiteSpace: 'nowrap', textAlign: 'center' }}>{l}</button>
        ))}
      </div>
        ...
        <div style={{ position: 'absolute', inset: 0, zIndex: 100, background: 'rgba(0,0,0,0.85)', display: 'flex', flexDirection: 'column', padding: 10, overflow: 'auto', borderRadius: 'var(--radius-lg)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <span style={{ color: C.text, fontSize: 11, fontWeight: 700 }}>{t('asp_engineVerification')}</span>
            </div>
            <button onClick={() => setShowVerification(false)} style={{ background: 'none', border: 'none', color: C.mut, fontSize: 15, cursor: 'pointer', outline: 'none' }}>×</button>
          </div>
          ...
          <div style={{ textAlign: 'center', padding: '8px 0', marginBottom: 8, borderRadius: 'var(--radius-sm)', background: verificationReport.allReal ? `${C.green}10` : `${C.gold}10`, border: `1px solid ${verificationReport.allReal ? C.green : C.gold}30` }}>
            <div style={{ fontSize: 27, fontWeight: 800, color: verificationReport.allReal ? C.green : C.gold, fontFamily: "var(--font-mono)" }}>
              {verificationReport.overallScore}%
            </div>
            <div style={{ fontSize: 11, color: C.dim, marginTop: 2 }}>
              {verificationReport.allReal ? t('asp_allEnginesReal') : t('asp_someEnginesNeedReview')}
            </div>
          </div>
          ...
          {verificationReport.engines.map((eng: EngineVerificationResult) => (
            <div key={eng.engine} style={{ marginBottom: 6, borderRadius: 'var(--radius-sm)', border: `1px solid ${eng.isReal ? C.green : C.red}30`, background: `${eng.isReal ? C.green : C.red}08`, padding: '6px 8px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: C.text }}>{eng.engine}</span>
                <span style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: eng.isReal ? C.green : C.red, fontWeight: 700 }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', minHeight: 0 }}>
              ...
              <>
                <div style={{ background: `${sigColor}12`, border: `1px solid ${sigColor}30`, borderRadius: 'var(--radius-md)', padding: '10px 12px', marginBottom: 8 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                    ...
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ color: sigColor, fontSize: 19, fontWeight: 900 }}>{pct}%</div>
                      <div style={{ color: C.mut, fontSize: 11 }}>{t('confidence')}</div>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: signal.fusionScore !== undefined ? '1fr 1fr' : '1fr', gap: 4, marginBottom: 6 }}>
                    {signal.bayesianDir && (
                      <div style={{ background: `${C.purple}0a`, border: `1px solid ${C.purple}25`, borderRadius: 'var(--radius-sm)', padding: '4px 7px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span style={{ color: C.purple, fontSize: 11, fontWeight: 700 }}>{t('bayesian')}</span>
                        <span style={{ color: signal.bayesianDir === 'BUY' ? C.green : signal.bayesianDir === 'SELL' ? C.red : C.yellow, fontSize: 11, fontWeight: 700 }}>
                    ...
                    {signal.fusionScore !== undefined && (
                      <div style={{ background: `${C.gold}0a`, border: `1px solid ${C.gold}25`, borderRadius: 'var(--radius-sm)', padding: '4px 7px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span style={{ color: C.gold, fontSize: 11, fontWeight: 700 }}>{t('confluence')}</span>
                        <span style={{ color: signal.fusionScore > 50 ? C.green : signal.fusionScore > 25 ? C.yellow : C.dim, fontSize: 11, fontWeight: 700 }}>
                          {signal.fusionScore}%
                        </span>
                      </div>
                    )}
                ...
                {signal.bayesianDir && signal.fusionScore !== undefined &&
                  ...
                  <div style={{
                    ...
                    <span style={{ color: '#FFB800', fontSize: 11, fontWeight: 600, lineHeight: 1.4 }}>
                      {locale === 'ar'
                        ? t('asp_bayesianConfluenceContradiction')
                ...
                {signal.dir !== 'WAIT' && (
                  <>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 5, marginBottom: 8 }}>
                      {([[t('entry'), signal.entry, C.cyan], [t('stopLoss'), signal.sl, C.red], [t('target'), signal.tp, C.green]] as [string, number, string][]).map(([l, v, col]) => (
                        <div key={l} style={{ background: `${col}0a`, border: `1px solid ${col}25`, borderRadius: 'var(--radius-sm)', padding: 5, textAlign: 'center' }}>
                          <div style={{ color: C.mut, fontSize: 11, marginBottom: 2 }}>{l}</div>
                          <div style={{ color: col, fontSize: 11, fontWeight: 700, fontFamily: "var(--font-mono)" }}>{fp(v)}</div>
                        </div>
                      ))}
                    </div>
                    {(() => {
                      ...
                      return (
                        <>
                          <div style={{
                            ...
                            <span style={{ color: C.dim, fontSize: 11 }}>{t('riskReward')}</span>
                            <span style={{
                          ...
                          {(isWeak) && (
                            <div style={{
                              ...
                              <span style={{ fontSize: 11, color: isCritical ? C.red : '#FFB800' }}>⚠</span>
                              <span style={{ fontSize: 11, color: isCritical ? C.red : '#FFB800', fontWeight: 600, lineHeight: 1.3 }}>
                                {locale === 'ar'
                                  ? (isCritical ? t('asp_weakRRNoEntry') : t('asp_suboptimalRR'))
                    ...
                    {onExecuteTrade && (
                      <button onClick={() => onExecuteTrade(signal.dir === 'BUY' ? 'long' : 'short', signal.entry, signal.sl, signal.tp)} style={{ width: '100%', padding: '7px', borderRadius: 'var(--radius-sm)', border: 'none', background: signal.dir === 'BUY' ? C.green : C.red, color: '#fff', fontSize: 11, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit', marginBottom: 8 }}>
                ...
                {performanceStats && performanceStats.totalTrades > 0 && (
                  <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '7px 9px', marginBottom: 6, border: `1px solid ${C.border}` }}>
                    <div style={{ color: C.gold, fontSize: 11, fontWeight: 700, marginBottom: 4 }}>{t('previousPatternPerformance')}</div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ color: C.dim, fontSize: 11 }}>{t('winRate')}</span>
                      <span style={{ color: performanceStats.winRate > 50 ? C.green : C.red, fontSize: 11, fontWeight: 700 }}>{Math.round(performanceStats.winRate)}% ({performanceStats.totalTrades})</span>
                    </div>
                    {performanceStats.bestPattern && (
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 2 }}>
                        <span style={{ color: C.dim, fontSize: 11 }}>{t('bestPattern')}</span>
                        <span style={{ color: C.green, fontSize: 11, fontWeight: 600 }}>{performanceStats.bestPattern}</span>
                      </div>
                    )}
                  </div>
                )}
                ...
                {stateMachineResult && stateMachineResult.alerts.length > 0 && (
                  <div style={{ background: `${C.gold}08`, border: `1px solid ${C.gold}20`, borderRadius: 'var(--radius-sm)', padding: '6px 8px', marginBottom: 6 }}>
                    <div style={{ color: C.gold, fontSize: 11, fontWeight: 700, marginBottom: 3 }}>{t('patternLifecycleAlerts')}</div>
                    {stateMachineResult.alerts.slice(0, 3).map((a: any, i: number) => (
                      <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '2px 0' }}>
                        <span style={{ color: C.dim, fontSize: 11 }}>{a.patternType}</span>
                        <span style={{ color: a.priority === 'critical' ? C.red : a.priority === 'warning' ? C.yellow : C.cyan, fontSize: 11, fontWeight: 600 }}>
                    ...
                    {(() => {
                      ...
                      return (
                        <div style={{
                          ...
                          <span style={{ color: '#FFB800', fontSize: 11, fontWeight: 600, lineHeight: 1.3 }}>
                            {locale === 'ar'
                              ? t('asp_contradictingPatterns', { patterns: patternNames })
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 5 }}>
                    {([[t('resistance'), resistance, C.red], [t('support'), support, C.green]] as [string, SupportResistanceLevel[], string][]).map(([lbl, arr, col]) => arr.length > 0 ? (
                      <div key={lbl} style={{ background: `${col}07`, border: `1px solid ${col}18`, borderRadius: 'var(--radius-sm)', padding: '5px 7px' }}>
                        <div style={{ color: col, fontSize: 11, fontWeight: 700, marginBottom: 3 }}>{lbl}</div>
                        {arr.slice(0, 2).map((l, i) => <div key={i} style={{ color: C.dim, fontSize: 11, fontFamily: "var(--font-mono)" }}>{fp(l.price)}</div>)}
                      </div>
                    ) : null)}
                  </div>
                )}
                <div style={{ textAlign: 'center', marginTop: 6, color: C.mut, fontSize: 11 }}>{new Date(signal.ts).toLocaleTimeString(timeLocale)}</div>
              </>
            ) : (
              <div style={{ textAlign: 'center', padding: 24, color: C.dim }}>
                <div style={{ fontSize: 27, marginBottom: 8 }}>🧠</div>
                <div style={{ fontSize: 11 }}>{t('pressToAnalyze')}</div>
              </div>
            )}
          </div>
        )}
          ...
          <div style={{ padding: 8, overflowY: 'auto', flex: 1, minHeight: 0 }}>
            ...
            {bayesianResult && (
              <div style={{ background: `${C.purple}0a`, border: `1px solid ${C.purple}20`, borderRadius: 'var(--radius-sm)', padding: '7px 9px', marginBottom: 8 }}>
                <div style={{ color: C.purple, fontSize: 11, fontWeight: 700, marginBottom: 4 }}>{t('bayesianConsensus')}</div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 3 }}>
                  <span style={{ color: C.green, fontSize: 11 }}>{Math.round((bayesianResult.posteriorBullish ?? bayesianResult.bullish ?? 0) * 100)}%</span>
                  <span style={{ color: C.yellow, fontSize: 11 }}>{Math.round((bayesianResult.posteriorNeutral ?? bayesianResult.neutral ?? 0) * 100)}%</span>
                  <span style={{ color: C.red, fontSize: 11 }}>{Math.round((bayesianResult.posteriorBearish ?? bayesianResult.bearish ?? 0) * 100)}%</span>
                </div>
              </div>
            )}
            {patterns.length === 0 ? <div style={{ textAlign: 'center', padding: 20, color: C.dim, fontSize: 11 }}>{t('noPatterns')}</div>
              : patterns.map((p, i) => {
                ...
                return (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 8px', borderRadius: 'var(--radius-sm)', marginBottom: 4, background: C.card, border: `1px solid ${col}18` }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <span style={{ color: C.text, fontSize: 11, fontWeight: 600 }}>{PATTERN_KEYS[p.type] ? t(PATTERN_KEYS[p.type]) : p.type}</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                      ...
                      <span style={{ color: C.mut, fontSize: 11 }}>{Math.round(p.confidence * 100)}%</span>
                    </div>
                  </div>
                );
              })}
          ...
          <div style={{ padding: 8, overflowY: 'auto', flex: 1, minHeight: 0 }}>
            {([[t('resistance'), resistance, C.red], [t('support'), support, C.green]] as [string, SupportResistanceLevel[], string][]).map(([lbl, arr, col]) => arr.length > 0 ? (
              <div key={lbl} style={{ marginBottom: 10 }}>
                <div style={{ color: col, fontSize: 11, fontWeight: 700, marginBottom: 4, letterSpacing: 0.5 }}>{lbl} ({arr.length})</div>
                {arr.map((l, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 8px', borderRadius: 'var(--radius-sm)', background: C.card, marginBottom: 3, border: `1px solid ${col}15` }}>
                    <span style={{ color: col, fontSize: 11, fontFamily: "var(--font-mono)", fontWeight: 700 }}>{fp(l.price)}</span>
                    <span style={{ color: l.strength === 'strong' ? col : C.mut, fontSize: 11 }}>{strengthLabel(l.strength)}</span>
                  </div>
                ))}
              </div>
            ) : null)}
          ...
          <div style={{ padding: 8, overflowY: 'auto', flex: 1, minHeight: 0 }}>
            ...
            {fusionResult && (
              <div style={{ background: `${C.gold}0a`, border: `1px solid ${C.gold}25`, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 8 }}>
                <div style={{ color: C.gold, fontSize: 11, fontWeight: 700, marginBottom: 4 }}>{t('elliottSmcConfluence')}</div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ color: fusionResult.direction === 'bullish' ? C.green : fusionResult.direction === 'bearish' ? C.red : C.yellow, fontSize: 13, fontWeight: 800 }}>
                ...
                <div style={{ color: C.dim, fontSize: 11, marginTop: 3 }}>{fusionResult.interpretation}</div>
              </div>
            )}
            {wyckoffData && wyckoffData.phase !== 'Unknown' && (
              <div style={{ background: C.card, border: `1px solid ${wyckoffData.bias==='bullish'?C.green:wyckoffData.bias==='bearish'?C.red:C.yellow}30`, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 8 }}>
                <div style={{ color: C.dim, fontSize: 11, marginBottom: 3 }}>{t('wyckoff')}</div>
                ...
                <div style={{ color: C.mut, fontSize: 11, marginTop: 2 }}>{Math.round((wyckoffData.confidence||0)*100)}% {t('confidence')}</div>
              </div>
            )}
            {volProfile && volProfile.poc > 0 && (
              <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 8 }}>
                <div style={{ color: C.dim, fontSize: 11, marginBottom: 6 }}>{t('volumeProfile')}</div>
                {([[t('poc'), volProfile.poc, C.yellow], [t('vah'), volProfile.vah, C.cyan], [t('val'), volProfile.val, C.red]] as [string,number,string][]).map(([l,v,col]) => (
                  <div key={l} style={{ display:'flex', justifyContent:'space-between', marginBottom:4, padding:'3px 0', borderBottom:`1px solid ${C.border}` }}>
                    <span style={{ color: col, fontSize: 11, fontWeight: 700 }}>{l}</span>
                    <span style={{ color: C.text, fontSize: 11, fontFamily: "var(--font-mono)" }}>{v>999?v.toFixed(2):v.toFixed(5)}</span>
                  </div>
                ))}
              </div>
            )}
            {!wyckoffData && !volProfile && !fusionResult && <div style={{ textAlign:'center', padding: 20, color: C.dim, fontSize: 11 }}>{t('pressForAnalysis')}</div>}
          </div>
        )}
        ...
        {tab === 'wyckoff' && (
          <div style={{ padding: 8, overflowY: 'auto', flex: 1, minHeight: 0 }}>
            {!wyckoffAdvanced || wyckoffAdvanced.scheme === 'none' ? (
              <div style={{ color: C.mut, textAlign: 'center', padding: 24 }}>{t('asp_noWyckoff')}</div>
            ) : (
              <>
                {/* Scheme Header */}
                <div style={{ padding: 8, borderRadius: 'var(--radius-sm)', marginBottom: 8, background: wyckoffAdvanced.direction === 'bullish' ? 'rgba(16,185,129,0.06)' : 'rgba(239,68,68,0.06)', border: `1px solid ${wyckoffAdvanced.direction === 'bullish' ? 'rgba(16,185,129,0.2)' : 'rgba(239,68,68,0.2)'}` }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span style={{ fontSize: 15 }}>{wyckoffAdvanced.direction === 'bullish' ? '📈' : '📉'}</span>
                      <span style={{ color: wyckoffAdvanced.direction === 'bullish' ? C.green : C.red, fontWeight: 700, fontSize: 13 }}>
                        {wyckoffAdvanced.scheme === 'accumulation' ? t('asp_wyckoffAccumulation') : t('asp_wyckoffDistribution')}
                      </span>
                    </div>
                    <div style={{ padding: '2px 8px', borderRadius: 'var(--radius-sm)', fontSize: 11, fontWeight: 700, background: 'rgba(245,158,11,0.12)', color: C.yellow, border: '1px solid rgba(245,158,11,0.2)' }}>
                      {t('asp_phase')} {wyckoffAdvanced.currentPhase}
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginTop: 8, fontSize: 11 }}>
                    <div><span style={{ color: C.dim }}>{t('asp_rangeHigh')}</span> <span style={{ color: C.text, fontFamily: "var(--font-mono)" }}>{safeToFixed(wyckoffAdvanced.range?.high, 2)}</span></div>
                    <div><span style={{ color: C.dim }}>{t('asp_rangeLow')}</span> <span style={{ color: C.text, fontFamily: "var(--font-mono)" }}>{safeToFixed(wyckoffAdvanced.range?.low, 2)}</span></div>
                    <div><span style={{ color: C.dim }}>{t('asp_support')}</span> <span style={{ color: C.green, fontFamily: "var(--font-mono)" }}>{safeToFixed(wyckoffAdvanced.support, 2)}</span></div>
                    <div><span style={{ color: C.dim }}>{t('asp_resistance')}</span> <span style={{ color: C.red, fontFamily: "var(--font-mono)" }}>{safeToFixed(wyckoffAdvanced.resistance, 2)}</span></div>
                  </div>
                </div>

                {/* Events Timeline */}
                <div style={{ marginBottom: 8 }}>
                  <div style={{ color: C.dim, fontSize: 11, marginBottom: 4, fontWeight: 600 }}>{t('asp_eventsTimeline')}</div>
                  {wyckoffAdvanced.events.length === 0 ? (
                    <div style={{ color: C.mut, textAlign: 'center', padding: 8 }}>{t('asp_noEvents')}</div>
                  ) : (
                    wyckoffAdvanced.events.map((evt: WyckoffEvent, i: number) => (
                      <div key={i} style={{ display: 'flex', gap: 6, padding: '4px 6px', background: 'rgba(255,255,255,0.02)', borderRadius: 'var(--radius-sm)', marginBottom: 3, alignItems: 'flex-start' }}>
                        <div style={{ padding: '1px 5px', borderRadius: 'var(--radius-xs)', fontSize: 11, fontWeight: 700, fontFamily: "var(--font-mono)", background: evt.phase === 'A' ? 'rgba(59,130,246,0.12)' : evt.phase === 'B' ? 'rgba(168,85,247,0.12)' : evt.phase === 'C' ? 'rgba(245,158,11,0.12)' : evt.phase === 'D' ? 'rgba(16,185,129,0.12)' : 'rgba(239,68,68,0.12)', color: evt.phase === 'A' ? C.blue : evt.phase === 'B' ? C.purple : evt.phase === 'C' ? C.yellow : evt.phase === 'D' ? C.green : C.red, border: `1px solid ${evt.phase === 'A' ? 'rgba(59,130,246,0.2)' : evt.phase === 'B' ? 'rgba(168,85,247,0.2)' : evt.phase === 'C' ? 'rgba(245,158,11,0.2)' : evt.phase === 'D' ? 'rgba(16,185,129,0.2)' : 'rgba(239,68,68,0.2)'}`, flexShrink: 0 }}>
                          {evt.type}
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                            <span style={{ color: C.dim, fontSize: 11 }}>{t('asp_phase')} {evt.phase}</span>
                            <span style={{ color: C.text, fontFamily: "var(--font-mono)", fontSize: 11 }}>{evt.price.toFixed(2)}</span>
                          </div>
                          <div style={{ color: C.mut, fontSize: 11 }}>{evt.description}</div>
                        </div>
                        <span style={{ color: C.mut, fontFamily: "var(--font-mono)", fontSize: 11 }}>V:{safeToFixed((evt.volume ?? 0) / 1000, 0)}K</span>
                      </div>
                    ))
                  )}
                </div>
                ...
                {/* Phase Progress */}
                <div>
                  <div style={{ color: C.dim, fontSize: 11, marginBottom: 4, fontWeight: 600 }}>{t('asp_phaseProgress')}</div>
        ...
        {tab === 'elliott' && (
          <div style={{ padding: 8, overflowY: 'auto', flex: 1, minHeight: 0 }}>
            {!elliottAdvanced || elliottAdvanced.counts.length === 0 ? (
              <div style={{ color: C.mut, textAlign: 'center', padding: 24 }}>{t('asp_noElliott')}</div>
            ) : (
              <>
                {/* Dominant Count */}
                  ...
                  <div style={{ padding: 8, borderRadius: 'var(--radius-sm)', marginBottom: 8, background: elliottAdvanced.dominantCount.direction === 'bullish' ? 'rgba(16,185,129,0.06)' : 'rgba(239,68,68,0.06)', border: `1px solid ${elliottAdvanced.dominantCount.direction === 'bullish' ? 'rgba(16,185,129,0.2)' : 'rgba(239,68,68,0.2)'}` }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        ...
                        <span style={{ color: elliottAdvanced.dominantCount.direction === 'bullish' ? C.green : C.red, fontWeight: 700, fontSize: 13 }}>
                          {elliottAdvanced.dominantCount.type === 'impulse' ? t('asp_impulse') : elliottAdvanced.dominantCount.type === 'zigzag' ? t('asp_zigzag') : elliottAdvanced.dominantCount.type === 'flat' ? t('asp_flat') : elliottAdvanced.dominantCount.type === 'triangle' ? t('asp_triangle') : t('asp_complex')} {elliottAdvanced.dominantCount.direction === 'bullish' ? '↑' : '↓'}
                        </span>
                      </div>
                      <div style={{ padding: '1px 6px', borderRadius: 'var(--radius-xs)', fontSize: 11, fontWeight: 700, background: 'rgba(245,158,11,0.12)', color: C.yellow, border: '1px solid rgba(245,158,11,0.2)' }}>
                    ...
                    <div style={{ color: C.dim, fontSize: 11, marginTop: 4 }}>{elliottAdvanced.dominantCount.label}</div>
                    {elliottAdvanced.dominantCount.targetPrice !== null && (
                      <div style={{ color: C.dim, fontSize: 11, marginTop: 2 }}>{t('asp_target')}: <span style={{ color: C.text, fontFamily: "var(--font-mono)" }}>{safeToFixed(elliottAdvanced.dominantCount.targetPrice, 2)}</span></div>
                    )}
                  </div>
                )}
                ...
                {elliottAdvanced.dominantCount && elliottAdvanced.dominantCount.type === 'impulse' && (
                  <div style={{ marginBottom: 8 }}>
                    <div style={{ color: C.dim, fontSize: 11, marginBottom: 4, fontWeight: 600 }}>{t('asp_fibRatios')}</div>
                    {[
                      ...
                      <div key={label} style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 6px', background: 'rgba(255,255,255,0.02)', borderRadius: 'var(--radius-sm)', marginBottom: 2 }}>
                        ...
                        <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: Number.isFinite(val) && target > 0 && Math.abs(val - target) / target < 0.08 ? C.green : C.yellow }}>
                ...
                {elliottAdvanced.counts.length > 1 && (
                  <div>
                    <div style={{ color: C.dim, fontSize: 11, marginBottom: 4, fontWeight: 600 }}>{t('asp_altCounts')}</div>
                    {elliottAdvanced.counts.slice(1).map((count: WaveCount, i: number) => (
                      <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 6px', background: 'rgba(255,255,255,0.02)', borderRadius: 'var(--radius-sm)', marginBottom: 3 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                          <span style={{ color: count.direction === 'bullish' ? C.green : C.red, fontSize: 11 }}>{count.direction === 'bullish' ? '↑' : '↓'}</span>
                          <span style={{ color: C.dim, textTransform: 'capitalize', fontSize: 11 }}>{count.type}</span>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                          <span style={{ color: C.mut, fontSize: 11 }}>{count.label}</span>
                          <span style={{ padding: '1px 4px', borderRadius: 'var(--radius-xs)', fontSize: 11, fontFamily: "var(--font-mono)", background: 'rgba(255,255,255,0.04)', color: C.dim }}>{safeToFixed((count.probability ?? 0) * 100, 0)}%</span>
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          <div style={{ padding: 8, overflowY: 'auto', flex: 1, minHeight: 0 }}>
            {geoList.length > 0 && (
              <div style={{ marginBottom: 10 }}>
                <div style={{ color: C.cyan, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>{t('geometricPatterns')} ({geoList.length})</div>
                {geoList.map((g: any, i: number) => {
                  ...
                  return (
                    <div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'5px 8px', borderRadius: 'var(--radius-sm)', background:C.card, marginBottom:3, border:`1px solid ${col}18` }}>
                      <span style={{ color:col, fontSize: 11, fontWeight:600 }}>{g.direction==='bullish'?'▲':'▼'} {t(GEO_TYPE_TO_I18N[g.type] || g.type)}</span>
                      <span style={{ color:C.mut, fontSize: 11 }}>{Math.round(g.confidence*100)}%</span>
                    </div>
                  );
                })}
              </div>
            ...
            {elliottData && (
              <div style={{ background:C.card, borderRadius: 'var(--radius-sm)', padding:'8px 10px', marginBottom:8, border:`1px solid ${elliottData.direction==='bullish'?C.green:C.red}25` }}>
                <div style={{ color:C.dim, fontSize: 11, marginBottom:3 }}>{t('elliottWaves')}</div>
                <div style={{ color:elliottData.direction==='bullish'?C.green:C.red, fontSize: 13, fontWeight:700 }}>
                  {elliottData.type === '5-wave' ? t('impulse5Wave') : t('abcCorrection')} — {t('wave')} {elliottData.currentWave}
                </div>
                <div style={{ display:'flex', gap:4, marginTop:5 }}>
                  {elliottData.waves?.map((w: any) => (
                    <span key={w.waveNumber} style={{ background:`${elliottData.direction==='bullish'?C.green:C.red}20`, color:elliottData.direction==='bullish'?C.green:C.red, padding:'2px 5px', borderRadius: 'var(--radius-xs)', fontSize: 11, fontWeight:700 }}>{w.waveNumber}</span>
                  ))}
                </div>
                {elliottData.nextTarget && <div style={{ color:C.dim, fontSize: 11, marginTop:4 }}>{t('nextTarget')}: <span style={{ color:C.cyan, fontFamily: "var(--font-mono)" }}>{elliottData.nextTarget.toFixed(2)}</span></div>}
              </div>
            )}
            ...
            {performanceStats && performanceStats.totalPatterns > 0 && (
              <div style={{ background: `${C.gold}08`, border: `1px solid ${C.gold}20`, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 8 }}>
                <div style={{ color: C.gold, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>{t('historicalPatternPerformance')}</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4 }}>
                  <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px', textAlign: 'center' }}>
                    <div style={{ color: C.mut, fontSize: 11 }}>{t('patterns')}</div>
                    <div style={{ color: C.text, fontSize: 11, fontWeight: 700 }}>{performanceStats.totalPatterns}</div>
                  </div>
                  <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px', textAlign: 'center' }}>
                    <div style={{ color: C.mut, fontSize: 11 }}>{t('winRate')}</div>
                    <div style={{ color: (performanceStats.overallWinRate ?? 0) > 0.5 ? C.green : C.red, fontSize: 11, fontWeight: 700 }}>{Math.round((performanceStats.overallWinRate ?? 0) * 100)}%</div>
                  </div>
                  {performanceStats.bestPattern && (
                    <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px', textAlign: 'center' }}>
                      <div style={{ color: C.mut, fontSize: 11 }}>{t('bestPattern')}</div>
                      <div style={{ color: C.green, fontSize: 11, fontWeight: 600 }}>{performanceStats.bestPattern}</div>
                    </div>
                  )}
                  {performanceStats.worstPattern && (
                    <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px', textAlign: 'center' }}>
                      <div style={{ color: C.mut, fontSize: 11 }}>{t('worstPattern')}</div>
                      <div style={{ color: C.red, fontSize: 11, fontWeight: 600 }}>{performanceStats.worstPattern}</div>
                    </div>
                  )}
                </div>
              </div>
            )}
            {geoList.length===0 && !elliottData && !performanceStats && <div style={{ textAlign:'center', padding:20, color:C.dim, fontSize: 11 }}>{t('pressForAnalysis')}</div>}
              ...
              try {
                ...
                return (
                  <div style={{ background: `${C.blue}08`, border: `1px solid ${C.blue}20`, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 8 }}>
                    <div style={{ color: C.blue, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>{t('dynamicATRThresholds')}</div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4 }}>
                      <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px', textAlign: 'center' }}>
                        <div style={{ color: C.mut, fontSize: 11 }}>ATR</div>
                        <div style={{ color: C.text, fontSize: 11, fontWeight: 700, fontFamily: "var(--font-mono)" }}>{thresholds.atrValue.toFixed(2)}</div>
                      </div>
                      <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px', textAlign: 'center' }}>
                        <div style={{ color: C.mut, fontSize: 11 }}>ATR %</div>
                        <div style={{ color: regimeColor, fontSize: 11, fontWeight: 700, fontFamily: "var(--font-mono)" }}>{atrPct.toFixed(2)}%</div>
                      </div>
                      <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px', textAlign: 'center' }}>
                        <div style={{ color: C.mut, fontSize: 11 }}>{t('retracement')}</div>
                        <div style={{ color: C.text, fontSize: 11, fontWeight: 700, fontFamily: "var(--font-mono)" }}>{(thresholds.pullback * 100).toFixed(1)}%</div>
                      </div>
                      <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px', textAlign: 'center' }}>
                        <div style={{ color: C.mut, fontSize: 11 }}>{t('peakSimilarity')}</div>
                        <div style={{ color: C.text, fontSize: 11, fontWeight: 700, fontFamily: "var(--font-mono)" }}>{(thresholds.peakSimilarity * 100).toFixed(1)}%</div>
                      </div>
                      <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px', textAlign: 'center' }}>
                        <div style={{ color: C.mut, fontSize: 11 }}>{t('shoulderDivergence')}</div>
                        <div style={{ color: C.text, fontSize: 11, fontWeight: 700, fontFamily: "var(--font-mono)" }}>{(thresholds.shoulderTolerance * 100).toFixed(1)}%</div>
                      </div>
                      <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px', textAlign: 'center' }}>
                        <div style={{ color: C.mut, fontSize: 11 }}>{t('breakoutConfirmation')}</div>
                        <div style={{ color: C.text, fontSize: 11, fontWeight: 700, fontFamily: "var(--font-mono)" }}>{(thresholds.breakoutConfirm * 100).toFixed(1)}%</div>
                      </div>
                    </div>
          ...
          <div style={{ padding: 8, overflowY: 'auto', flex: 1, minHeight: 0 }}>
            {mtfLoading && (
              <div style={{ textAlign: 'center', padding: 12 }}>
                ...
                <div style={{ color: C.dim, fontSize: 11, marginTop: 6 }}>{t('asp_analyzingTimeframes')}</div>
              </div>
            )}
            {mtfResult && !mtfLoading && (
              <>
                ...
                <div style={{ background: mtfResult.confluenceDirection === 'bullish' ? `${C.green}12` : mtfResult.confluenceDirection === 'bearish' ? `${C.red}12` : `${C.yellow}08`, border: `1px solid ${mtfResult.confluenceDirection === 'bullish' ? C.green : mtfResult.confluenceDirection === 'bearish' ? C.red : C.yellow}30`, borderRadius: 'var(--radius-md)', padding: '10px 12px', marginBottom: 8 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontSize: 19 }}>{mtfResult.confluenceDirection === 'bullish' ? '▲' : mtfResult.confluenceDirection === 'bearish' ? '▼' : '◆'}</span>
                      <div>
                        <div style={{ color: mtfResult.confluenceDirection === 'bullish' ? C.green : mtfResult.confluenceDirection === 'bearish' ? C.red : C.yellow, fontSize: 13, fontWeight: 800 }}>
                          MTF Confluence: {mtfResult.confluenceDirection === 'bullish' ? t('asp_bullish') : mtfResult.confluenceDirection === 'bearish' ? t('asp_bearish') : t('asp_neutral')}
                        </div>
                        <div style={{ color: C.dim, fontSize: 11 }}>{mtfResult.agreeingTFs} / {mtfResult.totalTFs}</div>
                      </div>
                    </div>
                    <div style={{ textAlign: 'center' }}>
                      ...
                      <div style={{ color: C.mut, fontSize: 11 }}>{t('asp_totalConfluence')}</div>
                    </div>
                  </div>
                </div>
                ...
                <div style={{ marginBottom: 8 }}>
                  <div style={{ color: C.cyan, fontSize: 11, fontWeight: 700, marginBottom: 4 }}>{t('asp_perTimeframeAnalysis')}</div>
                      ...
                      <div key={tf.timeframe} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 8px', borderRadius: 'var(--radius-sm)', marginBottom: 3, background: C.card, border: `1px solid ${tfCol}15` }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          ...
                          <div>
                            ...
                            <div style={{ color: C.mut, fontSize: 11 }}>{tf.trendState === 'uptrend' ? t('asp_uptrend') : tf.trendState === 'downtrend' ? t('asp_downtrend') : tf.trendState === 'ranging' ? t('asp_sideways') : tf.trendState === 'counter-uptrend' ? t('asp_bullishReversal') : t('asp_bearishReversal')} | Momentum: {tf.momentum === 'accelerating' ? t('asp_accelerating') : tf.momentum === 'decelerating' ? t('asp_decelerating') : tf.momentum === 'diverging' ? t('asp_divergence') : t('asp_normal')}</div>
                          </div>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                          ...
                          <span style={{ color: C.mut, fontSize: 11 }}>{Math.round(tf.strength * 100)}%</span>
                        </div>
                      </div>
                    );
                  })}
            ...
            {!mtfResult && !mtfLoading && (
              <div style={{ textAlign: 'center', padding: 24, color: C.dim }}>
                <div style={{ fontSize: 27, marginBottom: 8 }}>📊</div>
                <div style={{ fontSize: 11 }}>{t('asp_clickToAnalyze')}</div>
              </div>
            )}
          </div>
        )}
        ...
        {tab === 'alerts' && (
          <div style={{ padding: 8, overflowY: 'auto', flex: 1, minHeight: 0 }}>
            {/* Active Alerts */}
            <div style={{ marginBottom: 10 }}>
              <div style={{ color: C.cyan, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>🚨 Smart Alerts ({smartAlerts.length})</div>
              {smartAlerts.length === 0 ? (
                <div style={{ color: C.mut, fontSize: 11, padding: '8px', textAlign: 'center', background: C.card, borderRadius: 'var(--radius-sm)' }}>
                  No alerts currently — activates at 3+ signal confluence
                </div>
              ) : (
                smartAlerts.slice(0, 10).map((alert, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 8px', borderRadius: 'var(--radius-sm)', background: C.card, marginBottom: 3, border: `1px solid ${pColor}25` }}>
                      ...
                      <span style={{ background: `${pColor}20`, color: pColor, fontSize: 11, fontWeight: 700, padding: '2px 5px', borderRadius: 'var(--radius-xs)' }}>
                        {alert.priority === 'critical' ? t('asp_critical') : alert.priority === 'high' ? t('asp_high') : t('asp_medium')}
                      </span>
                    </div>
                  );
                })
            ...
            {liquidityResult && (
              <div style={{ marginBottom: 10 }}>
                <div style={{ color: C.purple, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>💧 Liquidity Zones ({liquidityResult.activeZones} active / {liquidityResult.sweptZones} swept)</div>
          ...
          <div style={{ padding: 8, overflowY: 'auto', flex: 1, minHeight: 0 }}>
            ...
            {proposalStats && (
              <div style={{ background: `${C.gold}08`, border: `1px solid ${C.gold}20`, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 10 }}>
                <div style={{ color: C.gold, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>💰 Proposal Statistics</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 4 }}>
                  <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px', textAlign: 'center' }}>
                    <div style={{ color: C.mut, fontSize: 11 }}>All</div>
                    <div style={{ color: C.text, fontSize: 11, fontWeight: 700 }}>{proposalStats.total}</div>
                  </div>
                  <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px', textAlign: 'center' }}>
                    <div style={{ color: C.mut, fontSize: 11 }}>Success Rate</div>
                    <div style={{ color: proposalStats.winRate > 0.5 ? C.green : C.red, fontSize: 11, fontWeight: 700 }}>{Math.round(proposalStats.winRate * 100)}%</div>
                  </div>
                  <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px', textAlign: 'center' }}>
                    <div style={{ color: C.mut, fontSize: 11 }}>Avg R:R</div>
                    <div style={{ color: C.cyan, fontSize: 11, fontWeight: 700 }}>1:{safeToFixed(proposalStats.avgRR, 1, '—')}</div>
                  </div>
                </div>
              </div>
            )}
            {/* Trade Proposals */}
            <div style={{ color: C.cyan, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>📋 Trade Proposals ({tradeProposals.length})</div>
            {tradeProposals.length === 0 ? (
              <div style={{ color: C.mut, fontSize: 11, padding: '8px', textAlign: 'center', background: C.card, borderRadius: 'var(--radius-sm)' }}>
                No proposals currently — activates at 3+ signal confluence and confidence ≥ 60%
              </div>
            ) : (
              tradeProposals.slice(0, 5).map((proposal, i) => {
                ...
                return (
                  <div key={proposal.id} style={{ background: C.card, border: `1px solid ${dirColor}25`, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 6 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                      <span style={{ color: dirColor, fontSize: 11, fontWeight: 700 }}>
                        {proposal.direction === 'bullish' ? '▲ Buy' : '▼ Sell'}
                      </span>
                      <span style={{ background: `${dirColor}20`, color: dirColor, fontSize: 11, fontWeight: 700, padding: '2px 5px', borderRadius: 'var(--radius-xs)' }}>
                        R:R 1:{proposal.rrRatio}
                      </span>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 3, fontSize: 11 }}>
                      <div><span style={{ color: C.mut }}>Entry:</span> <span style={{ color: C.text, fontFamily: "var(--font-mono)" }}>{safeToFixed(proposal.entryPrice, 2)}</span></div>
                      <div><span style={{ color: C.mut }}>Stop:</span> <span style={{ color: C.red, fontFamily: "var(--font-mono)" }}>{safeToFixed(proposal.stopLoss, 2)}</span></div>
        ...
        {tab === 'adaptive' && (
          <div style={{ padding: 8, overflowY: 'auto', flex: 1, minHeight: 0 }}>
            ...
            <div style={{ background: `${C.purple}08`, border: `1px solid ${C.purple}20`, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 10 }}>
              <div style={{ color: C.purple, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>🧠 Adaptive Bayesian System</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 4 }}>
                <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px', textAlign: 'center' }}>
                  ...
                  <div style={{ color: marketRegime === 'trending' ? C.green : marketRegime === 'volatile' ? C.red : marketRegime === 'ranging' ? C.yellow : C.dim, fontSize: 11, fontWeight: 700 }}>
                    {marketRegime === 'trending' ? 'Trending' : marketRegime === 'volatile' ? 'Volatile' : marketRegime === 'ranging' ? t('asp_sideways') : 'Calm'}
                  </div>
                </div>
                <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px', textAlign: 'center' }}>
                ...
                <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px', textAlign: 'center' }}>
                  <div style={{ color: C.mut, fontSize: 11 }}>Adaptive Confidence</div>
                  <div style={{ color: C.cyan, fontSize: 11, fontWeight: 700 }}>{adaptiveResult ? `${Math.round(adaptiveResult.confidence * 100)}%` : '—'}</div>
                </div>
              </div>
            </div>
            ...
            {adaptiveResult && adaptiveResult.insights.length > 0 && (
              <div style={{ marginBottom: 10 }}>
                <div style={{ color: C.cyan, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>Learning Insights</div>
                {adaptiveResult.insights.map((insight, i) => (
                  <div key={i} style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 8px', marginBottom: 3, fontSize: 11, color: C.dim }}>{insight}</div>
                ))}
              </div>
            )}
            ...
            {adaptiveResult && adaptiveResult.sourceContributions.length > 0 && (
              <div style={{ marginBottom: 10 }}>
                <div style={{ color: C.gold, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>Source Contribution (Adaptive Weights)</div>
                {adaptiveResult.sourceContributions.slice(0, 8).map((sc, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '3px 8px', borderRadius: 'var(--radius-sm)', background: C.card, marginBottom: 2 }}>
                    <span style={{ fontSize: 11, color: C.text }}>{sc.source} {sc.isUserOverride ? '👤' : ''}</span>
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                      <span style={{ fontSize: 11, color: C.mut }}>Weight: <b style={{ color: sc.adaptiveWeight > 1.2 ? C.green : sc.adaptiveWeight < 0.8 ? C.red : C.dim }}>{sc.adaptiveWeight.toFixed(2)}</b></span>
                      <span style={{ fontSize: 11, color: C.mut }}>Win: <b style={{ color: sc.winRate > 0.6 ? C.green : sc.winRate < 0.4 ? C.red : C.dim }}>{Math.round(sc.winRate * 100)}%</b></span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {adaptiveSummary && (
              <div style={{ background: `${C.blue}08`, border: `1px solid ${C.blue}20`, borderRadius: 'var(--radius-sm)', padding: '8px 10px' }}>
                <div style={{ color: C.blue, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>Learning Summary</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4, fontSize: 11 }}>
                  <div><span style={{ color: C.mut }}>Total Sources:</span> <span style={{ color: C.text }}>{adaptiveSummary.totalSources}</span></div>
                  <div><span style={{ color: C.mut }}>Adapted Sources:</span> <span style={{ color: C.text }}>{adaptiveSummary.adaptedSources}</span></div>
                  ...
                  <div><span style={{ color: C.mut }}>Dominant Regime:</span> <span style={{ color: C.text }}>{adaptiveSummary.dominantRegime === 'trending' ? 'Trending' : adaptiveSummary.dominantRegime === 'volatile' ? 'Volatile' : adaptiveSummary.dominantRegime === 'ranging' ? t('asp_sideways') : 'Calm'}</span></div>
                </div>
              </div>
            )}
          </div>
        ...
        {tab === 'rules' && (
          <div style={{ padding: 8, overflowY: 'auto', flex: 1, minHeight: 0 }}>
            <div style={{ background: `${C.blue}08`, border: `1px solid ${C.blue}20`, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 10 }}>
              <div style={{ color: C.blue, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>📐 Visual Rule Builder</div>
            ...
            {visualRuleResults.length > 0 ? (
              <div style={{ marginBottom: 10 }}>
                <div style={{ color: C.gold, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>Active Rules ({visualRuleResults.length})</div>
                    ...
                    <div key={i} style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '6px 8px', marginBottom: 4, border: `1px solid ${dirColor}20` }}>
                      ...
                      <div style={{ display: 'flex', gap: 3, marginTop: 3, flexWrap: 'wrap' }}>
                        {rr.result.trace.filter(t => t.matched).map((t, j) => (
                          <span key={j} style={{ background: `${dirColor}10`, color: dirColor, fontSize: 11, padding: '1px 4px', borderRadius: 'var(--radius-xs)' }}>{t.signalType}</span>
                        ))}
                      </div>
                    </div>
                  );
              ...
              <div style={{ color: C.mut, fontSize: 11, textAlign: 'center', padding: 20 }}>No active rules — create one from the block library</div>
            )}

            {/* Signal Block Library */}
            <div style={{ marginBottom: 10 }}>
              <div style={{ color: C.cyan, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>Signal Block Library</div>
          ...
          <div style={{ padding: 8, overflowY: 'auto', flex: 1, minHeight: 0 }}>
            ...
            {paperAccountState && (
              <div style={{ background: `${C.green}08`, border: `1px solid ${C.green}20`, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 10 }}>
                <div style={{ color: C.green, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>📝 Paper Trading</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 4 }}>
                  <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px', textAlign: 'center' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 4, marginTop: 4 }}>
                  <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '3px 5px', textAlign: 'center' }}>
                    <div style={{ color: C.mut, fontSize: 11 }}>Trades</div>
                    <div style={{ color: C.text, fontSize: 11 }}>{paperAccountState.totalTrades}</div>
                  </div>
                  <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '3px 5px', textAlign: 'center' }}>
                    <div style={{ color: C.mut, fontSize: 11 }}>Profit</div>
                    <div style={{ color: C.green, fontSize: 11 }}>{paperAccountState.wins}</div>
                  </div>
                  <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '3px 5px', textAlign: 'center' }}>
                    <div style={{ color: C.mut, fontSize: 11 }}>Loss</div>
                    <div style={{ color: C.red, fontSize: 11 }}>{paperAccountState.losses}</div>
                  </div>
                  <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '3px 5px', textAlign: 'center' }}>
                    <div style={{ color: C.mut, fontSize: 11 }}>Sharpe</div>
                    <div style={{ color: C.cyan, fontSize: 11 }}>{paperAccountState.sharpeRatio.toFixed(2)}</div>
                  </div>
                </div>
              </div>
            )}
            ...
            {paperComparison && (
              <div style={{ background: `${C.gold}08`, border: `1px solid ${C.gold}20`, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 10 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 4, fontSize: 11 }}>
                  ...
                  <div><span style={{ color: C.mut }}>Buy & Hold:</span> <span style={{ color: C.text }}>{paperComparison.buyAndHoldReturnPct.toFixed(2)}%</span></div>
            ...
            {paperAccountState && signal && signal.dir !== 'WAIT' && (
              <div style={{ marginBottom: 10 }}>
                <div style={{ color: C.gold, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>⚡ Quick Execute</div>
                    ...
                    onClick={() => {
                      try {
                        ...
                        const trade = openPaperTrade({
                          ...
                          entryReasonAr: `Signal ${dir === 'long' ? t('asp_uptrend') : t('asp_downtrend')} — Confidence ${Math.round(signal.conf * 100)}%`,
                ...
                {getOpenPaperTrades().length > 0 && (
                  <div style={{ marginTop: 4, fontSize: 11, color: C.dim }}>
                    Open trades: {getOpenPaperTrades().length} | SL: {signal.sl?.toFixed(2)} | TP: {signal.tp?.toFixed(2)}
                  </div>
                )}
              </div>
            )}
        ...
        {tab === 'scanner' && (
          <div style={{ padding: 8, overflowY: 'auto', flex: 1, minHeight: 0 }}>
            <div style={{ background: `${C.cyan}08`, border: `1px solid ${C.cyan}20`, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 10 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ color: C.cyan, fontSize: 11, fontWeight: 700 }}>🔍 Market Scanner</div>
              ...
              <>
                ...
                <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 10 }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 4 }}>
                    <div style={{ textAlign: 'center' }}><div style={{ color: C.mut, fontSize: 11 }}>{t('asp_uptrend')}</div><div style={{ color: C.green, fontSize: 11, fontWeight: 700 }}>{scanResult.marketOverview.bullishPct}%</div></div>
                    <div style={{ textAlign: 'center' }}><div style={{ color: C.mut, fontSize: 11 }}>{t('asp_downtrend')}</div><div style={{ color: C.red, fontSize: 11, fontWeight: 700 }}>{scanResult.marketOverview.bearishPct}%</div></div>
                    <div style={{ textAlign: 'center' }}><div style={{ color: C.mut, fontSize: 11 }}>{t('asp_medium')}</div><div style={{ color: C.cyan, fontSize: 11, fontWeight: 700 }}>{scanResult.marketOverview.avgStrength}</div></div>
                    <div style={{ textAlign: 'center' }}><div style={{ color: C.mut, fontSize: 11 }}>Scanned</div><div style={{ color: C.text, fontSize: 11, fontWeight: 700 }}>{scanResult.totalScanned}</div></div>
                  </div>
                </div>
                    ...
                    {scanResult.topBullish.slice(0, 5).map((asset, i) => (
                      <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 8px', borderRadius: 'var(--radius-sm)', background: C.card, marginBottom: 2 }}>
                        ...
                        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                          ...
                          <span style={{ fontSize: 11, color: asset.change24h > 0 ? C.green : C.red }}>{asset.change24h > 0 ? '+' : ''}{asset.change24h.toFixed(1)}%</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {scanResult.sectorSummary.length > 0 && (
                  <div>
                    <div style={{ color: C.gold, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>Sector Summary</div>
                    {scanResult.sectorSummary.slice(0, 6).map((sector, i) => (
                      <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 8px', borderRadius: 'var(--radius-sm)', background: C.card, marginBottom: 2 }}>
                        <span style={{ fontSize: 11, color: C.text }}>{sector.labelAr.split('—')[0].trim()}</span>
                        <div style={{ display: 'flex', gap: 4 }}>
                          <span style={{ fontSize: 11, color: C.green }}>{sector.bullishCount}▲</span>
                          <span style={{ fontSize: 11, color: C.red }}>{sector.bearishCount}▼</span>
                          <span style={{ fontSize: 11, color: C.cyan }}>{Math.round(sector.avgStrength)}%</span>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        {tab === 'council' && (
          <div style={{ padding: 8, overflowY: 'auto', flex: 1, minHeight: 0 }}>
            <div style={{ background: `${C.purple}08`, border: `1px solid ${C.purple}20`, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 10 }}>
              <div style={{ color: C.purple, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>🤖 AI Council — Live Model Predictions</div>
              <div style={{ fontSize: 11, color: C.dim }}>Passes actual analysis data to AI models and compares predictions with algorithmic engines.</div>
            </div>

            {/* LIVE Council Model Results — NEW */}
            {councilAnalyses.length > 0 && (
              <div style={{ marginBottom: 10 }}>
                <div style={{ color: C.purple, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>🔬 Model Predictions ({councilAnalyses.length} models)</div>
                {councilAnalyses.map((a, i) => {
                  const dirCol = a.direction === 'BUY' ? C.green : a.direction === 'SELL' ? C.red : C.dim;
                  const dirAr = a.direction === 'BUY' ? t('asp_buy') : a.direction === 'SELL' ? t('asp_sell') : 'Wait';
                  ...
                  return (
                    <div key={i} style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '6px 8px', marginBottom: 4, borderLeft: `2px solid ${dirCol}` }}>
                      ...
                      {a.reasoning && (
                        <div style={{ fontSize: 11, color: C.dim, lineHeight: 1.3, maxHeight: 30, overflow: 'hidden', textOverflow: 'ellipsis' }}>{a.reasoning.substring(0, 120)}</div>
                      )}
                    </div>
                  );
                })}
            ...
            {signal && councilAnalyses.length > 0 && (
              <div style={{ background: `${signal.dir === 'BUY' ? C.green : signal.dir === 'SELL' ? C.red : C.dim}10`, border: `1px solid ${signal.dir === 'BUY' ? C.green : signal.dir === 'SELL' ? C.red : C.dim}30`, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 10 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: signal.dir === 'BUY' ? C.green : signal.dir === 'SELL' ? C.red : C.dim }}>
                    {signal.dir === 'BUY' ? '▲ Bullish Consensus' : signal.dir === 'SELL' ? '▼ Bearish Consensus' : '◆ Wait'}
                  </span>
                  <span style={{ fontSize: 11, color: C.mut }}>Confidence {Math.round(signal.conf * 100)}%</span>
                </div>
                {signal.bayesianDir && (
                  <div style={{ marginTop: 4, fontSize: 11, color: C.dim }}>
                    Bayesian: {signal.bayesianDir === 'BUY' ? t('asp_uptrend') : signal.bayesianDir === 'SELL' ? t('asp_downtrend') : t('asp_neutral')} ({Math.round((signal.bayesianConf || 0) * 100)}%)
                    {signal.fusionScore !== undefined && ` | Confluence: ${signal.fusionScore}%`}
                  </div>
                )}
              </div>
            )}
            ...
            {aiVsAlgoStats && (
              <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 10 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 4 }}>
                  <div style={{ textAlign: 'center' }}><div style={{ color: C.mut, fontSize: 11 }}>AI Win</div><div style={{ color: C.purple, fontSize: 11, fontWeight: 700 }}>{Math.round(aiVsAlgoStats.aiWinRate * 100)}%</div></div>
                  <div style={{ textAlign: 'center' }}><div style={{ color: C.mut, fontSize: 11 }}>Algo Win</div><div style={{ color: C.cyan, fontSize: 11, fontWeight: 700 }}>{Math.round(aiVsAlgoStats.algoWinRate * 100)}%</div></div>
                  <div style={{ textAlign: 'center' }}><div style={{ color: C.mut, fontSize: 11 }}>Agreement</div><div style={{ color: C.text, fontSize: 11, fontWeight: 700 }}>{Math.round(aiVsAlgoStats.agreementRate * 100)}%</div></div>
                </div>
                {aiVsAlgoStats.totalVerified > 0 && (
                  <div style={{ marginTop: 6, fontSize: 11, color: aiVsAlgoStats.aiBetter ? C.purple : C.cyan, textAlign: 'center' }}>
                    {aiVsAlgoStats.aiBetter ? 'AI currently leading' : 'Algorithms currently leading'} ({aiVsAlgoStats.totalVerified} verified predictions)
                  </div>
                )}
              </div>
            )}
            ...
            {aiBridgePayload && (
              <details style={{ marginBottom: 10 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4, fontSize: 11, marginTop: 5 }}>
                  ...
                  <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '3px 6px' }}><span style={{ color: C.mut }}>Price:</span> <span style={{ color: C.text }}>{aiBridgePayload.currentPrice.toFixed(2)}</span></div>
                  <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '3px 6px' }}><span style={{ color: C.mut }}>Regime:</span> <span style={{ color: C.text }}>{aiBridgePayload.regime}</span></div>
                  <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '3px 6px' }}><span style={{ color: C.mut }}>Bayesian:</span> <span style={{ color: aiBridgePayload.bayesian.direction === 'bullish' ? C.green : aiBridgePayload.bayesian.direction === 'bearish' ? C.red : C.dim }}>{aiBridgePayload.bayesian.direction} ({Math.round(aiBridgePayload.bayesian.confidence * 100)}%)</span></div>
                  ...
                  <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '3px 6px' }}><span style={{ color: C.mut }}>Wyckoff:</span> <span style={{ color: C.text }}>{aiBridgePayload.wyckoffSummary.scheme}</span></div>
            ...
            {!aiBridgePayload && councilAnalyses.length === 0 && (
              <div style={{ color: C.mut, fontSize: 11, textAlign: 'center', padding: 20 }}>Run analysis first to see AI Council data AI</div>
            )}
          </div>
        )}
        ...
        {tab === 'backtest' && (
          <div style={{ padding: 8, overflowY: 'auto', flex: 1, minHeight: 0 }}>
            <div style={{ background: `${C.blue}08`, border: `1px solid ${C.blue}20`, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 10 }}>
              <div style={{ color: C.blue, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>⏪ Visual Backtest — Visual Backtest</div>
              <div style={{ fontSize: 11, color: C.dim }}>Replays historical signals and tracks their profitability. Green = correct signal, Red = wrong, Yellow = pending.</div>
            </div>
            ...
            {backtestStats && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 4, marginBottom: 10 }}>
                ...
                <div style={{ background: `${C.gold}10`, border: `1px solid ${C.gold}30`, borderRadius: 'var(--radius-sm)', padding: '6px 8px', textAlign: 'center' }}>
                  <div style={{ color: C.mut, fontSize: 11 }}>Avg Profit</div>
            ...
            {backtestStats && (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 3, marginBottom: 10 }}>
                <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px', textAlign: 'center' }}>
                  ...
                  <div style={{ color: C.mut, fontSize: 11 }}>success</div>
                </div>
                <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px', textAlign: 'center' }}>
                ...
                <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px', textAlign: 'center' }}>
                  <div style={{ color: C.yellow, fontSize: 11, fontWeight: 700 }}>{backtestStats.breakevens}</div>
                  <div style={{ color: C.mut, fontSize: 11 }}>{t('asp_tie')}</div>
                </div>
                <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px', textAlign: 'center' }}>
                  <div style={{ color: C.dim, fontSize: 11, fontWeight: 700 }}>{backtestStats.pending}</div>
                  <div style={{ color: C.mut, fontSize: 11 }}>Pending</div>
                </div>
              </div>
            )}
            ...
            {backtestStats && Object.entries(backtestStats.bySource).length > 0 && (
              <div style={{ marginBottom: 10 }}>
                <div style={{ color: C.blue, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>📊 Success Rate by Source</div>
                {Object.entries(backtestStats.bySource).sort((a, b) => b[1].winRate - a[1].winRate).map(([source, stats]: [string, any]) => (
                  <div key={source} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3, padding: '3px 6px', background: C.card, borderRadius: 'var(--radius-sm)' }}>
                    ...
                    <span style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: stats.winRate > 0.6 ? C.green : stats.winRate > 0.4 ? C.yellow : C.red, fontWeight: 700, width: 32, textAlign: 'right' }}>{Math.round(stats.winRate * 100)}%</span>
                    <span style={{ fontSize: 11, color: C.mut, width: 20, textAlign: 'right' }}>({stats.total})</span>
                  </div>
                ))}
              </div>
            )}
            ...
            {backtestSignals.length > 0 && (
              <div>
                ...
                {backtestSignals.slice(0, 15).map((s, i) => {
                  ...
                  const outAr = s.outcome === 'win' ? '✓ success' : s.outcome === 'loss' ? '✗ Loss' : s.outcome === 'breakeven' ? '◆ Breakeven' : '⏳ Pending';
                  return (
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '3px 6px', background: C.card, borderRadius: 'var(--radius-sm)', marginBottom: 2, borderLeft: `2px solid ${outCol}` }}>
                      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                        <span style={{ fontSize: 11, color: s.direction === 'bullish' ? C.green : s.direction === 'bearish' ? C.red : C.dim }}>{s.direction === 'bullish' ? '▲' : '▼'}</span>
            ...
            {!backtestStats && <div style={{ color: C.mut, fontSize: 11, textAlign: 'center', padding: 20 }}>Run analysis first to see visual backtest results</div>}
          </div>
        )}
        ...
        {tab === 'confluence' && (
          <div style={{ padding: 8, overflowY: 'auto', flex: 1, minHeight: 0 }}>
            <div style={{ background: `${C.gold}08`, border: `1px solid ${C.gold}20`, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 10 }}>
              <div style={{ color: C.gold, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>🎯 Confluence Zones</div>
              <div style={{ fontSize: 11, color: C.dim }}>Identifies zones where multiple analysis engines agree on the same price level. The more agreeing signals, the stronger the zone.</div>
            </div>

            {confluenceZones.length > 0 ? (
              <div>
                <div style={{ color: C.gold, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>📍 Discovered Zones ({confluenceZones.length})</div>
                {confluenceZones.sort((a, b) => b.score - a.score).map((z, i) => {
                  ...
                  const strengthAr = z.strength === 'extreme' ? 'Extreme' : z.strength === 'strong' ? 'Strong' : z.strength === 'moderate' ? 'Medium' : 'Weak';
                  return (
                    <div key={z.id} style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '6px 8px', marginBottom: 6, borderLeft: `3px solid ${strengthCol}` }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                        <div style={{ display: 'flex', gap: 5, alignItems: 'center' }}>
                          <span style={{ fontSize: 11, color: z.direction === 'bullish' ? C.green : z.direction === 'bearish' ? C.red : C.dim, fontWeight: 700 }}>
                            {z.direction === 'bullish' ? '▲ Bullish' : z.direction === 'bearish' ? '▼ Bearish' : '◆ Neutral'}
                          </span>
                      ...
                      <div style={{ display: 'flex', gap: 4, marginBottom: 4 }}>
                        <div style={{ flex: 1, background: `${C.gold}08`, borderRadius: 'var(--radius-xs)', padding: '2px 4px', textAlign: 'center' }}>
                          ...
                          <div style={{ color: C.mut, fontSize: 11 }}>Confluence</div>
                        </div>
                        ...
                        <div style={{ flex: 1, background: `${C.purple}08`, borderRadius: 'var(--radius-xs)', padding: '2px 4px', textAlign: 'center' }}>
                          ...
                          <div style={{ color: C.mut, fontSize: 11 }}>Distance</div>
                        </div>
                      </div>
                      ...
                      {z.isActive && <div style={{ marginTop: 3, fontSize: 11, color: C.gold, fontWeight: 700 }}>⚡ Price is near this zone</div>}
                    </div>
                  );
                })}
              </div>
            ) : (
              <div style={{ color: C.mut, fontSize: 11, textAlign: 'center', padding: 20 }}>Run analysis first to see confluence zones</div>
            )}
          </div>
        )}
        ...
        {tab === 'explain' && (
          <div style={{ padding: 8, overflowY: 'auto', flex: 1, minHeight: 0 }}>
            <div style={{ background: `${C.purple}08`, border: `1px solid ${C.purple}20`, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 10 }}>
              <div style={{ color: C.purple, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>❓ Why? — AI Explanation</div>
              <div style={{ fontSize: 11, color: C.dim }}>Click on any Signal to learn why it was triggered, what supporting data exists, and what invalidates or confirms it.</div>
            </div>
              ...
              <div style={{ maxHeight: 120, overflowY: 'auto' }}>
                {patterns.filter(p => p.direction !== 'neutral').slice(0, 20).map((p, i) => {
                  ...
                  return (
                    ...
                    }} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', padding: '4px 8px', borderRadius: 'var(--radius-sm)', border: `1px solid ${isSelected ? C.purple : C.border}`, background: isSelected ? `${C.purple}15` : C.card, color: C.text, fontSize: 11, cursor: 'pointer', outline: 'none', marginBottom: 2, fontFamily: 'inherit', textAlign: 'left' }}>
                ...
                {patterns.filter(p => p.direction !== 'neutral').length === 0 && (
                  <div style={{ color: C.mut, fontSize: 11, padding: 10, textAlign: 'center' }}>No signals yet — run analysis first</div>
                )}
              </div>
            </div>
            ...
            {signalExplanation && (
              <div>
                <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 8 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
                    <span style={{ color: signalExplanation.signal.direction === 'bullish' ? C.green : signalExplanation.signal.direction === 'bearish' ? C.red : C.dim, fontSize: 11, fontWeight: 800 }}>
                      {signalExplanation.signal.direction === 'bullish' ? '▲ Bullish' : signalExplanation.signal.direction === 'bearish' ? '▼ Bearish' : '◆ Neutral'}
                    </span>
                  ...
                  <div style={{ display: 'flex', gap: 4 }}>
                    <span style={{ fontSize: 11, padding: '2px 5px', borderRadius: 'var(--radius-xs)', background: `${C.green}10`, color: C.green, border: `1px solid ${C.green}30` }}>✓ Confirm: {signalExplanation.confirmationAr}</span>
                    <span style={{ fontSize: 11, padding: '2px 5px', borderRadius: 'var(--radius-xs)', background: `${C.red}10`, color: C.red, border: `1px solid ${C.red}30` }}>✗ Invalidate: {signalExplanation.invalidationAr}</span>
                  </div>
                </div>
                ...
                {signalExplanation.factors.length > 0 && (
                  <div style={{ marginBottom: 8 }}>
                    <div style={{ color: C.cyan, fontSize: 11, fontWeight: 700, marginBottom: 4 }}>🔧 Influencing Factors</div>
                ...
                {signalExplanation.relatedSignals.length > 0 && (
                  <div>
                    <div style={{ color: C.gold, fontSize: 11, fontWeight: 700, marginBottom: 4 }}>🔗 Related Signals</div>
                ...
                <div style={{ marginTop: 8, padding: '4px 8px', borderRadius: 'var(--radius-sm)', background: signalExplanation.riskLevel === 'low' ? `${C.green}08` : signalExplanation.riskLevel === 'medium' ? `${C.yellow}08` : `${C.red}08`, border: `1px solid ${signalExplanation.riskLevel === 'low' ? C.green : signalExplanation.riskLevel === 'medium' ? C.yellow : C.red}30` }}>
                  <span style={{ fontSize: 11, color: signalExplanation.riskLevel === 'low' ? C.green : signalExplanation.riskLevel === 'medium' ? C.yellow : C.red, fontWeight: 700 }}>
                    Risk Level: {signalExplanation.riskLevel === 'low' ? t('lowLabel') : signalExplanation.riskLevel === 'medium' ? t('asp_medium') : 'High'}
                  </span>
                  {signalExplanation.historicalWinRate !== null && (
                    <span style={{ fontSize: 11, color: C.dim, marginRight: 8 }}>| Historically: {Math.round(signalExplanation.historicalWinRate * 100)}% success</span>
                  )}
                </div>
              </div>
            )}
        ...
        {tab === 'correlate' && (
          <div style={{ padding: 8, overflowY: 'auto', flex: 1, minHeight: 0 }}>
            <div style={{ background: `${C.cyan}08`, border: `1px solid ${C.cyan}20`, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 10 }}>
              <div style={{ color: C.cyan, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>🔗 Correlation Engine</div>
              <div style={{ fontSize: 11, color: C.dim }}>Tracks which signal combinations work best together. For example, "BOS Bullish + Wyckoff accumulation" has a 72% win rate, while "BOS Bullish alone" only 54%.</div>
            </div>
            ...
            {correlationMatrix && (
              <>
                ...
                {correlationMatrix.topCombinations.length > 0 && (
                  <div style={{ marginBottom: 10 }}>
                    <div style={{ color: C.gold, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>🏆 Best Combinations</div>
                      ...
                      return (
                        <div key={i} style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '6px 8px', marginBottom: 4, borderLeft: `3px solid ${col}` }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 3 }}>
                            <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
                              {combo.sources.map((s, j) => (
                                <span key={j} style={{ fontSize: 11, padding: '1px 4px', borderRadius: 'var(--radius-xs)', background: `${C.cyan}15`, color: C.cyan }}>{s}</span>
                              ))}
                            </div>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                            ...
                            <span style={{ fontSize: 11, color: C.mut }}>({combo.sampleSize} samples)</span>
                          </div>
                        </div>
                      );
                    })}
                ...
                {correlationMatrix.correlations.length > 0 && (
                  <div>
                    <div style={{ color: C.cyan, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>📊 Binary Correlations</div>
                      ...
                      .map((corr, i) => {
                        ...
                        return (
                          <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr auto auto', gap: 4, padding: '3px 6px', background: C.card, borderRadius: 'var(--radius-sm)', marginBottom: 2, alignItems: 'center' }}>
                            <span style={{ fontSize: 11, color: C.text, fontWeight: 600 }}>{corr.sourceA}</span>
                            <span style={{ fontSize: 11, color: C.mut }}>×</span>
                            <span style={{ fontSize: 11, color: C.text, fontWeight: 600 }}>{corr.sourceB}</span>
                            <span style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: liftCol, fontWeight: 700 }}>×{corr.lift.toFixed(2)}</span>
                            <span style={{ fontSize: 11, color: C.mut }}>{Math.round(corr.combinedWinRate * 100)}%</span>
                          </div>
                        );
                      })}
                  </div>
                ...
                {correlationMatrix.correlations.length === 0 && correlationMatrix.topCombinations.length === 0 && (
                  <div style={{ color: C.mut, fontSize: 11, textAlign: 'center', padding: 20 }}>Need more data to build correlations — use analysis on multiple candles</div>
                )}
              </>
            )}

            {!correlationMatrix && <div style={{ color: C.mut, fontSize: 11, textAlign: 'center', padding: 20 }}>Run analysis first to see correlations</div>}
          </div>
        )}
        ...
        {tab === 'predict' && (
          <div style={{ padding: 8, overflowY: 'auto', flex: 1, minHeight: 0 }}>
            <div style={{ background: `${C.red}08`, border: `1px solid ${C.red}20`, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 10 }}>
              <div style={{ color: '#FF4757', fontSize: 11, fontWeight: 700, marginBottom: 5 }}>🔮 Predictive Pattern Completion</div>
              <div style={{ fontSize: 11, color: C.dim }}>When a pattern is Completed partially, this engine predicts where remaining points will complete. You can prepare before pattern completion and enter at better prices.</div>
            </div>

            {patternPredictions.length > 0 ? (
              <div>
                <div style={{ color: '#FF4757', fontSize: 11, fontWeight: 700, marginBottom: 5 }}>🔮 Patterns Forming ({patternPredictions.length})</div>
                    ...
                    <div key={i} style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 8, borderLeft: `3px solid ${dirCol}` }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 3, marginBottom: 5 }}>
                        <div style={{ background: `${dirCol}08`, borderRadius: 'var(--radius-xs)', padding: '3px 5px', textAlign: 'center' }}>
                          <div style={{ color: C.mut, fontSize: 11 }}>Completion Zone</div>
                        ...
                        <div style={{ background: `${C.cyan}08`, borderRadius: 'var(--radius-xs)', padding: '3px 5px', textAlign: 'center' }}>
                          <div style={{ color: C.mut, fontSize: 11 }}>Upper Bound</div>
                        ...
                        <div style={{ background: `${C.purple}08`, borderRadius: 'var(--radius-xs)', padding: '3px 5px', textAlign: 'center' }}>
                          <div style={{ color: C.mut, fontSize: 11 }}>Lower Bound</div>
                      ...
                      {pred.predictedPoints.length > 0 && (
                        <div>
                          <div style={{ color: C.gold, fontSize: 11, fontWeight: 700, marginBottom: 3 }}>Expected Points:</div>
                          {pred.predictedPoints.map((pt, j) => (
                            <div key={j} style={{ display: 'flex', justifyContent: 'space-between', padding: '2px 5px', background: C.dim + '05', borderRadius: 'var(--radius-xs)', marginBottom: 1 }}>
                              <span style={{ fontSize: 11, color: dirCol, fontWeight: 600 }}>Point {pt.label}</span>
                      ...
                      <div style={{ marginTop: 4, fontSize: 11, color: C.mut }}>
                        ⏱ Estimate: ~{pred.estimatedCandlesToCompletion} candle | Prediction Confidence: {Math.round(pred.confidence * 100)}%
                      </div>
                    </div>
                  );
                })}
              ...
              <div style={{ color: C.mut, fontSize: 11, textAlign: 'center', padding: 20 }}>
                {patterns.length > 0 ? 'No patterns currently forming — try different timeframes' : 'Run analysis first to see pattern predictions'}
              </div>
            )}
          </div>
        )}
        ...
        {tab === 'intelligence' && (
          <div style={{ padding: 8, overflowY: 'auto', flex: 1, minHeight: 0 }}>
            <div style={{ background: `${C.purple}08`, border: `1px solid ${C.purple}20`, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 10 }}>
              <div style={{ color: C.purple, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>🔬 Adaptive Intelligence</div>
              <div style={{ fontSize: 11, color: C.dim }}>The system learns from each engine's performance and adjusts its weights automatically. The more you trade, the more accurate predictions become.</div>
            </div>
            ...
            {adaptiveIntelligence ? (
              <div>
                ...
                {adaptiveIntelligence.sources.length > 0 ? adaptiveIntelligence.sources.map((src, i) => (
                  <div key={i} style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '6px 8px', marginBottom: 4, borderLeft: `3px solid ${src.isHot ? C.green : src.emaWinRate < 0.4 ? C.red : C.dim}` }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 3 }}>
                      <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                        <span style={{ fontSize: 11, color: src.isHot ? C.green : C.text, fontWeight: 700 }}>{src.source}</span>
                        {src.isHot && <span style={{ fontSize: 11, color: C.green, fontWeight: 800 }}>🔥</span>}
                      </div>
                      <div style={{ display: 'flex', gap: 6 }}>
                        <span style={{ fontSize: 11, color: src.emaWinRate > 0.5 ? C.green : C.red, fontWeight: 700 }}>Win: {Math.round(src.emaWinRate * 100)}%</span>
                        <span style={{ fontSize: 11, color: C.cyan, fontWeight: 600 }}>Weight: {src.adaptiveWeight.toFixed(2)}</span>
                      </div>
                    </div>
                    {/* Weight bar */}
                  ...
                  <div style={{ color: C.mut, fontSize: 11, textAlign: 'center', padding: 15 }}>No performance data yet — keep analyzing to accumulate data</div>
                )}
                ...
                {adaptiveIntelligence.regimeRecommendation && (
                  <div style={{ background: `${C.gold}08`, borderRadius: 'var(--radius-sm)', padding: '6px 8px', marginTop: 8, border: `1px solid ${C.gold}20` }}>
                    <div style={{ color: C.gold, fontSize: 11, fontWeight: 700, marginBottom: 3 }}>🎯 Recommendation by Market Regime</div>
              ...
              <div style={{ color: C.mut, fontSize: 11, textAlign: 'center', padding: 20 }}>Run analysis first to enable adaptive intelligence</div>
            )}
          </div>
        )}
        ...
        {tab === 'scenario' && (
          <div style={{ padding: 8, overflowY: 'auto', flex: 1, minHeight: 0 }}>
            <div style={{ background: `${C.blue}08`, border: `1px solid ${C.blue}20`, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 10 }}>
              <div style={{ color: C.blue, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>🎲 Scenario Engine — What If?</div>
              <div style={{ fontSize: 11, color: C.dim }}>Doesn't predict just one direction — calculates multiple potential scenarios with probabilities, price targets, and invalidation levels.</div>
            </div>
            ...
            {scenarioResult ? (
              <div>
                ...
                <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '6px 8px', marginBottom: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <span style={{ fontSize: 11, color: C.mut }}>Tilt: </span>
                    <span style={{ fontSize: 11, color: scenarioResult.tiltDirection === 'bullish' ? C.green : scenarioResult.tiltDirection === 'bearish' ? C.red : C.yellow, fontWeight: 800 }}>
                      {scenarioResult.tiltDirection === 'bullish' ? '▲ Bullish' : scenarioResult.tiltDirection === 'bearish' ? '▼ Bearish' : '◆ Neutral'}
                    </span>
                  </div>
                  <div style={{ display: 'flex', gap: 8 }}>
                    <span style={{ fontSize: 11, color: C.dim }}>Strength: {Math.round(scenarioResult.tiltStrength * 100)}%</span>
                ...
                <div style={{ background: `${C.gold}08`, borderRadius: 'var(--radius-sm)', padding: '4px 6px', marginBottom: 8, textAlign: 'center' }}>
                  <span style={{ fontSize: 11, color: C.mut }}>Critical Level: </span>
                  <span style={{ fontSize: 11, color: C.gold, fontWeight: 700 }}>{scenarioResult.keyLevel}</span>
                  <span style={{ fontSize: 11, color: C.mut }}> ({scenarioResult.keyLevelType === 'support' ? t('asp_supportShort') : scenarioResult.keyLevelType === 'resistance' ? t('asp_resistanceShort') : 'Pivotal'})</span>
                </div>

                {/* Scenarios */}
                <div style={{ color: C.cyan, fontSize: 11, fontWeight: 700, marginBottom: 4 }}>Potential Scenarios</div>
                  ...
                  return (
                    <div key={i} style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '6px 8px', marginBottom: 5, borderLeft: `3px solid ${dirCol}`, border: isDominant ? `1px solid ${dirCol}40` : undefined }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 3 }}>
                        ...
                        <div style={{ background: `${C.cyan}08`, borderRadius: 'var(--radius-xs)', padding: '3px 5px', textAlign: 'center' }}>
                          <div style={{ color: C.mut, fontSize: 11 }}>R:R</div>
                          <div style={{ color: C.cyan, fontSize: 11, fontWeight: 700, fontFamily: "var(--font-mono)" }}>1:{sc.riskRewardRatio}</div>
                        </div>
                      </div>
                      {/* Confirming / Contradicting signals */}
                      {sc.keySignals.length > 0 && (
                        <div style={{ marginTop: 3, fontSize: 11, color: C.green }}>✓ {sc.keySignals.join(' • ')}</div>
                      )}
                      {sc.contradictingSignals.length > 0 && (
                        <div style={{ fontSize: 11, color: C.red }}>✗ {sc.contradictingSignals.join(' • ')}</div>
                      )}
                    </div>
                  );
                })}
                ...
                {scenarioResult.warnings.length > 0 && (
                  <div style={{ marginTop: 8 }}>
                    {scenarioResult.warnings.map((w, i) => (
                      <div key={i} style={{ background: `${C.yellow}08`, borderRadius: 'var(--radius-sm)', padding: '4px 6px', marginBottom: 2, fontSize: 11, color: C.yellow }}>⚠️ {w}</div>
                    ))}
                  </div>
                )}
              </div>
            ) : (
              <div style={{ color: C.mut, fontSize: 11, textAlign: 'center', padding: 20 }}>Run analysis first to see scenarios</div>
            )}
          </div>
        )}
        ...
        {tab === 'spring' && (
          <div style={{ padding: 8, overflowY: 'auto', flex: 1, minHeight: 0 }}>
            <div style={{ background: `${C.green}08`, border: `1px solid ${C.green}20`, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 10 }}>
              <div style={{ color: C.green, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>🌀 Spring & Trap Detection</div>
              <div style={{ fontSize: 11, color: C.dim }}>Detects false breakout moments where traders get trapped before price reverses strongly. These are the highest-probability setups in trading.</div>
            </div>
            ...
            {springResult ? (
              <div>
                ...
                <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '6px 8px', marginBottom: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <span style={{ fontSize: 11, color: C.mut }}>Strength Signal: </span>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 3, marginBottom: 8 }}>
                  <div style={{ background: C.card, borderRadius: 'var(--radius-xs)', padding: '3px 4px', textAlign: 'center' }}>
                    <div style={{ color: C.mut, fontSize: 11 }}>Spring</div>
                    <div style={{ color: C.green, fontSize: 11, fontWeight: 700 }}>{springResult.counts.spring}</div>
                  </div>
                  <div style={{ background: C.card, borderRadius: 'var(--radius-xs)', padding: '3px 4px', textAlign: 'center' }}>
                    <div style={{ color: C.mut, fontSize: 11 }}>Push</div>
                ...
                {springResult.bestSetup && (
                  <div style={{ background: `${C.gold}08`, border: `1px solid ${C.gold}30`, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 8 }}>
                    ...
                    {springResult.bestSetup.confirmations.length > 0 && (
                      <div style={{ marginTop: 4, fontSize: 11, color: C.green }}>✓ {springResult.bestSetup.confirmations.join(' • ')}</div>
                    )}
                  </div>
                )}
                ...
                {springResult.springs.length > 1 && (
                  <div>
                    <div style={{ color: C.cyan, fontSize: 11, fontWeight: 700, marginBottom: 4 }}>All Springs ({springResult.springs.length})</div>
                        ...
                        <div key={i} style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '5px 7px', marginBottom: 3, borderLeft: `2px solid ${dirCol}`, opacity: sp.isActionable ? 1 : 0.5 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <div style={{ display: 'flex', gap: 6 }}>
                              ...
                              {sp.isActionable && <span style={{ fontSize: 11, color: C.green, fontWeight: 700 }}>● Actionable</span>}
                            </div>
                          </div>
                          <div style={{ fontSize: 11, color: C.mut, marginTop: 2 }}>Age: {sp.ageCandles} candle | Penetration: {sp.penetrationDepth.toFixed(2)} | Level: {sp.springLevel.toFixed(2)}</div>
                        </div>
                      );
                    })}
                  </div>
                ...
                {springResult.springs.length === 0 && (
                  <div style={{ color: C.mut, fontSize: 11, textAlign: 'center', padding: 20 }}>No springs or traps detected currently — try different assets or timeframes</div>
                )}
              </div>
            ) : (
              <div style={{ color: C.mut, fontSize: 11, textAlign: 'center', padding: 20 }}>Run analysis first to detect springs</div>
            )}
          </div>
        )}
        ...
        {tab === 'journal' && (
          <div style={{ padding: 8, overflowY: 'auto', flex: 1, minHeight: 0 }}>
            <div style={{ background: `${C.gold}08`, border: `1px solid ${C.gold}20`, borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 10 }}>
              <div style={{ color: C.gold, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>📋 Trade Journal</div>
              <div style={{ fontSize: 11, color: C.dim }}>Records every trade proposal automatically with its results — your proof of system performance</div>
            </div>
            ...
            {journalStats && journalStats.closedTrades > 0 && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 4, marginBottom: 8 }}>
                <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '5px 4px', textAlign: 'center' }}>
                  <div style={{ fontSize: 11, color: C.mut }}>Success Rate</div>
                ...
                <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '5px 4px', textAlign: 'center' }}>
                  <div style={{ fontSize: 11, color: C.mut }}>Medium R</div>
                  <div style={{ fontSize: 15, fontWeight: 700, color: C.cyan }}>{journalStats.avgRMultiple}</div>
                </div>
              </div>
            )}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 4, marginBottom: 8 }}>
                <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px' }}>
                  ...
                  <div style={{ display: 'flex', gap: 4, fontSize: 11 }}>
                    <span style={{ color: C.text }}>{journalStats.closedTrades} closed</span>
                    <span style={{ color: C.dim }}>|</span>
                    <span style={{ color: C.green }}>{journalStats.wins} Win</span>
                    <span style={{ color: C.red }}>{journalStats.losses} Loss</span>
                  </div>
                </div>
                <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px' }}>
                  <div style={{ fontSize: 11, color: C.mut }}>{t('asp2_trendPerformance')}</div>
                  <div style={{ display: 'flex', gap: 6, fontSize: 11 }}>
                    <span style={{ color: C.green }}>Buy {Math.round(journalStats.byDirection.bullish.winRate * 100)}%</span>
                    <span style={{ color: C.red }}>Sell {Math.round(journalStats.byDirection.bearish.winRate * 100)}%</span>
                  </div>
                </div>
                <div style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '4px 6px' }}>
                  <div style={{ fontSize: 11, color: C.mut }}>{t('asp2_maxDrawdown')}</div>
                  <div style={{ fontSize: 11, color: C.red }}>{journalStats.maxDrawdown} | Sharpe: {journalStats.sharpeEstimate}</div>
                </div>
              </div>
            )}
            ...
            {journalStats && journalStats.boostTradesCount > 0 && (
              <div style={{ background: `${C.cyan}06`, border: `1px solid ${C.cyan}15`, borderRadius: 'var(--radius-sm)', padding: '5px 7px', marginBottom: 8 }}>
                <div style={{ fontSize: 11, color: C.cyan, fontWeight: 700 }}>Revolutionary Engines Impact</div>
                <div style={{ display: 'flex', gap: 10, fontSize: 11, marginTop: 3 }}>
                  <span style={{ color: C.green }}>With Boost: {Math.round(journalStats.boostTradesWinRate * 100)}%</span>
                  <span style={{ color: C.dim }}>Without: {Math.round(journalStats.noBoostTradesWinRate * 100)}%</span>
                  <span style={{ color: journalStats.boostLift >= 1 ? C.green : C.red }}>Improvement: {journalStats.boostLift > 0 ? `${Math.round((journalStats.boostLift - 1) * 100)}%` : '—'}</span>
                </div>
              </div>
            )}
            ...
            <div style={{ display: 'flex', gap: 4, marginBottom: 8 }}>
              ...
              }} style={{ flex: 1, background: `${C.gold}15`, border: `1px solid ${C.gold}30`, borderRadius: 'var(--radius-sm)', padding: '5px', color: C.gold, fontSize: 11, cursor: 'pointer', fontWeight: 600 }}>
                Export JSON
              </button>
              <button onClick={() => {
                ...
                a.click(); URL.revokeObjectURL(url);
              }} style={{ flex: 1, background: `${C.green}15`, border: `1px solid ${C.green}30`, borderRadius: 'var(--radius-sm)', padding: '5px', color: C.green, fontSize: 11, cursor: 'pointer', fontWeight: 600 }}>
                PDF Report (HTML)
              </button>
              <button onClick={() => {
                if (confirm('Are you sure you want to clear the journal? This cannot be undone..')) {
                  clearJournal();
                  setJournalEntries([]);
            ...
            {journalStats && journalStats.weeklyBreakdown.length > 0 && (
              <div style={{ marginBottom: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 2, fontSize: 11, color: C.mut, marginTop: 1 }}>
                  {journalStats.weeklyBreakdown.map((w, i) => (
            ...
            {journalEntries.length === 0 ? (
              <div style={{ color: C.mut, fontSize: 11, textAlign: 'center', padding: 20 }}>
                No trades recorded yet — they will appear here automatically when trades are proposed
              </div>
            ) : (
              journalEntries.slice(0, 30).map((entry) => {
                ...
                const statusLabel = entry.status === 'hit_tp1' ? 'TP1 ✓' : entry.status === 'hit_tp2' ? 'TP2 ✓' : entry.status === 'hit_tp3' ? 'TP3 ✓' : entry.status === 'hit_sl' ? 'SL ✗' : entry.status === 'trail_sl' ? 'Trail' : entry.status === 'pending' ? 'Pending' : entry.status;
                return (
                  <div key={entry.id} style={{ background: C.card, borderRadius: 'var(--radius-sm)', padding: '5px 7px', marginBottom: 3, borderLeft: `2px solid ${statusColor}` }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                        <span style={{ fontSize: 11, color: entry.direction === 'bullish' ? C.green : C.red, fontWeight: 700 }}>
                          {entry.direction === 'bullish' ? '▲' : '▼'}
                        </span>
                        <span style={{ fontSize: 11, color: C.text, fontWeight: 600 }}>{entry.entryPrice.toFixed(2)}</span>
                        <span style={{ fontSize: 11, color: statusColor, fontWeight: 700 }}>{statusLabel}</span>
                      </div>
                      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                        <span style={{ fontSize: 11, color: C.dim }}>{entry.date}</span>
                        <span style={{ fontSize: 11, color: isWin ? C.green : isLoss ? C.red : C.dim, fontWeight: 700, fontFamily: "var(--font-mono)" }}>
                          {entry.realizedPnL !== 0 ? `${entry.realizedPnL > 0 ? '+' : ''}${entry.realizedPnL.toFixed(2)}` : '—'}
                        </span>
                      </div>
                    </div>
                    <div style={{ fontSize: 11, color: C.mut, marginTop: 2 }}>
                      R:R 1:{entry.rrRatio} | Confidence {Math.round(entry.confidence * 100)}% | {entry.agreeingSignals.map(s => s.source).join(' + ')}
            ...
            {journalEntries.length > 30 && (
              <div style={{ textAlign: 'center', fontSize: 11, color: C.mut, padding: '5px 0' }}>
                Showing last 30 of {journalEntries.length} — export JSON for full data
              </div>
            )}
          </div>
        )}
