  ...
  return (
    <div className="m-page" style={{ direction: 'rtl' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
        ...
        <span style={{ fontSize: 20, fontWeight: 900, color: '#FFF', fontFamily: 'var(--cairo)' }}>المراكز</span>
      </div>
      {openPositions.length === 0 ? (
        <div className="m-card" style={{ textAlign: 'center', padding: 40 }}>
          ...
          <div style={{ fontSize: 14, fontWeight: 800, color: '#8B92A8', fontFamily: 'var(--cairo)' }}>لا توجد مراكز مفتوحة</div>
        </div>
      ) : (
        openPositions.map(p => (
          <div key={p.id} className="m-card" onClick={() => router.push('/mobile/chart?symbol=' + p.symbol)} style={{ cursor: 'pointer' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 800, color: p.side === 'long' ? '#00FFA3' : '#FF4757', fontFamily: 'var(--cairo)' }}>{p.side === 'long' ? 'شراء' : 'بيع'}</span>
            </div>
            <div style={{ fontSize: 11, color: '#8B92A8', fontFamily: 'var(--mono)' }}>الكمية: {p.qty} | الدخول: ${typeof p.entryPrice === 'number' ? p.entryPrice.toFixed(2) : '—'}</div>
          </div>
        ))
      )}
    </div>
