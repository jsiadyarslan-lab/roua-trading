...
import { X, Target, ShieldAlert, Loader2, CheckCircle, AlertCircle, Minus, Plus, MousePointer2, Clock, Zap, Timer, BarChart3, ChevronDown, Pencil, CandlestickChart, LineChart, BarChart, Eye, Layers, ArrowLeft } from 'lucide-react'
...
const CHART_TYPES: { key: ChartType; label: string; icon: any }[] = [
  ...
  { key: 'line', label: 'خط', icon: LineChart },
  { key: 'area', label: 'منطقة', icon: Layers },
  { key: 'bar', label: 'أعمدة', icon: BarChart },
]
...
function ChartContent() {
  ...
  const handleChartTypeChange = useCallback((type: ChartType) => {
    setChartType(type)
    chartActionsRef.current?.setChartType(type)
  ...
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      ...
      if (ctPanelRef.current && !ctPanelRef.current.contains(e.target as Node)) setShowChartTypePanel(false)
    }
  ...
  const ToolBtn = ({ children, onClick, active, title }: { children: React.ReactNode; onClick: () => void; active?: boolean; title: string }) => (
    <button
      onClick={onClick}
      title={title}
      style={{ width: 36, height: 30, borderRadius: 6, background: active ? 'rgba(0,212,255,0.12)' : 'transparent', border: active ? '1px solid rgba(0,212,255,0.3)' : '1px solid transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: active ? '#00D4FF' : 'rgba(255,255,255,0.5)', touchAction: 'manipulation' }}
    >
      {children}
    </button>
  )
  const Separator = () => <div style={{ width: 1, height: 14, background: 'rgba(255,255,255,0.08)', margin: '0 1px', flexShrink: 0 }} />
  ...
  return (
    <div className="m-page--chart">
      ...
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, direction: 'ltr' }}>
        ...
        {/* Pair + Price + OHLC Info overlay */}
        <div style={{ position: 'absolute', top: 36, left: 8, right: 8, display: 'flex', flexDirection: 'column', gap: 4, pointerEvents: 'none', zIndex: 5, direction: 'ltr' }}>
          {/* Row 1: Pair selector + Live price */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div ref={dropdownRef} style={{ position: 'relative', pointerEvents: 'auto' }}>
              <button onClick={() => { setShowPairDropdown(!showPairDropdown); setShowTimeframePanel(false); setShowChartTypePanel(false) }} style={{ display: 'flex', alignItems: 'center', gap: 3, padding: '3px 8px', borderRadius: 6, background: 'rgba(0,0,0,0.6)', border: '1px solid rgba(0,212,255,0.15)', cursor: 'pointer', backdropFilter: 'blur(12px)' }}>
              ...
              {showPairDropdown && (
                <div style={{ position: 'absolute', top: '100%', left: 0, zIndex: 60, minWidth: 140, maxHeight: 200, overflowY: 'auto', background: 'rgba(15,17,23,0.98)', backdropFilter: 'blur(20px)', border: '1px solid rgba(0,212,255,0.15)', borderRadius: 8, padding: 4, boxShadow: '0 8px 32px rgba(0,0,0,0.7)' }}>
                  {PAIRS.map(pair => {
                    const pQuote = quotes[pair]
                    const pPrice = pQuote ? Number(pQuote.price) : null
                    const pChange = pQuote?.changePercent ?? 0
                    const pUp = pChange >= 0
                    return (
                      <button key={pair} onClick={() => { setSelectedSymbol(pair); setShowPairDropdown(false) }} style={{ width: '100%', padding: '7px 8px', borderRadius: 4, background: selectedSymbol === pair ? 'rgba(0,212,255,0.12)' : 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        {pPrice !== null && <span style={{ fontSize: 9, fontWeight: 700, color: pUp ? '#00FFA3' : '#FF453A', fontFamily: "'JetBrains Mono', monospace" }}>{pUp ? '+' : ''}{pChange.toFixed(2)}%</span>}
                      </button>
                    )
                  })}
                </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '3px 8px', borderRadius: 6, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.06)' }}>
              <span style={{ fontSize: 14, fontWeight: 900, color: priceColor, fontFamily: "'JetBrains Mono', monospace" }}>{fmtPrice(livePrice)}</span>
      ...
      {/* Enhanced Toolbar */}
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 34, zIndex: 10, paddingLeft: 2, paddingRight: 2, display: 'flex', alignItems: 'center', gap: 0, background: 'rgba(11,14,20,0.92)', backdropFilter: 'blur(16px)', direction: 'ltr', borderBottom: '0.5px solid rgba(255,255,255,0.04)' }}>
        <ToolBtn onClick={() => openExecution('buy')} title="شراء"><Zap size={13} color="#00FFA3" /></ToolBtn>
        <ToolBtn onClick={openPendingOrder} title="أوامر معلقة"><Timer size={13} /></ToolBtn>
        <Separator />
        <ToolBtn onClick={() => chartActionsRef.current?.toggleIndicators()} title="المؤشرات"><BarChart3 size={13} /></ToolBtn>
        <ToolBtn onClick={() => chartActionsRef.current?.toggleDrawings()} title="أدوات الرسم"><Pencil size={13} /></ToolBtn>
        <ToolBtn onClick={() => chartActionsRef.current?.setTool('cursor')} title="المؤشر" active={chartActionsRef.current?.activeTool === 'cursor'}><MousePointer2 size={13} /></ToolBtn>
        <Separator />
        {/* Chart Type Selector */}
        <div ref={ctPanelRef} style={{ position: 'relative' }}>
          <ToolBtn onClick={() => { setShowChartTypePanel(!showChartTypePanel); setShowTimeframePanel(false); setShowPairDropdown(false) }} title="نوع الشارت" active={showChartTypePanel}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              {chartType === 'candle' ? <CandlestickChart size={12} /> : chartType === 'line' ? <LineChart size={12} /> : chartType === 'area' ? <Layers size={12} /> : <BarChart size={12} />}
              <ChevronDown size={7} color="#00D4FF" />
            </div>
          </ToolBtn>
          {showChartTypePanel && (
            <div style={{ position: 'absolute', top: '100%', left: 0, zIndex: 60, minWidth: 120, background: 'rgba(15,17,23,0.98)', backdropFilter: 'blur(20px)', border: '1px solid rgba(0,212,255,0.15)', borderRadius: 8, padding: 4, boxShadow: '0 8px 32px rgba(0,0,0,0.7)' }}>
              {CHART_TYPES.map(ct => {
                const Icon = ct.icon
                return (
                  <button key={ct.key} onClick={() => handleChartTypeChange(ct.key)} style={{ width: '100%', padding: '6px 8px', borderRadius: 4, background: chartType === ct.key ? 'rgba(0,212,255,0.12)' : 'transparent', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, textAlign: 'left' }}>
                    <Icon size={12} color={chartType === ct.key ? '#00D4FF' : '#8B92A8'} />
                    <span style={{ fontSize: 10, fontWeight: chartType === ct.key ? 800 : 600, color: chartType === ct.key ? '#00D4FF' : '#8B92A8', fontFamily: "'Cairo', sans-serif" }}>{ct.label}</span>
                  </button>
                )
              })}
            </div>
        ...
        <div ref={tfPanelRef} style={{ position: 'relative' }}>
          <ToolBtn onClick={() => { setShowTimeframePanel(!showTimeframePanel); setShowChartTypePanel(false); setShowPairDropdown(false) }} title="الإطار الزمني">
            <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}><Clock size={11} /><span style={{ fontSize: 8, fontWeight: 800, color: '#00D4FF', fontFamily: "'JetBrains Mono', monospace" }}>{tfLabel}</span><ChevronDown size={7} color="#00D4FF" /></div>
          </ToolBtn>
          {showTimeframePanel && (
        ...
        <div style={{ flex: 1 }} />
        {/* Fullscreen toggle */}
        <ToolBtn onClick={() => setChartFullscreen(!chartFullscreen)} title="ملء الشاشة" active={chartFullscreen}>
          <Eye size={13} />
        </ToolBtn>
      </div>

      {/* Quick Trade Bar at Bottom */}
      {showQuickTrade && !showOrderSheet && (
        <div style={{ position: 'absolute', bottom: 8, left: 8, right: 8, zIndex: 8, display: 'flex', gap: 6, direction: 'ltr' }}>
          <button onClick={() => openExecution('buy')} style={{ flex: 1, height: 36, borderRadius: 10, border: 'none', background: 'linear-gradient(135deg, #32D74B, #28A745)', color: '#FFF', fontSize: 12, fontWeight: 800, fontFamily: "'Cairo', sans-serif", cursor: 'pointer', boxShadow: '0 4px 16px rgba(50,215,75,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, touchAction: 'manipulation', backdropFilter: 'blur(10px)' }}>
            <Zap size={14} /> شراء
          </button>
          <button onClick={() => openExecution('sell')} style={{ flex: 1, height: 36, borderRadius: 10, border: 'none', background: 'linear-gradient(135deg, #FF453A, #DC2626)', color: '#FFF', fontSize: 12, fontWeight: 800, fontFamily: "'Cairo', sans-serif", cursor: 'pointer', boxShadow: '0 4px 16px rgba(255,69,58,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, touchAction: 'manipulation', backdropFilter: 'blur(10px)' }}>
            <Zap size={14} /> بيع
          </button>
          <button onClick={openPendingOrder} style={{ width: 36, height: 36, borderRadius: 10, border: '0.5px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(10px)', color: '#8B92A8', fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', touchAction: 'manipulation' }}>
            <Timer size={14} />
          </button>
        </div>
      )}
