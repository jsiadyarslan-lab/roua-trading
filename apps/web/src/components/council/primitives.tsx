...
export function DirectionBadge({ direction, size = "md", label }: { direction: Direction; size?: "sm" | "md" | "lg"; label?: string }) {
  ...
  const sizing = size === "lg" ? { padding: "8px 14px", fontSize: 15, fontWeight: 700 } : size === "sm" ? { padding: "3px 8px", fontSize: 11, fontWeight: 600 } : { padding: "5px 10px", fontSize: 13, fontWeight: 700 };
  return (
...
export function StatusPill({ status, label }: { status: ReviewStatus; label: string }) {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "3px 9px", borderRadius: 'var(--radius-2xl)', fontSize: 11, fontWeight: 600, textTransform: "uppercase", background: hexToRgba(color, 0.1), color, border: `1px solid ${hexToRgba(color, 0.3)}` }}>
  ...
  return (
    <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 16, marginBottom: 18, flexWrap: "wrap" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 14, minWidth: 0 }}>
        {index ? (
          <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, fontWeight: 600, letterSpacing: "0.18em", color: COLORS.council, padding: "4px 8px", borderRadius: 'var(--radius-sm)', background: hexToRgba(COLORS.council, 0.1), border: `1px solid ${hexToRgba(COLORS.council, 0.25)}` }}>{index}</div>
        ) : null}
...
export function StatTile({ label, value, sub, accent, icon }: {
  ...
  return (
    <GlassCard padding={16} style={{ height: "100%" }}>
      ...
      <div style={{ fontSize: 27, fontWeight: 600, letterSpacing: "-0.025em", color: accent ?? COLORS.textPrimary, lineHeight: 1, fontFamily: "var(--font-mono)" }}>{value}</div>
      {sub ? <div style={{ marginTop: 6, fontSize: 13, color: COLORS.textMuted }}>{sub}</div> : null}
    </GlassCard>
  );
}
