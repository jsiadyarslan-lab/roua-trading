...
// ─── AI Council i18n map ────────────────────────────────────────────
// Used by runDirectCouncilConsensus to build masterStrategy & conflictExplanation
// in the user's language instead of hardcoded English.
const AI_COUNCIL_I18N: Record<string, {
  recBuy: string; recSell: string; recHold: string
  recStrong: string; recClear: string; recProbable: string
  masterStrategyFormat: string
  conflictFewModels: string; conflictBroadCoverage: string; conflictAligned: string
}> = {
  'en': { recBuy:'Buy',recSell:'Sell',recHold:'Hold',recStrong:'Strong',recClear:'Clear',recProbable:'Probable',masterStrategyFormat:'AI Council Consensus ({roles} roles from {models} models): {recLabel} {recStrength} with {confidence}% confidence.',conflictFewModels:'Some models did not respond — available models ({models}) reached consensus.',conflictBroadCoverage:'Roles are relatively aligned with broad coverage.',conflictAligned:'Core roles are relatively aligned.' },
  'ar': { recBuy:'شراء',recSell:'بيع',recHold:'انتظار',recStrong:'قوي',recClear:'واضح',recProbable:'محتمل',masterStrategyFormat:'إجماع مجلس الذكاء الاصطناعي ({roles} أدوار من {models} نماذج): {recLabel} {recStrength} بنسبة ثقة {confidence}%.',conflictFewModels:'لم تستجب بعض النماذج — النماذج المتاحة ({models}) وصلت إلى الإجماع.',conflictBroadCoverage:'الأدوار متوافقة نسبياً مع تغطية واسعة.',conflictAligned:'الأدوار الأساسية متوافقة نسبياً.' },
  'bn': { recBuy:'কিনুন',recSell:'বিক্রয়',recHold:'ধরে রাখুন',recStrong:'শক্তিশালী',recClear:'পরিষ্কার',recProbable:'সম্ভাব্য',masterStrategyFormat:'এআই কাউন্সিল কনসেনসাস ({models} মডেল থেকে {roles} ভূমিকা): {recLabel} {recStrength} {confidence}% আত্মবিশ্বাসের সাথে।',conflictFewModels:'কিছু মডেল সাড়া দেয়নি — উপলব্ধ মডেল ({models}) ঐক্যমত্যে পৌঁছেছে।',conflictBroadCoverage:'ভূমিকা তুলনামূলকভাবে বিস্তৃত কভারেজের সাথে সারিবদ্ধ।',conflictAligned:'মূল ভূমিকা তুলনামূলকভাবে সারিবদ্ধ।' },
  'cs': { recBuy:'Nakoupit',recSell:'Prodat',recHold:'Držet',recStrong:'Silný',recClear:'Jasný',recProbable:'Pravděpodobný',masterStrategyFormat:'Konsenzus AI Council (role {roles} z modelů {models}): {recLabel} {recStrength} se spolehlivostí {confidence} %.',conflictFewModels:'Některé modely nereagovaly — dostupné modely ({models}) dosáhly konsenzu.',conflictBroadCoverage:'Role jsou relativně sladěny s širokým pokrytím.',conflictAligned:'Základní role jsou relativně sladěny.' },
  'da': { recBuy:'Købe',recSell:'Sælge',recHold:'Holde',recStrong:'Stærk',recClear:'Klar',recProbable:'Sandsynlig',masterStrategyFormat:'AI Council Konsensus ({roles}-roller fra {models}-modeller): {recLabel} {recStrength} med {confidence}% tillid.',conflictFewModels:'Nogle modeller reagerede ikke - tilgængelige modeller ({models}) nåede konsensus.',conflictBroadCoverage:'Roller er relativt afstemt med bred dækning.',conflictAligned:'Kernerollerne er relativt afstemte.' },
  'de': { recBuy:'Kaufen',recSell:'Verkaufen',recHold:'Halten',recStrong:'Stark',recClear:'Klar',recProbable:'Wahrscheinlich',masterStrategyFormat:'Konsens des AI Council ({roles}-Rollen von {models}-Modellen): {recLabel} {recStrength} mit {confidence}%-Konfidenz.',conflictFewModels:'Einige Modelle antworteten nicht – verfügbare Modelle ({models}) erzielten einen Konsens.',conflictBroadCoverage:'Die Rollen sind relativ auf eine breite Abdeckung abgestimmt.',conflictAligned:'Die Kernrollen sind relativ aufeinander abgestimmt.' },
  'es': { recBuy:'Comprar',recSell:'Vender',recHold:'Mantener',recStrong:'Fuerte',recClear:'Claro',recProbable:'Probable',masterStrategyFormat:'Consenso del Consejo de IA (roles {roles} de modelos {models}): {recLabel} {recStrength} con {confidence}% de confianza.',conflictFewModels:'Algunos modelos no respondieron: los modelos disponibles ({models}) llegaron a un consenso.',conflictBroadCoverage:'Los roles están relativamente alineados con una cobertura amplia.',conflictAligned:'Los roles centrales están relativamente alineados.' },
  'fa': { recBuy:'خرید',recSell:'فروش',recHold:'نگه دارید',recStrong:'قوی',recClear:'واضح',recProbable:'محتمل',masterStrategyFormat:'اجماع شورای هوش مصنوعی (نقش‌های {roles} از مدل‌های {models}): {recLabel} {recStrength} با اطمینان {confidence}٪.',conflictFewModels:'برخی از مدل‌ها پاسخ ندادند - مدل‌های موجود ({models}) به توافق رسیدند.',conflictBroadCoverage:'نقش‌ها با پوشش گسترده نسبتاً همسو هستند.',conflictAligned:'نقش‌های اصلی نسبتاً همسو هستند.' },
  'fi': { recBuy:'Ostaa',recSell:'Myydä',recHold:'Pidä',recStrong:'Vahva',recClear:'Selkeä',recProbable:'Todennäköistä',masterStrategyFormat:'AI Council Konsensus ({roles}-roolit {models}-malleista): {recLabel} {recStrength} {confidence}%-varmuudella.',conflictFewModels:'Jotkut mallit eivät vastanneet – saatavilla olevat mallit ({models}) pääsivät yksimielisyyteen.',conflictBroadCoverage:'Roolit ovat suhteellisesti linjassa laajan kattavuuden kanssa.',conflictAligned:'Ydinroolit ovat suhteellisesti linjassa.' },
  'fil': { recBuy:'Bumili',recSell:'Ibenta',recHold:'Hawakan',recStrong:'Malakas',recClear:'Maaliwalas',recProbable:'Malamang',masterStrategyFormat:'Konsenso ng AI Council ({roles} roles mula sa {models} models): {recLabel} {recStrength} na may {confidence}% kumpiyansa.',conflictFewModels:'Hindi tumugon ang ilang modelo — naabot ng mga available na modelo ({models}) ang pinagkasunduan.',conflictBroadCoverage:'Ang mga tungkulin ay medyo naaayon sa malawak na saklay.',conflictAligned:'Ang mga pangunahing tungkulin ay medyo nakahanay.' },
  'fr': { recBuy:'Acheter',recSell:'Vendre',recHold:'Conserver',recStrong:'Fort',recClear:'Clair',recProbable:'Probable',masterStrategyFormat:"Consensus du Conseil de l'IA (rôles {roles} à partir des modèles {models}) : {recLabel} {recStrength} avec confiance {confidence}%.",conflictFewModels:"Certains modèles n'ont pas répondu — les modèles disponibles ({models}) ont atteint un consensus.",conflictBroadCoverage:'Les rôles sont relativement alignés avec une large couverture.',conflictAligned:'Les rôles principaux sont relativement alignés.' },
  'he': { recBuy:'לקנות',recSell:'מכירה',recHold:'להחזיק',recStrong:'חזק',recClear:'ברור',recProbable:'מסתבר',masterStrategyFormat:'קונצנזוס מועצת AI (תפקידי {roles} מדגמי {models}): {recLabel} {recStrength} עם ביטחון {confidence}%.',conflictFewModels:'חלק מהדגמים לא הגיבו - הדגמים הזמינים ({models}) הגיעו לקונצנזוס.',conflictBroadCoverage:'תפקידים מתאימים יחסית לכיסוי רחב.',conflictAligned:'תפקידי הליבה מיושרים יחסית.' },
  'hi': { recBuy:'खरीदना',recSell:'बेचना',recHold:'पकड़ना',recStrong:'मज़बूत',recClear:'स्पष्ट',recProbable:'संभावित',masterStrategyFormat:'एआई परिषद की सहमति ({models} मॉडल से {roles} भूमिकाएँ): {confidence}% आत्मविश्वास के साथ {recLabel} {recStrength}।',conflictFewModels:'कुछ मॉडलों ने प्रतिक्रिया नहीं दी - उपलब्ध मॉडल ({models}) आम सहमति पर पहुंच गए।',conflictBroadCoverage:'भूमिकाएँ व्यापक कवरेज के साथ अपेक्षाकृत संरेखित हैं।',conflictAligned:'मुख्य भूमिकाएँ अपेक्षाकृत संरेखित हैं।' },
  'hu': { recBuy:'Vétel',recSell:'Elad',recHold:'Tart',recStrong:'Erős',recClear:'Világos',recProbable:'Valószínű',masterStrategyFormat:'AI Council Konszenzus ({roles} szerepek {models} modellekből): {recLabel} {recStrength} {confidence}%-os biztonsággal.',conflictFewModels:'Egyes modellek nem válaszoltak – a rendelkezésre álló modellek ({models}) konszenzusra jutottak.',conflictBroadCoverage:'A szerepkörök viszonylag jól illeszkednek a széles lefedettséghez.',conflictAligned:'Az alapszerepek viszonylag össze vannak hangolva.' },
  'id': { recBuy:'Membeli',recSell:'Menjual',recHold:'Memegang',recStrong:'Kuat',recClear:'Jernih',recProbable:'Mungkin',masterStrategyFormat:'Konsensus Dewan AI (peran {roles} dari model {models}): {recLabel} {recStrength} dengan keyakinan {confidence}%.',conflictFewModels:'Beberapa model tidak merespons — model yang tersedia ({models}) mencapai konsensus.',conflictBroadCoverage:'Perannya relatif selaras dengan cakupan yang luas.',conflictAligned:'Peran inti relatif selaras.' },
  'it': { recBuy:'Acquistare',recSell:'Vendere',recHold:'Mantenere',recStrong:'Forte',recClear:'Chiaro',recProbable:'Probabile',masterStrategyFormat:"Consensus dell'AI Council (ruoli {roles} dai modelli {models}): {recLabel} {recStrength} con confidenza {confidence}%.",conflictFewModels:'Alcuni modelli non hanno risposto: i modelli disponibili ({models}) hanno raggiunto il consenso.',conflictBroadCoverage:"I ruoli sono relativamente allineati con un'ampia copertura.",conflictAligned:'I ruoli principali sono relativamente allineati.' },
  'ja': { recBuy:'買い',recSell:'売り',recHold:'ホールド',recStrong:'強い',recClear:'明確',recProbable:'可能性',masterStrategyFormat:'AI Council のコンセンサス ({models} モデルからの {roles} の役割): {recLabel} {recStrength}、{confidence}% の信頼度。',conflictFewModels:'一部のモデルは応答しませんでした - 利用可能なモデル ({models}) はコンセンサスに達しました。',conflictBroadCoverage:'役割は比較的広範囲に対応しています。',conflictAligned:'中核的な役割は比較的一致しています。' },
  'ko': { recBuy:'매수',recSell:'매도',recHold:'관망',recStrong:'강한',recClear:'분명한',recProbable:'유망한',masterStrategyFormat:'AI 위원회 합의({models} 모델의 {roles} 역할): {confidence}% 신뢰도를 갖춘 {recLabel} {recStrength}.',conflictFewModels:'일부 모델은 응답하지 않았습니다. 사용 가능한 모델({models})이 합의에 도달했습니다.',conflictBroadCoverage:'역할은 상대적으로 광범위한 범위에 맞춰져 있습니다.',conflictAligned:'핵심 역할은 상대적으로 일치합니다.' },
  'ms': { recBuy:'Beli',recSell:'Jual',recHold:'Tahan',recStrong:'Kuat',recClear:'Jelas',recProbable:'Berkemungkinan',masterStrategyFormat:'Konsensus Majlis AI (peranan {roles} daripada model {models}): {recLabel} {recStrength} dengan keyakinan {confidence}%.',conflictFewModels:'Sesetengah model tidak bertindak balas — model tersedia ({models}) mencapai kata sepakat.',conflictBroadCoverage:'Peranan secara relatifnya sejajar dengan liputan luas.',conflictAligned:'Peranan teras agak sejajar.' },
  'nl': { recBuy:'Kopen',recSell:'Verkopen',recHold:'Uitstel',recStrong:'Sterk',recClear:'Duidelijk',recProbable:'Waarschijnlijk',masterStrategyFormat:'AI Council Consensus ({roles}-rollen van {models}-modellen): {recLabel} {recStrength} met {confidence}% vertrouwen.',conflictFewModels:'Sommige modellen reageerden niet: de beschikbare modellen ({models}) bereikten consensus.',conflictBroadCoverage:'De rollen zijn relatief afgestemd op een brede dekking.',conflictAligned:'De kernrollen liggen relatief op één lijn.' },
  'no': { recBuy:'Kjøpe',recSell:'Selge',recHold:'Holde',recStrong:'Sterk',recClear:'Klar',recProbable:'Sannsynlig',masterStrategyFormat:'AI Council Konsensus ({roles}-roller fra {models}-modeller): {recLabel} {recStrength} med {confidence}% tillit.',conflictFewModels:'Noen modeller svarte ikke - tilgjengelige modeller ({models}) nådde konsensus.',conflictBroadCoverage:'Roller er relativt på linje med bred dekning.',conflictAligned:'Kjernerollene er relativt samkjørte.' },
  'pl': { recBuy:'Kupić',recSell:'Sprzedać',recHold:'Trzymać',recStrong:'Mocny',recClear:'Jasne',recProbable:'Prawdopodobny',masterStrategyFormat:'Konsensus Rady AI (role {roles} z modeli {models}): {recLabel} {recStrength} z pewnością {confidence}%.',conflictFewModels:'Niektóre modele nie odpowiedziały – dostępne modele ({models}) osiągnęły konsensus.',conflictBroadCoverage:'Role są stosunkowo wyrównane i mają szeroki zasięg.',conflictAligned:'Podstawowe role są stosunkowo wyrównane.' },
  'pt': { recBuy:'Comprar',recSell:'Vender',recHold:'Segurar',recStrong:'Forte',recClear:'Claro',recProbable:'Provável',masterStrategyFormat:'Consenso do AI Council (funções {roles} dos modelos {models}): {recLabel} {recStrength} com {confidence}% de confiança.',conflictFewModels:'Alguns modelos não responderam — os modelos disponíveis ({models}) chegaram a um consenso.',conflictBroadCoverage:'As funções estão relativamente alinhadas com uma ampla cobertura.',conflictAligned:'As funções principais estão relativamente alinhadas.' },
  'ro': { recBuy:'Cumpăra',recSell:'Vinde',recHold:'Ține',recStrong:'Puternic',recClear:'Clar',recProbable:'Probabil',masterStrategyFormat:'Consensul Consiliului AI (roluri {roles} de la modelele {models}): {recLabel} {recStrength} cu {confidence}% încredere.',conflictFewModels:'Unele modele nu au răspuns - modelele disponibile ({models}) au ajuns la un consens.',conflictBroadCoverage:'Rolurile sunt relativ aliniate cu o acoperire largă.',conflictAligned:'Rolurile de bază sunt relativ aliniate.' },
  'ru': { recBuy:'Покупать',recSell:'Продавать',recHold:'Держать',recStrong:'Сильный',recClear:'Ясный',recProbable:'Вероятный',masterStrategyFormat:'Консенсус Совета ИИ (роли {roles} из моделей {models}): {recLabel} {recStrength} с достоверностью {confidence}%.',conflictFewModels:'Некоторые модели не ответили — доступные модели ({models}) достигли консенсуса.',conflictBroadCoverage:'Роли относительно совпадают с широким охватом.',conflictAligned:'Основные роли относительно выровнены.' },
  'sv': { recBuy:'Köpa',recSell:'Sälja',recHold:'Hålla',recStrong:'Stark',recClear:'Tydlig',recProbable:'Sannolik',masterStrategyFormat:'AI Council Konsensus ({roles}-roller från {models}-modeller): {recLabel} {recStrength} med {confidence}% förtroende.',conflictFewModels:'Vissa modeller svarade inte - tillgängliga modeller ({models}) nådde konsensus.',conflictBroadCoverage:'Roller är relativt anpassade med bred täckning.',conflictAligned:'Kärnrollerna är relativt samordnade.' },
  'th': { recBuy:'ซื้อ',recSell:'ขาย',recHold:'ถือ',recStrong:'แข็งแกร่ง',recClear:'ชัดเจน',recProbable:'น่าจะเป็น',masterStrategyFormat:'ฉันทามติสภา AI (บทบาท {roles} จากรุ่น {models}): {recLabel} {recStrength} พร้อมความมั่นใจ {confidence}%',conflictFewModels:'บางรุ่นไม่ตอบสนอง — รุ่นที่มี ({models}) บรรลุข้อตกลงร่วมกัน',conflictBroadCoverage:'บทบาทค่อนข้างสอดคล้องกับความครอบคลุมในวงกว้าง',conflictAligned:'บทบาทหลักค่อนข้างสอดคล้องกัน' },
  'tr': { recBuy:'Al',recSell:'Sat',recHold:'Bekle',recStrong:'Güçlü',recClear:'Net',recProbable:'Muhtemel',masterStrategyFormat:'AI Konseyi Konsensüsü ({models} modellerinden {roles} rolleri): {recLabel} {recStrength} %{confidence} güvenle.',conflictFewModels:'Bazı modeller yanıt vermedi; mevcut modeller ({models}) fikir birliğine vardı.',conflictBroadCoverage:'Roller nispeten geniş kapsama alanıyla uyumludur.',conflictAligned:'Temel roller nispeten uyumludur.' },
  'uk': { recBuy:'Купити',recSell:'Продати',recHold:'Тримати',recStrong:'Сильний',recClear:'Ясно',recProbable:'Ймовірно',masterStrategyFormat:'Консенсус Ради ШІ (ролі {roles} з моделей {models}): {recLabel} {recStrength} із впевненістю {confidence}%.',conflictFewModels:'Деякі моделі не відповіли — доступні моделі ({models}) досягли консенсусу.',conflictBroadCoverage:'Ролі відносно узгоджені з широким охопленням.',conflictAligned:'Основні ролі відносно вирівняні.' },
  'ur': { recBuy:'خریدیں',recSell:'بیچنا',recHold:'پکڑو',recStrong:'مضبوط',recClear:'صاف',recProbable:'ممکنہ',masterStrategyFormat:'AI کونسل کا اتفاق ({models} ماڈلز سے {roles} کردار): {recLabel} {recStrength} {confidence}% اعتماد کے ساتھ۔',conflictFewModels:'کچھ ماڈلز نے جواب نہیں دیا — دستیاب ماڈلز ({models}) اتفاق رائے پر پہنچ گئے۔',conflictBroadCoverage:'کردار نسبتاً وسیع کوریج کے ساتھ منسلک ہوتے ہیں۔',conflictAligned:'بنیادی کردار نسبتاً منسلک ہیں۔' },
  'vi': { recBuy:'Mua',recSell:'Bán',recHold:'Giữ',recStrong:'Mạnh',recClear:'Rõ ràng',recProbable:'Có thể',masterStrategyFormat:'Sự đồng thuận của Hội đồng AI (vai trò {roles} từ các mô hình {models}): {recLabel} {recStrength} với độ tin cậy {confidence}%.',conflictFewModels:'Một số mô hình không phản hồi - các mô hình có sẵn ({models}) đã đạt được sự đồng thuận.',conflictBroadCoverage:'Các vai trò tương đối phù hợp với phạm vi bao phủ rộng rãi.',conflictAligned:'Các vai trò cốt lõi tương đối phù hợp.' },
  'zh': { recBuy:'买入',recSell:'卖出',recHold:'持有',recStrong:'强势',recClear:'明确',recProbable:'可能',masterStrategyFormat:'AI 委员会共识（{models} 模型中的 {roles} 角色）：{recLabel} {recStrength}，置信度为 {confidence}%。',conflictFewModels:'一些型号没有回应——可用型号（{models}）达成共识。',conflictBroadCoverage:'角色与广泛的覆盖面相对一致。',conflictAligned:'核心角色相对一致。' },
}

function getAICouncilI18n(locale: string) {
  return AI_COUNCIL_I18N[locale] || AI_COUNCIL_I18N['en']
}
 ...
 */
export async function runDirectCouncilConsensus(symbol: string, language: string = 'ar'): Promise<{
  success: boolean
  ...
  const modelCalls: Array<{
    ...
    {
      modelName: 'Groq',
      callFn: () => callGroq(`${marketDataPrefix}${P.sentiment}`, language as "ar" | "en"),
    ...
    {
      modelName: 'Gemini',
      callFn: () => callGemini(`${marketDataPrefix}${P.technical}`, language as "ar" | "en"),
    ...
    {
      modelName: 'GLM-4',
      callFn: () => callGLM(`${marketDataPrefix}${P.macro}`, language as "ar" | "en"),
    ...
    },
    {
      ...
      callFn: () => callHuggingFace(`${marketDataPrefix}${P.patterns}`, language as "ar" | "en"),
    ...
    {
      modelName: 'Ollama',
      callFn: () => callOllama(`${marketDataPrefix}${P.execution}`, language as "ar" | "en"),
    ...
    {
      modelName: 'Bedrock',
      callFn: () => callBedrock(`${marketDataPrefix}${P.risk}`, language as "ar" | "en"),
    ...
    },
    {
      ...
      callFn: () => callOpenRouter(`${marketDataPrefix}${P.divergence}`, language as "ar" | "en"),
    ...
    {
      modelName: 'DeepSeek',
      callFn: () => callDeepSeek(`${marketDataPrefix}${P.scenario}`, language as "ar" | "en"),
  ...
  // i18n: Build masterStrategy and conflictExplanation in the user's language
  const i18n = getAICouncilI18n(language)
  const recLabel = recommendation === 'BUY' ? i18n.recBuy : recommendation === 'SELL' ? i18n.recSell : i18n.recHold
  const recStrength = consensusScore >= 80 ? i18n.recStrong : consensusScore >= 60 ? i18n.recClear : i18n.recProbable
  const masterStrategy = i18n.masterStrategyFormat
    .replace('{roles}', String(filledRoles))
    .replace('{models}', String(modelsRespondedCount))
    .replace('{recLabel}', recLabel)
    .replace('{recStrength}', recStrength)
    .replace('{confidence}', String(consensusScore))

  const conflictExplanation = filledRoles < 4
    ? i18n.conflictFewModels.replace('{models}', uniqueModels.join(', '))
    : filledRoles >= 5
      ? i18n.conflictBroadCoverage
      : i18n.conflictAligned
