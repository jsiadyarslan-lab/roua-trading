...
export function resolvePrediction(
  ...
  if (perf) {
    ...
    if (Math.abs(perf.adaptiveWeight - oldWeight) > 0.15) {
      const direction = perf.adaptiveWeight > oldWeight ? 'رفع' : 'خفض';
      insights.push({
        id: `insight_${Date.now()}_${Math.random().toString(36).slice(2, 5)}`,
        type: 'weight_change',
        source: record.source,
        messageAr: `${direction} وزن ${sourceNameAr(record.source)} إلى ${perf.adaptiveWeight.toFixed(2)} (معدل الفوز: ${Math.round(perf.emaWinRate * 100)}%)`,
    ...
    if (winStreak >= 7) {
      ...
      insights.push({
        ...
        type: 'streak',
        source: record.source,
        messageAr: `🔥 ${sourceNameAr(record.source)} في سلسلة انتصارات (${winStreak}/${recentOutcomes.length})! الوزن التكيفي زاد تلقائياً`,
        importance: 'info',
        timestamp: Date.now(),
      });
      ...
      insights.push({
        ...
        type: 'streak',
        source: record.source,
        messageAr: `⚠️ ${sourceNameAr(record.source)} في سلسلة خسائر (${lossStreak}/${recentOutcomes.length})! الوزن التكيفي خُفّض تلقائياً`,
        importance: 'critical',
        timestamp: Date.now(),
      });
    } else {
...
export function getAdaptiveIntelligenceState(regime: MarketRegime): AdaptiveIntelligenceState {
  ...
  if (bestSources.length > 0) {
    messageAr = `في نظام السوق "${regimeNameAr(regime)}"، أفضل المصادر: ${bestSources.map(s => sourceNameAr(s)).join('، ')}`;
  } else {
    messageAr = `لا توجد بيانات كافية لنظام السوق "${regimeNameAr(regime)}" — استمر في التداول لجمع البيانات`;
  }
  if (avoidSources.length > 0) {
    messageAr += ` | تجنّب: ${avoidSources.map(s => sourceNameAr(s)).join('، ')}`;
  }
