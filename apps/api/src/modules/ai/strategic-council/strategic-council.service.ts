...
export class StrategicCouncilService {
  ...
  constructor(
    ...
    this._checkAIHealth();
    // FIX: RE-ENABLED _triggerStartupSession(). Previously removed to prevent
    // phantom trades, but without it, briefs don't exist for 15 minutes after
    // boot — meaning the executor has NOTHING to execute. The phantom trade
    // problem was caused by the executor auto-enabling users on startup, NOT
    // by generating briefs. Briefs are harmless data — they only become trades
    // when an ENABLED user exists. Now that the executor auto-restores from DB
    // (only users who explicitly clicked "تشغيل"), phantom trades are prevented
    // while the executor has briefs ready to process immediately after boot.
