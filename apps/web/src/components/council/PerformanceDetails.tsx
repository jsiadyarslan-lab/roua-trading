  ...
  if (!stats) {
    return (
      <GlassCard padding={36} style={{ textAlign: 'center' }}>
        <div style={{ fontSize: 15, fontWeight: 600, color: COLORS.textPrimary, marginBottom: 4 }}>No analysis</div>
        <div style={{ fontSize: 13, color: COLORS.textMuted }}>Performance data will appear after briefs are issued</div>
      </GlassCard>
    );
  }
  ...
  const Bar = ({ label, value, total, color }: { label: string; value: number; total: number; color: string }) => {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 600, color: COLORS.textPrimary, fontFamily: "var(--font-mono)", minWidth: 40, textAlign: 'right' }}>{value}</span>
      ...
      <GlassCard padding={20}>
        ...
        <div style={{ marginTop: 12, padding: '10px 12px', borderRadius: 'var(--radius-md)', background: hexToRgba(COLORS.council, 0.08), border: `1px solid ${hexToRgba(COLORS.council, 0.2)}` }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <span style={{ fontSize: 19, fontWeight: 600, color: COLORS.council, fontFamily: "var(--font-mono)" }}>{stats.avgConf.toFixed(0)}%</span>
          </div>
        </div>
      </GlassCard>
