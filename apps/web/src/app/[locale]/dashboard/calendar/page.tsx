    ...
    <div style={{ padding: '24px 28px', direction: 'inherit', fontFamily: "var(--font-ar)", minHeight: '100vh', background: '#0B0E14' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
              ...
              <span style={{
                fontSize: 11, padding: '2px 10px', borderRadius: 'var(--radius-2xl)',
          ...
          <button onClick={fetchCalendar} disabled={loading} style={{
            ...
            borderRadius: 'var(--radius-lg)', color: '#0A84FF', fontSize: 13, fontWeight: 800, cursor: 'pointer',
          }}>
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 16, flexWrap: 'wrap' }}>
          {[
            ...
            <div key={s.label} style={{
              ...
              <span style={{ fontSize: 19, fontWeight: 900, color: s.color, fontFamily: "var(--font-mono)" }}>{s.value}</span>
              <span style={{ fontSize: 11, color: '#9CA3B5', fontWeight: 700 }}>{s.label}</span>
            </div>
          ))}
          {lastFetch && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 12px', background: '#151A22', border: `1px solid ${'#2A313C'}`, borderRadius: 'var(--radius-lg)' }}>
              ...
              <span style={{ fontSize: 11, color: '#9CA3B5' }}>{t('lastUpdate')} {lastFetch.toLocaleTimeString('ar-SA')}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <span style={{ fontSize: 11, color: '#9CA3B5', fontWeight: 700 }}>{t('currency')}</span>
          {CURRENCIES.map(c => (
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ fontSize: 11, color: '#9CA3B5', fontWeight: 700 }}>{t('impact')}</span>
      ...
      {loading ? (
        ...
        <div key={dateLabelKey} style={{ marginBottom: 24 }}>
          ...
          <div style={{
            ...
            <span style={{ fontSize: 15, fontWeight: 900, color: '#F0F2F5' }}>{t(dateLabelKey)} — {dayEvents[0]?.dateLabel || ''}</span>
            <span style={{
              fontSize: 11, padding: '1px 8px', borderRadius: 'var(--radius-2xl)',
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px' }}>
                    ...
                    <div style={{ minWidth: 48, textAlign: 'center' }}>
                      <div style={{ fontSize: 13, fontWeight: 900, color: '#F0F2F5', fontFamily: "var(--font-mono)" }}>{event.time}</div>
                    </div>
                    ...
                    {/* Currency badge */}
                    <span style={{
                      fontSize: 11, padding: '3px 10px', borderRadius: 'var(--radius-2xl)', fontFamily: "var(--font-mono)", fontWeight: 900,
                      background: `${style.color}15`, color: style.color,
                      border: `1px solid ${style.color}30`, minWidth: 38, textAlign: 'center',
                    }}>{event.currency}</span>

                    {/* Event name */}
                    <span style={{ flex: 1, fontSize: 13, fontWeight: 700, color: '#F0F2F5' }}>{event.event}</span>

                    {/* Forecast vs Previous */}
                    <div style={{ display: 'flex', gap: 12, textAlign: 'center', fontSize: 11, color: '#9CA3B5' }}>
                      {event.forecast !== '—' && (
                        <div>
                          <div style={{ fontWeight: 900, color: '#00D4FF', fontFamily: "var(--font-mono)" }}>{event.forecast}</div>
                          <div style={{ fontSize: 11 }}>{t('forecast')}</div>
                        </div>
                      )}
                      {event.previous !== '—' && (
                        <div>
                          <div style={{ fontWeight: 700, fontFamily: "var(--font-mono)" }}>{event.previous}</div>
                          <div style={{ fontSize: 11 }}>{t('previous')}</div>
                        </div>
                      )}
                    </div>
                  ...
                  {isOpen && (
                    <div style={{
                      ...
                      <div style={{ display: 'flex', gap: 10, marginBottom: 12, alignItems: 'flex-start' }}>
                        ...
                        <div>
                          <div style={{ fontSize: 11, fontWeight: 800, color: '#B388FF', marginBottom: 4 }}>{t('aiAnalysis')}</div>
                          <div style={{ fontSize: 13, color: '#F0F2F5', lineHeight: 1.7 }}>{event.ai.summary}</div>
                        </div>
                        {/* Strength bar */}
                        <div style={{ marginRight: 'auto', textAlign: 'center', minWidth: 60 }}>
                          <div style={{ fontSize: 17, fontWeight: 900, color: style.color, fontFamily: "var(--font-mono)" }}>{event.ai.strength}%</div>
                          <div style={{ fontSize: 11, color: '#9CA3B5' }}>{t('impactStrength')}</div>
                      ...
                      {event.affectedPairs?.length > 0 && (
                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
                          <span style={{ fontSize: 11, color: '#9CA3B5', fontWeight: 700 }}>{t('affectedPairs')}</span>
