...
export function verifyBayesianEngine(): EngineVerificationResult {
  ...
  checks.push({
    ...
    nameAr: 'المخرجات تتغير مع المدخلات المختلفة',
    ...
    detailAr: `إشارات صعودية → ${bullResult.direction}, إشارات هبوطية → ${bearResult.direction}`,
  });
  ...
  checks.push({
    ...
    nameAr: 'مبرهنة بايز مطبقة فعلاً',
    ...
    detailAr: `الاحتمال المسبق: ${bullResult.prior.bullish.toFixed(3)}, الاحتمال اللاحق: ${bullResult.posteriorBullish.toFixed(3)}, الفرق: ${priorDiff.toFixed(3)}`,
  ...
  checks.push({
    ...
    nameAr: 'الاحتمالات اللاحقة تتطبع للمجموع 1',
    passed: Math.abs(posteriorSum - 1.0) < 0.01,
    detail: `P(bullish|signals) + P(bearish|signals) = ${posteriorSum.toFixed(4)}`,
    detailAr: `P(صعودي|إشارات) + P(هبوطي|إشارات) = ${posteriorSum.toFixed(4)}`,
  ...
  checks.push({
    ...
    detailAr: `${bullResult.likelihoods.length} احتمالات لـ ${bullSignals.length} إشارات`,
...
export function verifyPatternStateMachine(): EngineVerificationResult {
  ...
  checks.push({
    ...
    nameAr: 'تنبيهات مولدة للأنماط عالية الجودة',
    passed: hasAlerts,
    detail: `Alerts: ${alertResult.alerts.length}, Completed: ${alertResult.summary.completed}`,
    detailAr: `تنبيهات: ${alertResult.alerts.length}, مكتمل: ${alertResult.summary.completed}`,
  });
  ...
  checks.push({
    ...
    nameAr: 'الأنماط الفاشلة تُكشف عند كسر وقف الخسارة',
    ...
    detailAr: `عدد الفاشلة: ${failResult2.summary.failed}`,
  ...
  checks.push({
    ...
    nameAr: 'آلة الحالة لديها أكثر من انتقالين',
    ...
    detailAr: `6 حالات: تتشكل → تتحقق → نشط → مُطلق → مؤكد/فاشل`,
  ...
  return {
    ...
    comparisonWithMarket: {
      ...
      featureAr: 'آلة حالة دورة حياة النمط',
      rouaHas: true,
      tradingViewHas: false,
      multiChartsHas: false,
      advantage: 'roua',
      detailAr: 'روا يتعقب دورة حياة كاملة للنمط — المنافسون يكتشفون النمط فقط بدون تتبع الحالة',
    },
    timestamp: Date.now(),
  };
}
...
export function verifyPatternPerformance(): EngineVerificationResult {
  ...
  checks.push({
    ...
    nameAr: 'record() يخزن بيانات التداول فعلاً',
    passed: summary.totalTrades >= 3,
    detail: `Total trades recorded: ${summary.totalTrades}`,
    detailAr: `إجمالي التداولات المسجلة: ${summary.totalTrades}`,
  });
  ...
  if (summary.statsByType.has('Gartley')) {
    ...
    checks.push({
      ...
      detailAr: `معدل فوز Gartley: ${gartleyStats.successRate.toFixed(3)}, المتوقع: ${expectedWinRate.toFixed(3)}`,
    ...
    checks.push({
      ...
      detailAr: 'إحصائيات Gartley غير موجودة',
    });
  }
  ...
  checks.push({
    ...
    detailAr: `تقدير شارب: ${summary.sharpeEstimate.toFixed(3)}`,
  ...
  checks.push({
    ...
    detailAr: `إجمالي التداولات: ${summary.totalTrades}, معدل النجاح: ${summary.overallSuccessRate.toFixed(3)}`,
  });
  ...
  return {
    ...
    comparisonWithMarket: {
      ...
      featureAr: 'تتبع أداء الأنماط',
      rouaHas: true,
      tradingViewHas: false,
      multiChartsHas: false,
      advantage: 'roua',
      detailAr: 'روا يتعقب أداء كل نمط تلقائياً مع نسبة شارب — المنافسون لا يفعلون ذلك',
    },
    timestamp: Date.now(),
  };
}
...
export function verifyConfidenceHeatmap(): EngineVerificationResult {
  ...
  checks.push({
    ...
    nameAr: 'الاتجاه مبني على تداخل الإشارات وليس لون الشمعة فقط',
  ...
  checks.push({
    ...
    nameAr: 'إشارات أكثر → ثقة أعلى في المتوسط',
    ...
    detailAr: `بدون إشارات: ${noSignalResult.avgConfidence.toFixed(3)}, مع إشارات: ${withSignalResult.avgConfidence.toFixed(3)}`,
  ...
  return {
    ...
    comparisonWithMarket: {
      ...
      featureAr: 'خريطة الثقة الحرارية',
      rouaHas: true,
      tradingViewHas: false,
      multiChartsHas: false,
      advantage: 'roua',
      detailAr: 'روا يعرض خريطة ثقة حرارية مبنية على تداخل الإشارات — لا يوجد منافس يفعل ذلك',
    },
    timestamp: Date.now(),
  };
}
...
export function verifyElliottSMCFusion(): EngineVerificationResult {
  ...
  checks.push({
    ...
    detailAr: `طبقة1: ${layers.directionalAgreement}%, طبقة2: ${layers.spatialConfluence}%, طبقة3: ${layers.volumeConfirmation}%, طبقة4: ${layers.patternStrength}%`,
  });
  ...
  checks.push({
    ...
    nameAr: 'نتيجة التقارب تتغير مع البيانات',
    ...
    detailAr: `صعودي: ${result.confluenceScore}%, هبوطي: ${bearishResult.confluenceScore}%`,
  });
  ...
  checks.push({
    ...
    nameAr: 'أوزان ديناميكية بناءً على القرب',
    passed: hasProximity,
    ...
    detailAr: `عوامل مع قرب: ${result.confluenceBreakdown.filter(f => f.proximity !== undefined).length}/${result.confluenceBreakdown.length}`,
  ...
  checks.push({
    ...
    nameAr: 'الاتجاه يعكس محاذاة الإشارات',
    ...
    detailAr: `سيناريو صعودي: ${result.direction}, سيناريو هبوطي: ${bearishResult.direction}`,
  });

  // ── Check 5: Breakdown factors exposed ──
  checks.push({
    name: 'Breakdown factors visible for transparency',
    nameAr: 'عوامل التقسيم مرئية للشفافية',
    ...
    detailAr: `${result.confluenceBreakdown.length} عوامل: ${result.confluenceBreakdown.map(f => f.factorAr).join(', ')}`,
  });

  // ── Check 6: Arabic interpretation provided ──
  checks.push({
    name: 'Arabic interpretation generated',
    nameAr: 'تفسير عربي مولّد',
  ...
  return {
    ...
    comparisonWithMarket: {
      ...
      featureAr: 'دمج متعدد المناهج',
      rouaHas: true,
      tradingViewHas: false,
      multiChartsHas: false,
      advantage: 'roua',
      detailAr: 'روا يدمج إليوت + SMC + ويكوف في 4 طبقات — المنافسون يعرضون كل طريقة منفصلة فقط',
    },
    timestamp: Date.now(),
  };
}
