  ...
  return (
    <button key={symbol} type="button"
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:1, alignItems:'flex-start' }}>
        <span style={{ fontSize:10, fontWeight:800, color: tickColor, fontFamily:"'JetBrains Mono',monospace", lineHeight:1, letterSpacing:'0.05em', transition: 'color 0.15s' }}>
          {symbol.split('/')[0]}
        </span>
        <span style={{ fontSize:11, fontWeight:800, color: tickColor, fontFamily:"'JetBrains Mono',monospace", lineHeight:1.2, textShadow: tickDir !== 'flat' ? `0 0 6px ${tickDir === 'up' ? 'rgba(0,255,136,0.6)' : 'rgba(255,51,85,0.6)'}` : 'none', transition: 'color 0.15s' }}>
          {price}
        </span>
        <span style={{ fontSize:9, fontWeight:700, color: isUp ? '#00FF88' : '#FF3355', fontFamily:"'JetBrains Mono',monospace", lineHeight:1, padding: '1px 5px', borderRadius: 4, background: pctBg }}>
