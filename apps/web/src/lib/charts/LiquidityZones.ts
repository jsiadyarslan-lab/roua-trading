...
const LABELS_AR: Record<LiquidityType, string> = {
 'equal_highs': ' ',
 'equal_lows': ' ',
 'previous_high': ' high previous',
 'previous_low': ' low previous',
 'liquidity_void': ' ',
};
...
export function detectLiquidityZones(candles: CandleData[]): LiquidityResult {
 ...
 return { zones: [], activeZones: 0, sweptZones: 0, dominantSweepDirection: 'neutral', interpretationAr: 'data in analysis ' };
 }
 ...
 const dirAr = dominantSweepDirection === 'bullish' ? 'bullish' : 'bearish';
 interpretationAr = ` ${dirAr} — ${sweptZones} who , ${activeZones} who active`;
 } else if (activeZones > 3) {
 interpretationAr = `${activeZones} who active — possibility `;
 } else {
 interpretationAr = `${activeZones} who active, ${sweptZones} `;
 }
