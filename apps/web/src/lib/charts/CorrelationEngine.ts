...
function computePairCorrelation(
  ...
  return {
    ...
    descriptionAr: `${sourceALabel} + ${sourceBLabel}: نسبة نجاح ${Math.round(combinedWinRate * 100)}% عند ظهورهما معاً (تحسن ${lift > 1 ? '+' : ''}${Math.round((lift - 1) * 100)}% مقارنة بـ ${sourceALabel} وحده)`,
  };
}
...
function findTopCombinations(
  ...
  return [...comboMap.entries()]
    ...
    .map(([key, v]) => ({
      ...
      descriptionAr: `${key.split('+').map(mapSourceToArabic).join(' + ')}: نسبة نجاح ${Math.round((v.wins / v.total) * 100)}% (${v.total} مرة)`,
    }))
