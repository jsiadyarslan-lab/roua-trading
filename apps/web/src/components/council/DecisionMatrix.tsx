    ...
    <div style={{ overflowX: 'auto', padding: '4px 0' }}>
      <div style={{ minWidth: 580 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: `120px repeat(${TIMEFRAMES.length}, 1fr)`, gap: 4, marginBottom: 4 }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: COLORS.textMuted, padding: '8px 10px' }}>
            Pair / TF
          </div>
          {TIMEFRAMES.map(tf => (
            <div key={tf} style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: COLORS.textMuted, padding: '8px 4px', textAlign: 'center', fontFamily: "var(--font-mono)" }}>
              {tf}
            </div>
          ))}
        </div>
        ...
        {pairs.map((pair, pi) => (
          <div key={pair} style={{ display: 'grid', gridTemplateColumns: `120px repeat(${TIMEFRAMES.length}, 1fr)`, gap: 4, marginBottom: 4 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: COLORS.textPrimary, padding: '8px 10px', fontFamily: "var(--font-mono)", display: 'flex', alignItems: 'center' }}>
              {pair}
            </div>
            {TIMEFRAMES.map(tf => {
              const brief = matrix.get(`${pair}:${tf}`);
              if (!brief) {
                return <div key={tf} style={{ padding: '8px 4px', textAlign: 'center', fontSize: 11, color: COLORS.textDim, borderRadius: 'var(--radius-sm)', background: 'rgba(255,255,255,0.015)' }}>·</div>;
              }
