...
const HARMONIC_PATTERNS: PatternSpec[] = [
  { name: 'Gartley', nameAr: 'جارتلي', direction: undefined as any, points: 5, ratios: [0.618, 0.786, 0.618] },
  { name: 'Bat', nameAr: 'بات', direction: undefined as any, points: 5, ratios: [0.382, 0.886, 0.618] },
  { name: 'Butterfly', nameAr: 'فراشة', direction: undefined as any, points: 5, ratios: [0.786, 1.27, 0.786] },
  { name: 'Crab', nameAr: 'سلطعون', direction: undefined as any, points: 5, ratios: [0.382, 1.618, 0.618] },
];

const CHART_PATTERNS: PatternSpec[] = [
  { name: 'DoubleTop', nameAr: 'قمة مزدوجة', direction: 'bearish', points: 2, ratios: [] },
  { name: 'DoubleBottom', nameAr: 'قاع مزدوج', direction: 'bullish', points: 2, ratios: [] },
  { name: 'HeadAndShoulders', nameAr: 'رأس وكتفين', direction: 'bearish', points: 3, ratios: [] },
  { name: 'InvHeadAndShoulders', nameAr: 'رأس وكتفين معكوس', direction: 'bullish', points: 3, ratios: [] },
  { name: 'AscendingTriangle', nameAr: 'مثلث صاعد', direction: 'bullish', points: 2, ratios: [] },
  { name: 'DescendingTriangle', nameAr: 'مثلث هابط', direction: 'bearish', points: 2, ratios: [] },
];
      ...
      for (const ratio of pattern.ratios.slice(0, 1)) { // Only check first ratio (AB of XA)
        if (Math.abs(abRetrace - ratio) < 0.1) { // 10% tolerance
          ...
          predictions.push({
            ...
            patternTypeAr: `${pattern.nameAr} غير مكتمل`,
            completionPct,
            ...
            descriptionAr: `نمط ${pattern.nameAr} غير مكتمل — النقاط X, A, B تشكلت. يُتوقع اكتمال النمط عند ${Math.round(dTarget * 100) / 100}`,
      ...
      for (const ratio of pattern.ratios.slice(0, 1)) {
        if (Math.abs(abRetrace - ratio) < 0.1) {
          ...
          predictions.push({
            ...
            confidence: Math.min(0.7, 0.3 + (1 - Math.abs(abRetrace - ratio)) * 3),
            descriptionAr: `نمط ${pattern.nameAr} هابط غير مكتمل — يُتوقع اكتماله عند ${Math.round(dTarget * 100) / 100}`,
            estimatedCandlesToCompletion: Math.round((candles.length - B.index) * 0.5),
          });
        }
      }
  ...
  if (swingHighs.length >= 2) {
    ...
    if (priceDiff < 0.01) { // Within 1%
      ...
      predictions.push({
        ...
        patternTypeAr: 'قمة مزدوجة محتملة',
        completionPct: 60,
        predictedDirection: 'bearish',
        formedPoints: [
        ...
        confidence: 0.55,
        descriptionAr: 'قمتان عند مستوى متقارب — إذا كسر السعر خط العنق قد يكتمل النمط الهابط',
  ...
  if (swingLows.length >= 2) {
    ...
    if (priceDiff < 0.01) {
      ...
      predictions.push({
        ...
        confidence: 0.55,
        descriptionAr: 'قاعان عند مستوى متقارب — إذا كسر السعر خط العنق قد يكتمل النمط الصاعد',
        estimatedCandlesToCompletion: 10,
      });
    }
  }
    ...
    if (head.price > left.price && head.price > right.price) {
      ...
      if (shoulderDiff < 0.02) { // Shoulders within 2%
        ...
        predictions.push({
          ...
          patternTypeAr: 'رأس وكتفين محتمل',
          completionPct: 80,
          predictedDirection: 'bearish',
          formedPoints: [
          ...
          confidence: 0.65,
          descriptionAr: 'نمط رأس وكتفين شبه مكتمل — الكتفان متقاربان والرأس أعلى. كسر خط العنق يؤكد النمط',
