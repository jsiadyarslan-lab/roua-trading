...
interface NewsItem { id?: string; title: string; summary?: string; url?: string; source?: string; publishedAt?: string; sentiment?: string; category?: string; imageUrl?: string | null }
...
export default function NewsPage() {
  ...
  // Map next-intl locale to the news API lang parameter
  const newsLang = locale === 'fr' ? 'fr' : locale === 'tr' ? 'tr' : locale === 'es' ? 'es' : locale === 'en' ? 'en' : 'ar'

  const fetchNews = useCallback(async () => {
    try {
      const res = await fetch(`/api/news?lang=${newsLang}&limit=20`)
      if (res.ok) {
  ...
  const formatDate = (dateStr?: string) => {
    if (!dateStr) return ''
    try {
      return new Date(dateStr).toLocaleDateString(locale === 'ar' ? 'ar-SA' : locale === 'fr' ? 'fr-FR' : locale === 'tr' ? 'tr-TR' : locale === 'es' ? 'es-ES' : 'en-US', {
        month: 'short',
        day: 'numeric',
        hour: '2-digit',
        minute: '2-digit',
  ...
  return (
    <div className="m-page">
      ...
      {news.map((item, i) => (
        <div key={item.id || i} className="m-card" onClick={() => item.url && window.open(item.url, '_blank')} style={{ cursor: item.url ? 'pointer' : 'default' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {item.publishedAt && <><Clock size={10} color="rgba(255,255,255,0.3)" /><span style={{ fontSize: 9, color: 'rgba(255,255,255,0.3)', fontFamily: 'var(--cairo)' }}>{formatDate(item.publishedAt)}</span></>}
