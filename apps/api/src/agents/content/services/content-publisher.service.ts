    ...
    try {
      ...
      const article = await this.prisma.contentArticle.create({
        data: {
          ...
          // V9: fr/tr/es fields removed — columns don't exist in DB yet
  ...
  async getFeed(options: {
    ...
    const [articles, total] = await Promise.all([
      this.prisma.contentArticle.findMany({
        ...
        skip,
        take: limit,
        // V9: Select without fr/tr/es fields (columns don't exist in DB)
        select: {
          id: true, userId: true, type: true, contentType: true,
          titleAr: true, titleEn: true, contentAr: true, contentEn: true,
          summaryAr: true, summaryEn: true, excerpt: true,
          category: true, categoryAr: true, tags: true, relatedSymbols: true,
          seo: true, aiModel: true, generationSource: true, confidence: true,
          qualityScore: true, sentimentScore: true, impactLevel: true,
          riskWarnings: true, sources: true, readingTimeMinutes: true,
          wordCountAr: true, wordCountEn: true, views: true, shares: true,
          likes: true, status: true, publishedAt: true, scheduledAt: true,
