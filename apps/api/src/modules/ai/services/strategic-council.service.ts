  ...
  async getConsensusAnalysis(
    ...
    try {
      ...
      const roles: CouncilRole[] = baseLanguageForPrompts === 'en' ? [
        { id: 'tech',   name: 'Technical Analyst',    model: 'gemini',     fallbackModels: ['groq', 'ollama', 'deepseek', 'glm', 'bedrock', 'huggingface', 'openrouter'],  prompt: `${contextPrefix}${marketDataPrefix}🎯 ROLE BOUNDARY: You are the TECHNICAL ANALYST. Your ONLY job is to analyze price action, indicators (RSI, MACD, EMA, Bollinger), support/resistance levels, and chart patterns. DO NOT discuss macroeconomic factors, news sentiment, or risk management — those are handled by other council members. Stay strictly technical.\n\nAnalyze the technical chart for ${symbol} based on trend, momentum, and resistance levels.${decisionInstruction}` },
        { id: 'sent',   name: 'Sentiment Analyst',     model: 'groq',       fallbackModels: ['deepseek', 'ollama', 'gemini', 'bedrock', 'glm', 'huggingface', 'openrouter'], prompt: `${contextPrefix}${marketDataPrefix}🎯 ROLE BOUNDARY: You are the SENTIMENT ANALYST. Your ONLY job is to gauge market sentiment from news, social media tone, fear/greed indicators, and trader positioning. DO NOT discuss technical indicators, macroeconomic policy, or stop-loss levels — those are handled by other council members. Stay strictly sentiment-focused.\n\nAnalyze current market sentiment for ${symbol} from a news and momentum perspective.${decisionInstruction}` },
        { id: 'risk',   name: 'Risk Expert',     model: 'gemini',     fallbackModels: ['cerebras', 'groq', 'ollama', 'deepseek', 'glm', 'mistral', 'nvidia', 'bedrock'],        prompt: `${contextPrefix}${marketDataPrefix}🎯 ROLE BOUNDARY: You are the RISK EXPERT. Your ONLY job is to identify downside risks, calculate optimal stop-loss levels, assess position sizing, and warn of worst-case scenarios. DO NOT discuss technical indicators, news, or macro trends except as they directly affect risk. Stay strictly risk-focused.\n\nIdentify risks of entering a trade on ${symbol} now, stop-loss levels, and worst-case scenario assessment.${decisionInstruction}` },
        { id: 'macro',  name: 'Macro Expert',     model: 'gemini',     fallbackModels: ['cerebras', 'groq', 'deepseek', 'ollama', 'glm', 'bedrock', 'huggingface', 'openrouter'], prompt: `${contextPrefix}${marketDataPrefix}🎯 ROLE BOUNDARY: You are the MACRO EXPERT. Your ONLY job is to analyze macroeconomic factors: interest rates, inflation, central bank policy, geopolitical events, and their impact on ${symbol}. DO NOT discuss technical indicators, sentiment, or stop-loss levels — those are handled by other council members. Stay strictly macro-focused.\n\nAnalyze the macroeconomic situation and its impact on ${symbol}.${decisionInstruction}` },
        { id: 'pattern',name: 'Pattern Expert',     model: 'cerebras',   fallbackModels: ['ollama', 'mistral', 'groq', 'gemini', 'bedrock', 'glm', 'nvidia'],        prompt: `${contextPrefix}${marketDataPrefix}🎯 ROLE BOUNDARY: You are the PATTERN EXPERT. Your ONLY job is to identify recurring historical chart patterns (head & shoulders, triangles, flags, double tops/bottoms, Elliott waves, etc.) in the current movement of ${symbol}. DO NOT discuss macro, news, or risk — those are handled by other council members. Stay strictly pattern-focused.\n\nDo you see any recurring historical patterns in the current movement of ${symbol}?${decisionInstruction}` },
        { id: 'exec',   name: 'Execution Strategist', model: 'ollama',     fallbackModels: ['deepseek', 'bedrock', 'glm', 'gemini', 'groq', 'huggingface', 'openrouter'],        prompt: `${contextPrefix}${marketDataPrefix}🎯 ROLE BOUNDARY: You are the EXECUTION STRATEGIST. Your ONLY job is to determine optimal entry timing based on liquidity, order book depth, spread, and session timing (Asian/European/US). DO NOT discuss technical indicators, macro, or sentiment — those are handled by other council members. Stay strictly execution-focused.\n\nWhat is the best timing for entering ${symbol} based on liquidity and available models?${decisionInstruction}` },
        { id: 'diverge',name: 'Divergence Analyst',     model: 'cerebras',   fallbackModels: ['groq', 'ollama', 'bedrock', 'gemini', 'mistral', 'glm', 'nvidia'],        prompt: `${contextPrefix}${marketDataPrefix}🎯 ROLE BOUNDARY: You are the DIVERGENCE ANALYST. Your ONLY job is to look for counter-signals and divergences: price vs RSI divergence, price vs volume divergence, price vs MACD divergence, cross-exchange price divergence. Your role is to challenge the prevailing trend. DO NOT discuss macro, news, or risk — those are handled by other council members. Stay strictly divergence-focused.\n\nLook for counter-signals or divergences in the analysis of ${symbol} — is there a reason not to follow the prevailing trend?${decisionInstruction}` },
        { id: 'scenario', name: 'Scenario Analyst', model: 'mistral',  fallbackModels: ['ollama', 'bedrock', 'gemini', 'groq', 'glm', 'cerebras', 'nvidia'],        prompt: `${contextPrefix}${marketDataPrefix}🎯 ROLE BOUNDARY: You are the SCENARIO ANALYST. Your ONLY job is to construct 3 probability-weighted scenarios (bull case, base case, bear case) with specific price targets and trigger conditions. DO NOT discuss technical indicators, sentiment, or risk in isolation — synthesize them into scenarios. Stay strictly scenario-focused.\n\nAnalyze possible scenarios for ${symbol} with probability estimates for each scenario.${decisionInstruction}` },
      ] : [
        { id: 'tech',   name: 'المحلل الفني',    model: 'gemini',     fallbackModels: ['groq', 'ollama', 'deepseek', 'glm', 'bedrock', 'huggingface', 'openrouter'],  prompt: `${contextPrefix}${marketDataPrefix}🎯 حدود الدور: أنت المحلل الفني. مهمتك الوحيدة هي تحليل حركة السعر والمؤشرات (RSI, MACD, EMA, Bollinger) ومستويات الدعم/المقاومة وأنماط الشارت. ممنوع مناقشة العوامل الماكرواقتصادية أو مشاعر الأخبار أو إدارة المخاطر — هذه أدوار أخرى تتولاها. ابقَ فنياً فقط.\n\nحلل الشارت الفني لـ ${symbol} بناءً على الاتجاه والزخم والمقاومات.${decisionInstruction}` },
        { id: 'sent',   name: 'محلل المشاعر',     model: 'groq',       fallbackModels: ['deepseek', 'ollama', 'gemini', 'bedrock', 'glm', 'huggingface', 'openrouter'], prompt: `${contextPrefix}${marketDataPrefix}🎯 حدود الدور: أنت محلل المشاعر. مهمتك الوحيدة هي قياس مشاعر السوق من الأخبار ونبرة وسائل التواصل ومؤشرات الخوف/الجشع وتموضع المتداولين. ممنوع مناقشة المؤشرات الفنية أو السياسة الماكرواقتصادية أو مستويات وقف الخسارة — هذه أدوار أخرى تتولاها. ابقَ مركزاً على المشاعر فقط.\n\nحلل مشاعر السوق الحالية لـ ${symbol} من منظور الأخبار والزخم.${decisionInstruction}` },
        { id: 'risk',   name: 'خبير المخاطر',     model: 'gemini',     fallbackModels: ['cerebras', 'groq', 'ollama', 'deepseek', 'glm', 'mistral', 'nvidia', 'bedrock'],        prompt: `${contextPrefix}${marketDataPrefix}🎯 حدود الدور: أنت خبير المخاطر. مهمتك الوحيدة هي تحديد المخاطر الهبوطية وحساب مستويات وقف الخسارة المثلى وتقييم حجم المركز والتحذير من السيناريوهات الأسوأ. ممنوع مناقشة المؤشرات الفنية أو الأخبار أو الاتجاهات الماكرو إلا بمقدار تأثيرها المباشر على المخاطر. ابقَ مركزاً على المخاطر فقط.\n\nحدد مخاطر دخول صفقة على ${symbol} الآن ومستويات وقف الخسارة مع تقييم السيناريو الأسوأ.${decisionInstruction}` },
        { id: 'macro',  name: 'خبير الماكرو',     model: 'gemini',     fallbackModels: ['cerebras', 'groq', 'deepseek', 'ollama', 'glm', 'bedrock', 'huggingface', 'openrouter'], prompt: `${contextPrefix}${marketDataPrefix}🎯 حدود الدور: أنت خبير الماكرو. مهمتك الوحيدة هي تحليل العوامل الماكرواقتصادية: أسعار الفائدة، التضخم، سياسة البنوك المركزية، الأحداث الجيوسياسية، وتأثيرها على ${symbol}. ممنوع مناقشة المؤشرات الفنية أو المشاعر أو مستويات وقف الخسارة — هذه أدوار أخرى تتولاها. ابقَ مركزاً على الماكرو فقط.\n\nحلل الوضع الاقتصادي العام وتأثيره على ${symbol} مع مراعاة السياق العربي.${decisionInstruction}` },
        { id: 'pattern',name: 'خبير الأنماط',     model: 'cerebras',   fallbackModels: ['ollama', 'mistral', 'groq', 'gemini', 'bedrock', 'glm', 'nvidia'],        prompt: `${contextPrefix}${marketDataPrefix}🎯 حدود الدور: أنت خبير الأنماط. مهمتك الوحيدة هي تحديد الأنماط التاريخية المتكررة في الشارت (رأس وكتفان، مثلثات، أعلام، قمم/قيعان مزدوجة، موجات إليوت، إلخ) في حركة ${symbol} الحالية. ممنوع مناقشة الماكرو أو الأخبار أو المخاطر — هذه أدوار أخرى تتولاها. ابقَ مركزاً على الأنماط فقط.\n\nهل ترى أي أنماط تاريخية متكررة في حركة ${symbol} الحالية؟${decisionInstruction}` },
        { id: 'exec',   name: 'استراتيجي التنفيذ', model: 'ollama',     fallbackModels: ['deepseek', 'bedrock', 'glm', 'gemini', 'groq', 'huggingface', 'openrouter'],        prompt: `${contextPrefix}${marketDataPrefix}🎯 حدود الدور: أنت استراتيجي التنفيذ. مهمتك الوحيدة هي تحديد توقيت الدخول الأمثل بناءً على السيولة وعمق دفتر الأوامر والسبريد وتوقيت الجلسات (آسيوية/أوروبية/أمريكية). ممنوع مناقشة المؤشرات الفنية أو الماكرو أو المشاعر — هذه أدوار أخرى تتولاها. ابقَ مركزاً على التنفيذ فقط.\n\nما هو أفضل توقيت للدخول في ${symbol} بناءً على السيولة والنماذج المتاحة؟${decisionInstruction}` },
        { id: 'diverge',name: 'محلل التباين',     model: 'cerebras',   fallbackModels: ['groq', 'ollama', 'bedrock', 'gemini', 'mistral', 'glm', 'nvidia'],        prompt: `${contextPrefix}${marketDataPrefix}🎯 حدود الدور: أنت محلل التباين. مهمتك الوحيدة هي البحث عن الإشارات المعاكسة والتباينات: تباين السعر vs RSI، تباين السعر vs الحجم، تباين السعر vs MACD، تباين الأسعار عبر البورصات. دورك هو تحدي الاتجاه السائد. ممنوع مناقشة الماكرو أو الأخبار أو المخاطر — هذه أدوار أخرى تتولاها. ابقَ مركزاً على التباين فقط.\n\nابحث عن إشارات معاكسة أو تباينات في تحليل ${symbol} — هل هناك سبب لعدم اتباع الاتجاه السائد؟${decisionInstruction}` },
        { id: 'scenario', name: 'محلل السيناريوهات', model: 'mistral',  fallbackModels: ['ollama', 'bedrock', 'gemini', 'groq', 'glm', 'cerebras', 'nvidia'],        prompt: `${contextPrefix}${marketDataPrefix}🎯 حدود الدور: أنت محلل السيناريوهات. مهمتك الوحيدة هي بناء 3 سيناريوهات مرجحة بالاحتمالات (سيناريو صعودي، سيناريو أساسي، سيناريو هبوطي) مع مستويات أسعار محددة وشروط تحقق. ممنوع مناقشة المؤشرات الفنية أو المشاعر أو المخاطر بشكل منفرد — اجمعها في سيناريوهات. ابقَ مركزاً على السيناريوهات فقط.\n\nحلل السيناريوهات المحتملة لـ ${symbol} مع تقدير احتمالات كل سيناريو.${decisionInstruction}` },
      ];
