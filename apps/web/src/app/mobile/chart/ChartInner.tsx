...
export default function ChartInner() {
  ...
  const executeOrder = useCallback(async () => {
    ...
    try {
      ...
      if (entryPrice <= 0) { addNotification({ source: 'trade', priority: 'urgent', action: 'WARN', title: 'سعر غير صالح', body: 'لا يمكن تنفيذ الأمر بسعر صفر' }); return }
      addTrade({
  ...
  return (
    <div style={{ direction: 'rtl', display: 'flex', flexDirection: 'column', height: isFullscreen ? '100dvh' : 'auto' }}>
      {/* شريط الأدوات */}
      ...
      {!isFullscreen && (
        <div style={{ display: 'flex', gap: 6, padding: '8px 12px', background: 'rgba(11,14,20,0.95)', borderTop: '0.5px solid var(--border)', position: 'fixed', bottom: 'calc(var(--nav-h) + var(--safe-b) + 4px)', left: 0, right: 0, zIndex: 9, direction: 'rtl' }}>
          <button onClick={() => { setOrderSide('buy'); setShowOrder(true) }} style={{ flex: 1, padding: '10px 0', borderRadius: 10, border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg, #00FFA3, #00D4FF)', color: '#000', fontSize: 13, fontWeight: 900, fontFamily: 'var(--cairo)' }}>شراء</button>
          <button onClick={() => { setOrderSide('sell'); setShowOrder(true) }} style={{ flex: 1, padding: '10px 0', borderRadius: 10, border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg, #FF4757, #FF6B6B)', color: '#FFF', fontSize: 13, fontWeight: 900, fontFamily: 'var(--cairo)' }}>بيع</button>
          <button onClick={() => { setOrderType('limit'); setOrderSide('buy'); setShowOrder(true) }} style={{ flex: 1, padding: '10px 0', borderRadius: 10, cursor: 'pointer', background: 'rgba(255,255,255,0.05)', border: '0.5px solid var(--border)', color: 'var(--text2)', fontSize: 13, fontWeight: 900, fontFamily: 'var(--cairo)' }}>أمر معلق</button>
        </div>
      )}
      ...
      {/* اختيار الإطار */}
      {showTf && <Overlay title="الإطار الزمني" onClose={() => setShowTf(false)}>
      ...
      {/* نوع الشارت */}
      {showTypes && <Overlay title="نوع الشارت" onClose={() => setShowTypes(false)}>
      ...
      {showOrder && <>
        ...
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 21, direction: 'rtl', background: 'var(--card)', borderRadius: '20px 20px 0 0', padding: 16, maxHeight: '80dvh', overflowY: 'auto', borderTop: '0.5px solid rgba(0,212,255,0.2)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <span style={{ fontSize: 17, fontWeight: 800, color: '#FFF', fontFamily: 'var(--cairo)' }}>أمر جديد</span>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, padding: '8px 12px', borderRadius: 12, background: 'rgba(255,255,255,0.03)', border: '0.5px solid var(--border)' }}>
            <div><div style={{ fontSize: 14, fontWeight: 900, color: '#FFF', fontFamily: 'var(--mono)' }}>{selectedSymbol}</div><div style={{ fontSize: 10, color: 'var(--text2)', fontFamily: 'var(--cairo)' }}>السعر الحالي</div></div>
          ...
          <div style={{ display: 'flex', gap: 0, background: 'rgba(255,255,255,0.03)', borderRadius: 10, padding: 2, marginBottom: 12 }}>
            <button onClick={() => setOrderSide('buy')} style={{ flex: 1, padding: '8px 0', borderRadius: 8, border: 'none', cursor: 'pointer', background: orderSide === 'buy' ? 'rgba(0,255,163,0.1)' : 'transparent', color: orderSide === 'buy' ? '#00FFA3' : 'var(--text3)', fontSize: 12, fontWeight: 800, fontFamily: 'var(--cairo)' }}>شراء</button>
            <button onClick={() => setOrderSide('sell')} style={{ flex: 1, padding: '8px 0', borderRadius: 8, border: 'none', cursor: 'pointer', background: orderSide === 'sell' ? 'rgba(255,71,87,0.1)' : 'transparent', color: orderSide === 'sell' ? '#FF4757' : 'var(--text3)', fontSize: 12, fontWeight: 800, fontFamily: 'var(--cairo)' }}>بيع</button>
          </div>
          <div style={{ display: 'flex', gap: 6, marginBottom: 12 }}>
            {(['market', 'limit', 'stop'] as const).map(t => { const labels: Record<string, string> = { market: 'سوقي', limit: 'محدد', stop: 'وقف' }; const sel = orderType === t; return (
              <button key={t} onClick={() => setOrderType(t)} style={{ flex: 1, padding: '6px 0', borderRadius: 8, border: sel ? '0.5px solid rgba(0,212,255,0.3)' : '0.5px solid var(--border)', background: sel ? 'rgba(0,212,255,0.06)' : 'transparent', color: sel ? 'var(--accent)' : 'var(--text3)', fontSize: 11, fontWeight: 800, fontFamily: 'var(--cairo)', cursor: 'pointer' }}>{labels[t]}</button>
            ) })}
          </div>
          {orderType !== 'market' && (
            <div style={{ marginBottom: 12, padding: '8px 12px', borderRadius: 10, background: 'rgba(255,255,255,0.03)', border: '0.5px solid var(--border)' }}>
              <div style={{ fontSize: 10, color: 'var(--text2)', fontFamily: 'var(--cairo)', marginBottom: 4 }}>سعر الأمر</div>
          ...
          <div style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 10, color: 'var(--text2)', fontFamily: 'var(--cairo)', marginBottom: 6 }}>الكمية</div>
            ...
            {currentPrice > 0 && <div style={{ textAlign: 'center', fontSize: 10, color: 'var(--text2)', fontFamily: 'var(--cairo)', marginTop: 4 }}>القيمة: ${fmtPrice(orderQty * currentPrice)}</div>}
          </div>
          <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                <span style={{ fontSize: 10, color: 'var(--text2)', fontFamily: 'var(--cairo)' }}>جني الأرباح</span>
              ...
              {showTp && <input type="number" value={tpValue || ''} onChange={e => setTpValue(Number(e.target.value))} placeholder="سعر الهدف" style={{ width: '100%', padding: '6px 10px', borderRadius: 8, background: 'rgba(0,255,163,0.05)', border: '0.5px solid rgba(0,255,163,0.15)', fontSize: 12, fontWeight: 800, color: '#00FFA3', fontFamily: 'var(--mono)', outline: 'none', appearance: 'none' }} />}
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                <span style={{ fontSize: 10, color: 'var(--text2)', fontFamily: 'var(--cairo)' }}>وقف الخسارة</span>
              ...
              {showSl && <input type="number" value={slValue || ''} onChange={e => setSlValue(Number(e.target.value))} placeholder="سعر الوقف" style={{ width: '100%', padding: '6px 10px', borderRadius: 8, background: 'rgba(255,71,87,0.05)', border: '0.5px solid rgba(255,71,87,0.15)', fontSize: 12, fontWeight: 800, color: '#FF4757', fontFamily: 'var(--mono)', outline: 'none', appearance: 'none' }} />}
            </div>
          </div>
