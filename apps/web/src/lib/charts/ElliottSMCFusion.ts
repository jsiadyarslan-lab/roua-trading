...
export function detectElliottSMCFusion(opts: {
  ...
  if (confluenceScore >= 70 && direction !== 'neutral') {
    interpretationAr = `تقارب قوي ${direction === 'bullish' ? 'إيجابي' : 'سلبي'} — ${agreeingCount} من ${totalSignals} إشارات تتفق (مجموع التقارب: ${confluenceScore}%)`;
  } else if (confluenceScore >= 50 && direction !== 'neutral') {
    interpretationAr = `تقارب ${direction === 'bullish' ? 'إيجابي' : 'سلبي'} متوسط — بعض الإشارات متضاربة (مجموع التقارب: ${confluenceScore}%)`;
  } else {
    interpretationAr = `لا تقارب واضح — الإشارات متضاربة أو ضعيفة (مجموع التقارب: ${confluenceScore}%)`;
  }
