...
// ═══════════════════════════════════════════════════════════
// FIX (4.6): PatternStateMachineTracker — per-instance factory
//
// Previously, `trackedPatterns` was a module-level Map shared
// across all chart instances. In a multi-chart grid, pattern
// state tracking would bleed across charts. Now each tracker
// instance owns its own Map.
// ═══════════════════════════════════════════════════════════
export class PatternStateMachineTracker {
  private trackedPatterns = new Map<string, PatternStateInfo>();
  private readonly MAX_TRACKED = 50;

  /**
   * Determine the initial state of a pattern based on its properties
   */
  private determineInitialState(pattern: any): PatternState {
   ...
   */
  private calculateCompletion(pattern: any, state: PatternState): number {
    const quality = pattern.quality?.overall ?? pattern.quality ?? pattern.confidence * 100;
  ...
  /** Create a PatternStateMachine interface bound to this tracker instance */
  getMachine(): PatternStateMachine {
    return {
      update: (candles: CandleData[], patterns: any[]): PatternStateMachineResult => {
        ...
        for (const p of patterns) {
          ...
          if (this.trackedPatterns.has(id)) {
            const existing = this.trackedPatterns.get(id)!;
            const { newState, alert } = this.checkTransition(existing, candles, p);
            ...
            if (newState !== existing.state) {
              ...
              existing.completionPct = this.calculateCompletion(p, newState);
              existing.confidence = p.confidence || existing.confidence;
            }
          } else if (this.trackedPatterns.size < this.MAX_TRACKED) {
            // New pattern — create state entry
            const initialState = this.determineInitialState(p);
            ...
            const info: PatternStateInfo = {
              ...
              type: p.type || 'unknown',
              state: initialState,
              completionPct: this.calculateCompletion(p, initialState),
            ...
            // FIX (4.6): Use per-instance Map
            this.trackedPatterns.set(id, info);
        ...
        for (const [id, info] of this.trackedPatterns) {
          if ((info.state === 'confirmed' || info.state === 'failed') && info.lastUpdated < fiveMinAgo) {
            this.trackedPatterns.delete(id);
          }
        }

        // Compute summary
        const allStates = Array.from(this.trackedPatterns.values());
        const summary = {
      ...
      getStates: (): PatternStateInfo[] => {
        return Array.from(this.trackedPatterns.values());
      },
...
// ═══════════════════════════════════════════════════════════
// FIX (4.6): Factory function — creates a new per-instance tracker.
// ═══════════════════════════════════════════════════════════
export function createPatternStateMachineTracker(): PatternStateMachineTracker {
  return new PatternStateMachineTracker();
}

// ═══════════════════════════════════════════════════════════
// FIX (4.6): Legacy module-level singleton for backward compat.
// Deprecated — new code should use createPatternStateMachineTracker()
// for per-instance state in multi-chart mode.
// ═══════════════════════════════════════════════════════════
const _defaultTracker = new PatternStateMachineTracker();

/** @deprecated Use `createPatternStateMachineTracker()` for per-instance state */
export function getPatternStateMachine(): PatternStateMachine {
  return _defaultTracker.getMachine();
}
