...
<head>
<meta charset="UTF-8">
<title>تقرير أداء نظام رواء للتداول الذكي</title>
<style>
...
<div class="header">
  <h1>تقرير أداء نظام رواء للتداول الذكي</h1>
  <p>الفترة: ${stats.period} | تم التصدير: ${new Date().toLocaleDateString('ar-SA')} | V260</p>
</div>
...
<div class="section">
  ...
  <table>
    <tr><th>الاتجاه</th><th>الصفقات</th><th>الرابحة</th><th>نسبة النجاح</th><th>صافي الربح</th></tr>
    <tr><td>شراء (صاعد)</td><td>${stats.byDirection.bullish.trades}</td><td>${stats.byDirection.bullish.wins}</td><td class="${stats.byDirection.bullish.winRate >= 0.5 ? 'win' : 'loss'}">${Math.round(stats.byDirection.bullish.winRate * 100)}%</td><td class="${stats.byDirection.bullish.pnl >= 0 ? 'win' : 'loss'}">${stats.byDirection.bullish.pnl}</td></tr>
    <tr><td>بيع (هابط)</td><td>${stats.byDirection.bearish.trades}</td><td>${stats.byDirection.bearish.wins}</td><td class="${stats.byDirection.bearish.winRate >= 0.5 ? 'win' : 'loss'}">${Math.round(stats.byDirection.bearish.winRate * 100)}%</td><td class="${stats.byDirection.bearish.pnl >= 0 ? 'win' : 'loss'}">${stats.byDirection.bearish.pnl}</td></tr>
  </table>
</div>

<div class="section">
  <h2>الأداء حسب مصدر الإشارة</h2>
  <table>
    <tr><th>المصدر</th><th>الصفقات</th><th>نسبة النجاح</th><th>صافي الربح</th><th>متوسط الثقة</th></tr>
...
<div class="section">
  <h2>تأثير المحركات الثورية</h2>
  <table>
    <tr><th>النوع</th><th>الصفقات</th><th>نسبة النجاح</th></tr>
    <tr><td>مع تعزيز الثوري</td><td>${stats.boostTradesCount}</td><td class="${stats.boostTradesWinRate >= 0.5 ? 'win' : 'loss'}">${Math.round(stats.boostTradesWinRate * 100)}%</td></tr>
    <tr><td>بدون تعزيز</td><td>${stats.noBoostTradesCount}</td><td class="${stats.noBoostTradesWinRate >= 0.5 ? 'win' : 'loss'}">${Math.round(stats.noBoostTradesWinRate * 100)}%</td></tr>
    <tr><td>التحسن</td><td colspan="2" class="${stats.boostLift >= 1 ? 'win' : 'loss'}">${stats.boostLift > 0 ? (stats.boostLift >= 1 ? '+' : '') + Math.round((stats.boostLift - 1) * 100) + '%' : 'لا بيانات كافية'}</td></tr>
  </table>
</div>
` : ''}
...
<div class="section">
  <h2>الأداء الأسبوعي</h2>
  <table>
    <tr><th>الأسبوع</th><th>الصفقات</th><th>الرابحة</th><th>نسبة النجاح</th><th>الربح</th><th>التراكمي</th></tr>
...
<div class="section">
  <h2>آخر الصفقات</h2>
  <table>
    <tr><th>التاريخ</th><th>الاتجاه</th><th>الدخول</th><th>SL</th><th>النتيجة</th><th>R:R</th><th>الربح</th><th>الإشارات</th></tr>
    ${closed.slice(-20).reverse().map(e => `<tr>
      <td>${e.date} ${e.time}</td>
      <td>${e.direction === 'bullish' ? 'شراء' : 'بيع'}</td>
      <td>${e.entryPrice}</td>
      <td>${e.stopLoss}</td>
...
<div class="footer">
  <p>تقرير مُنشأ تلقائياً من منصة رواء للتداول الذكي — V260</p>
  <p>هذا التقرير يعكس أداء نظام التحليل الآلي على بيانات حية. النتائج السابقة لا تضمن النتائج المستقبلية.</p>
</div>
