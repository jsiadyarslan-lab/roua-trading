...
const DEFAULT_RULES: AlertRule[] = [
 ...
 nameAr: 'pattern + but or within PRZ',
 conditions: [
...
export function fireBrowserNotification(alert: TriggeredAlert): void {
 ...
 body: `direction: ${alert.direction === 'bullish' ? 'bullish' : alert.direction === 'bearish' ? 'bearish' : 'neutral'} | confidence: ${Math.round(alert.confidence * 100)}% | level: ${alert.keyLevel.toFixed(2)}`,
 tag: alert.ruleId,
 silent: false,
 });
 }
