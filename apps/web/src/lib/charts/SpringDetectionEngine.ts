...
export function detectSprings(opts: {
      ...
      if (c.low < sup && c.close > sup) {
        ...
        if (penetration > atr * 0.3 && penetration < atr * 2) {
          ...
          if (wyckoffSprings?.some(s => s.direction === 'bullish')) {
            confirmations.push('سوستة ويكوف مؤكدة');
          }
          if (recentBreaks?.some(b => b.direction === 'bullish')) {
            confirmations.push('كسر هيكل صاعد لاحق');
          }
          ...
          springs.push({
            type: 'stop_hunt',
            nameAr: 'اصطياد وقف خسارة (صاعد)',
            descriptionAr: `السعر اخترق الدعم ${sup.toFixed(2)} ثم أغلق فوقه — اصطياد لوقف الخسارة. دخول عند ${entry.toFixed(2)}`,
            direction: 'bullish',
      ...
      if (c.high > res && c.close < res) {
        ...
        if (penetration > atr * 0.3 && penetration < atr * 2) {
          ...
          if (recentBreaks?.some(b => b.direction === 'bearish')) {
            confirmations.push('كسر هيكل هابط لاحق');
          }
          ...
          springs.push({
            type: 'stop_hunt',
            nameAr: 'اصطياد وقف خسارة (هابط)',
            descriptionAr: `السعر اخترق المقاومة ${res.toFixed(2)} ثم أغلق دونها — اصطياد لوقف الخسارة. دخول عند ${entry.toFixed(2)}`,
            direction: 'bearish',
            confidence: 0.55 + (confirmations.length > 0 ? 0.15 : 0),
            springLevel: res,
            penetrationDepth: penetration,
  ...
  if (last7.length >= 7) {
    ...
    if (recent3Narrow) {
      ...
      springs.push({
        type: 'springboard',
        nameAr: 'لوح سوستة (Springboard)',
        descriptionAr: `ضغط شموع (${lastRange.toFixed(2)} < متوسط ${avgRange.toFixed(2)}) — حركة انفجارية وشيكة باتجاه ${dir === 'bullish' ? 'صاعد' : 'هابط'}`,
        direction: dir,
        ...
        levelType: 'pivot',
        candleIndex: len - 1,
        confirmations: bullSignals + bearSignals > 0 ? [`كسر هيكل ${dir === 'bullish' ? 'صاعد' : 'هابط'}`] : [],
        ageCandles: 0,
        isActionable: true,
      });
    }
  ...
  const summaryAr = springs.length === 0
    ? 'لا توجد سوستات أو فخوخ مكتشفة حالياً'
    : `${springs.length} سوستة/فخ مكتشف | الأفضل: ${bestSetup?.nameAr || 'لا يوجد'} | قوة الإشارة: ${Math.round(signalStrength)}%`;
...
function detectSpringAtLevel(
  ...
  if (levelType === 'support') {
    ...
    if (recentBreaks?.some(b => b.direction === 'bullish' && b.price > level)) {
      confirmations.push('كسر هيكل صاعد بعد السوستة');
    }
    ...
    if (springCandle.volume && springCandle.volume > 0) {
      ...
      if (springCandle.volume > avgVol * 1.5) {
        confirmations.push('حجم تداول مرتفع عند السوستة');
      }
    }
    ...
    return {
      type: 'spring',
      nameAr: 'سوستة صاعدة (Spring)',
      descriptionAr: `السعر اخترق الدعم ${level.toFixed(2)} بمقدار ${maxPenetration.toFixed(2)} ثم ارتد — سوستة ويكوف صاعدة كلاسيكية`,
      direction: 'bullish',
      confidence,
      springLevel: level,
    ...
    if (recentBreaks?.some(b => b.direction === 'bearish' && b.price < level)) {
      confirmations.push('كسر هيكل هابط بعد السوستة');
    }
    ...
    return {
      type: 'upthrust',
      nameAr: 'سوستة هابطة (Upthrust)',
      descriptionAr: `السعر اخترق المقاومة ${level.toFixed(2)} بمقدار ${maxPenetration.toFixed(2)} ثم ارتد — سوستة ويكوف هابطة كلاسيكية`,
      direction: 'bearish',
      confidence,
      springLevel: level,
      penetrationDepth: maxPenetration,
...
function emptyResult(): SpringDetectionResult {
  return {
    ...
    bestSetup: null,
    signalStrength: 0,
    summaryAr: 'لا توجد سوستات أو فخوخ مكتشفة حالياً',
  };
}
