...
function ChartContent() {
  ...
  const executeOrder = async (side: 'buy' | 'sell') => {
    ...
    try {
      ...
      if (credentialId) {
        ...
        if (res.ok && j.id) { success = true; filledPrice = j.filledAvgPrice || j.avgFillPrice || livePrice || 0; source = 'nestjs' }
      ...
      try {
        ...
        const j = await res.json()
        if (j.success) { success = true; filledPrice = j.filledAvgPrice ? parseFloat(j.filledAvgPrice) : (livePrice || 0); source = 'alpaca' }
  ...
  const ToolBtn = ({ children, onClick, active, title }: { children: React.ReactNode; onClick: () => void; active?: boolean; title: string }) => (
    <button onClick={onClick} title={title} style={{ width: 44, height: 32, borderRadius: 6, background: active ? 'rgba(0,212,255,0.12)' : 'transparent', border: active ? '1px solid rgba(0,212,255,0.3)' : '1px solid transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: active ? '#00D4FF' : 'rgba(255,255,255,0.5)', touchAction: 'manipulation' }}>
      {children}
    </button>
  )
    ...
    <div className="m-page--chart">
      ...
      {showOrderSheet && (
        <>
          ...
          <div style={{ position: 'fixed', bottom: 'calc(56px + env(safe-area-inset-bottom, 0px))', left: 0, right: 0, zIndex: 55, background: C.bg, backdropFilter: 'blur(50px) saturate(200%)', borderRadius: '20px 20px 0 0', borderTop: '0.5px solid rgba(255,255,255,0.12)', direction: 'rtl', boxShadow: '0 -10px 40px rgba(0,0,0,0.5)', maxHeight: '75vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
