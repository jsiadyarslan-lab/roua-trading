  ...
  return (
    <div className="m-page" style={{ direction: 'rtl' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
        ...
        <span style={{ fontSize: 20, fontWeight: 900, color: '#FFF', fontFamily: 'var(--cairo)' }}>ربط الحسابات</span>
      </div>
      <div className="m-card" style={{ marginBottom: 12, padding: '14px', background: 'rgba(0,212,255,0.04)', border: '0.5px solid rgba(0,212,255,0.1)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 700, color: '#00D4FF', fontFamily: 'var(--cairo)' }}>اربط حساب التداول الخاص بك لبدء المتاجرة</span>
        </div>
      </div>
      ...
      {!loading && exchanges.length === 0 && (
        <div className="m-card" style={{ textAlign: 'center', padding: 40 }}>
          ...
          <div style={{ fontSize: 14, fontWeight: 800, color: '#8B92A8', fontFamily: 'var(--cairo)' }}>لا توجد حسابات مربوطة</div>
        </div>
      )}
      {exchanges.map(ex => (
        <div key={ex.id} className="m-card" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {ex.connected ? <Check size={16} color="#00FFA3" /> : <span style={{ fontSize: 10, color: 'var(--text3)', fontFamily: 'var(--cairo)' }}>غير مربوط</span>}
        </div>
      ))}
    </div>
  )
