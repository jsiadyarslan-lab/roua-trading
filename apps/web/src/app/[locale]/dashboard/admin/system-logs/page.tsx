    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h1 style={{ fontSize: 'var(--text-lg)', fontWeight: 700, color: COLORS.text, fontFamily: "var(--font-ar)", margin: 0 }}>سجلات النظام</h1>
            <p style={{ fontSize: 'var(--text-sm)', color: COLORS.muted, fontFamily: "var(--font-ar)", margin: '4px 0 0' }}>جاري تحميل السجلات...</p>
          </div>
        </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 'var(--text-lg)', fontWeight: 700, color: COLORS.text, fontFamily: "var(--font-ar)", margin: 0 }}>سجلات النظام</h1>
          <p style={{ fontSize: 'var(--text-sm)', color: COLORS.muted, fontFamily: "var(--font-ar)", margin: '4px 0 0' }}>مراقبة الأحداث والأخطاء في الوقت الفعلي</p>
        </div>
      ...
      {error && (
        <div style={{
          ...
          color: COLORS.danger, fontSize: 'var(--text-sm)', fontFamily: "var(--font-ar)",
        }}>
          <AlertCircle size={14} />
          {error}
        </div>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
        {(['all', 'info', 'warning', 'error'] as const).map(lv => {
          ...
          return (
            <button key={lv} onClick={() => setFilter(lv)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 'var(--radius-sm)', border: filter === lv ? `1px solid ${color}30` : `1px solid ${COLORS.border}`, background: filter === lv ? `${color}10` : 'rgba(255,255,255,0.03)', color: filter === lv ? color : COLORS.muted, fontSize: 'var(--text-xs)', fontFamily: "var(--font-ar)", cursor: 'pointer', transition: 'all 0.15s' }}>
              <span style={{ width: 6, height: 6, borderRadius: '50%', background: color }} />
              {label}
              <span style={{ fontSize: 'var(--text-xs)', fontFamily: "var(--font-mono)", fontWeight: 700 }}>{counts[lv]}</span>
            </button>
          )
        })}
        <div style={{ flex: 1, minWidth: 180, display: 'flex', alignItems: 'center', gap: 6, padding: '6px 10px', borderRadius: 'var(--radius-sm)', background: 'rgba(255,255,255,0.03)', border: `1px solid ${COLORS.border}` }}>
          ...
          <input type="text" placeholder="بحث في السجلات..." value={search} onChange={e => setSearch(e.target.value)} style={{ flex: 1, background: 'transparent', border: 'none', outline: 'none', color: COLORS.text, fontSize: 'var(--text-xs)', fontFamily: "var(--font-ar)" }} dir={dir} />
        </div>
      </div>
      ...
      <div style={{ background: 'rgba(255,255,255,0.02)', border: `1px solid ${COLORS.border}`, borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
        <div style={{ padding: '8px 12px', borderBottom: `1px solid ${COLORS.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(0,0,0,0.2)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ fontSize: 'var(--text-xs)', color: COLORS.muted }}>سجل الأحداث</span>
          </div>
          <span style={{ fontSize: 'var(--text-xs)', color: COLORS.muted }}>{filtered.length} سجل</span>
        </div>
        <div className="custom-scrollbar" style={{ height: 480, overflowY: 'auto', padding: 4, background: 'rgba(0,0,0,0.3)' }}>
          {filtered.length === 0 ? (
            <div style={{ padding: 40, textAlign: 'center', color: COLORS.muted, fontSize: 'var(--text-sm)', fontFamily: "var(--font-ar)" }}>
            ...
            return (
              <div key={log.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: '6px 10px', borderRadius: 'var(--radius-sm)', marginBottom: 2, background: `${cfg.color}10`, borderInlineStart: `2px solid ${cfg.color}`, fontSize: 'var(--text-xs)', lineHeight: 1.6 }}>
                <LIcon size={12} color={cfg.color} style={{ flexShrink: 0, marginTop: 3 }} />
                <span style={{ color: COLORS.muted, flexShrink: 0, fontSize: 'var(--text-xs)' }} dir="ltr">{new Date(log.timestamp).toLocaleTimeString('ar-SA', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span>
