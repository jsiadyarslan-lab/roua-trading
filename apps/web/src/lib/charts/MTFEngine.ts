...
const TREND_STATE_AR: Record<TFTrendState, string> = {
 'uptrend': 'direction bullish',
 'downtrend': 'direction bearish',
 'ranging': 'ranging/',
 'counter-uptrend': ' bullish ( direction)',
 'counter-downtrend': ' bearish ( direction)',
};
...
function detectMTFDivergences(tfAnalyses: TimeframeAnalysis[]): MTFDivergence[] {
 ...
 descriptionAr: `spacing : ${TF_LABELS_AR[higher.timeframe]} bearish but ${TF_LABELS_AR[lower.timeframe]} about — possibility reversal`,
 ...
 higherTF: higher.timeframe,
 lowerTF: lower.timeframe,
 descriptionAr: `spacing : ${TF_LABELS_AR[higher.timeframe]} bullish but ${TF_LABELS_AR[lower.timeframe]} about — possibility correct`,
 significance: Math.min(1.0, higher.strength * 0.5 + lower.strength * 0.3),
 });
 }
 ...
 higherTF: higher.timeframe,
 lowerTF: lower.timeframe,
 descriptionAr: `spacing momentum: ${TF_LABELS_AR[higher.timeframe]} rushes but ${TF_LABELS_AR[lower.timeframe]} — momentum`,
 significance: 0.4,
 });
 }
 }
...
export function runMTFAnalysis(
 ...
 `${TF_LABELS_AR[tf.timeframe]}: ${tf.direction === 'bullish' ? '🟢 bullish' : tf.direction === 'bearish' ? '🔴 bearish' : '⚪ neutral'} (${Math.round(tf.strength * 100)}% — ${TREND_STATE_AR[tf.trendState]})`
 ).join(' | ');

 let interpretationAr: string;
 if (score >= 70 && direction !== 'neutral') {
 interpretationAr = `confluence ${dirAr} strong through what — ${agreeingTFs} who ${tfAnalyses.length} what (: ${score}%). ${tfSummary}`;
 } else if (score >= 50 && direction !== 'neutral') {
 interpretationAr = `confluence ${dirAr} center — some what (: ${score}%). ${tfSummary}`;
 } else {
 interpretationAr = ` confluence clear — what or neutral (: ${score}%). ${tfSummary}`;
 }
 ...
 const trendWord = direction === 'bullish' ? '' : '';
 entryReasonAr = `direction ${trendWord} — confirmation on ${TF_LABELS_AR[preferredTF]} entry`;
 // Add fib confluence hints
 if (fibConfluences.length > 0) {
 entryReasonAr += ` | confluence in at ${fibConfluences[0].price}`;
 }
 if (srConfluences.length > 0) {
 entryReasonAr += ` | ${srConfluences[0].labelAr} at ${srConfluences[0].price}`;
 }
 } else {
 entryReasonAr = 'no signal entry clear — confluence best across timeframeswhat';
 }
