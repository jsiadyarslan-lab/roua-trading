...
export default function MobileMarketsPage() {
  ...
  return (
    <div className="r-page">
      ...
      {/* Hot mover */}
      {hotMover && (
        <div style={{ margin: '0 var(--space-lg) 8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 10px', borderRadius: 10, background: 'rgba(255,183,0,0.04)', border: '0.5px solid rgba(255,183,0,0.1)' }}>
          ...
          <span style={{ fontSize: 10, fontWeight: 900, color: '#FFF', fontFamily: 'var(--font-mono)' }}>{hotMover.s} {(hotMover.q?.changePercent ?? 0) >= 0 ? '+' : ''}{(hotMover.q?.changePercent ?? 0).toFixed(2)}%</span>
        </div>
      )}
          ...
          <Card key={sym} onClick={() => router.push(`/mobile/chart?symbol=${sym}`)}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 32, height: 32, borderRadius: 8, background: 'rgba(255,255,255,0.04)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 900, color: 'rgba(255,255,255,0.4)', fontFamily: 'var(--font-mono)', border: '0.5px solid rgba(255,255,255,0.06)' }}>{sym.split('/')[0].slice(0, 2)}</div>
                <span style={{ fontSize: 13, fontWeight: 800, color: '#FFF', fontFamily: 'var(--font-mono)' }}>{sym}</span>
              </div>
              <div style={{ textAlign: 'left' }}>
                {price !== null ? <div style={{ fontSize: 13, fontWeight: 900, color: '#FFF', fontFamily: 'var(--font-mono)' }}>{price > 100 ? price.toLocaleString('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : price.toFixed(price < 10 ? 4 : 2)}</div> : <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.2)' }}>—</div>}
                {q && <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 4 }}><span style={{ fontSize: 10, fontWeight: 800, color, fontFamily: 'var(--font-mono)' }}>{isUp ? '+' : ''}{changePct.toFixed(2)}%</span>{isUp ? <TrendingUp size={10} color={color} /> : <TrendingDown size={10} color={color} />}</div>}
              </div>
            </div>
          </Card>
        )
