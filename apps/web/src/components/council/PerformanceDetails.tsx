  ...
  if (!stats) {
    return (
      <GlassCard padding={36} style={{ textAlign: 'center' }}>
        <div style={{ fontSize: 'var(--text-base)', fontWeight: 600, color: COLORS.textPrimary, marginBottom: 4 }}>No analysis</div>
        <div style={{ fontSize: 'var(--text-sm)', color: COLORS.textMuted }}>Performance data will appear after briefs are issued</div>
      </GlassCard>
    );
  }
  ...
  const Bar = ({ label, value, total, color }: { label: string; value: number; total: number; color: string }) => {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        <span style={{ fontSize: 'var(--text-xs)', fontWeight: 600, color: COLORS.textMuted, minWidth: 60, textTransform: 'uppercase', letterSpacing: '0.04em' }}>{label}</span>
        <div style={{ flex: 1, height: 8, borderRadius: 'var(--radius-2xl)', background: 'rgba(255,255,255,0.04)', overflow: 'hidden' }}>
          <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }} style={{ height: '100%', background: `linear-gradient(90deg, ${color}, ${hexToRgba(color, 0.6)})`, borderRadius: 'var(--radius-2xl)' }} />
        </div>
        <span style={{ fontSize: 'var(--text-sm)', fontWeight: 600, color: COLORS.textPrimary, fontFamily: "var(--font-mono)", minWidth: 40, textAlign: 'right' }}>{value}</span>
        <span style={{ fontSize: 'var(--text-xs)', color, fontFamily: "var(--font-mono)", minWidth: 36, textAlign: 'right' }}>{pct.toFixed(0)}%</span>
      </div>
    );
  };
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
      ...
      <GlassCard padding={20}>
        <div style={{ fontSize: 'var(--text-xs)', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: COLORS.council, marginBottom: 14 }}>Status Distribution</div>
      ...
      <GlassCard padding={20}>
        <div style={{ fontSize: 'var(--text-xs)', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: COLORS.council, marginBottom: 14 }}>Direction Split</div>
        ...
        <div style={{ marginTop: 12, padding: '10px 12px', borderRadius: 'var(--radius-md)', background: hexToRgba(COLORS.council, 0.08), border: `1px solid ${hexToRgba(COLORS.council, 0.2)}` }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: 'var(--text-xs)', fontWeight: 600, color: COLORS.textMuted, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Avg Confidence</span>
            <span style={{ fontSize: 'var(--text-lg)', fontWeight: 600, color: COLORS.council, fontFamily: "var(--font-mono)" }}>{stats.avgConf.toFixed(0)}%</span>
          </div>
        </div>
      </GlassCard>
      ...
      {/* By Timeframe */}
      <GlassCard padding={20}>
        <div style={{ fontSize: 'var(--text-xs)', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: COLORS.council, marginBottom: 14 }}>By Timeframe</div>
      ...
      {/* By Pair */}
      <GlassCard padding={20}>
        <div style={{ fontSize: 'var(--text-xs)', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: COLORS.council, marginBottom: 14 }}>By Pair</div>
