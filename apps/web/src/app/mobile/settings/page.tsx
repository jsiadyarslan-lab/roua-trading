  ...
  return (
    <div className="m-page" style={{ direction: 'rtl' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
        ...
        <span style={{ fontSize: 20, fontWeight: 900, color: '#FFF', fontFamily: 'var(--cairo)' }}>الإعدادات</span>
      </div>
      <div className="m-card">
        <div style={{ fontSize: 13, fontWeight: 800, color: '#FFF', fontFamily: 'var(--cairo)', marginBottom: 12 }}>وضع التداول</div>
        <div style={{ display: 'flex', gap: 6 }}>
          {(['trader', 'investor', 'ai'] as TradingMode[]).map(m => (
            <button key={m} onClick={() => setMode(m)} style={{ flex: 1, padding: '8px 0', borderRadius: 8, border: mode === m ? '0.5px solid rgba(0,212,255,0.3)' : '0.5px solid var(--border)', background: mode === m ? 'rgba(0,212,255,0.06)' : 'transparent', color: mode === m ? 'var(--accent)' : 'var(--text3)', fontSize: 11, fontWeight: 800, fontFamily: 'var(--cairo)', cursor: 'pointer' }}>
              {m === 'trader' ? 'تاجر' : m === 'investor' ? 'مستثمر' : 'ذكاء'}
            </button>
          ))}
        </div>
      </div>
      <div className="m-card">
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <span style={{ fontSize: 13, fontWeight: 800, color: '#FFF', fontFamily: 'var(--cairo)' }}>الإشعارات</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 13, fontWeight: 800, color: '#FFF', fontFamily: 'var(--cairo)' }}>الصوت</span>
