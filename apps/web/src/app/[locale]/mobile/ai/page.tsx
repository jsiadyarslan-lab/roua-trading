...
export default function AIPage() {
  ...
  const t = useTranslations('mobile.ai')
  ...
  const recLabel = rec === 'BUY' ? tc('buy') : rec === 'SELL' ? tc('sell') : tc('hold')
    ...
    <div className="m-page">
      ...
      <div className="m-card">
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
          ...
          <div><div style={{ fontSize: 15, fontWeight: 800, color: '#FFF', fontFamily: 'var(--cairo)' }}>{t('models')}</div><div style={{ fontSize: 10, color: '#B388FF', fontFamily: 'var(--cairo)' }}>{selectedSymbol}</div></div>
        </div>
        {!loading && consensus && (
          <div style={{ padding: '10px 14px', borderRadius: 12, background: `${color}08`, border: `0.5px solid ${color}18` }}>
            <div style={{ fontSize: 14, fontWeight: 800, color, fontFamily: 'var(--cairo)' }}>{t('recommendation')}: {recLabel}</div>
            <div style={{ fontSize: 11, color: '#8B92A8', fontFamily: 'var(--cairo)' }}>{t('confidence')} {score}%</div>
          </div>
        )}
