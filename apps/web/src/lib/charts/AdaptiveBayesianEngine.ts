...
export function runAdaptiveBayesian(
 ...
 insights.push(`${sig.source}: strong (with success ${Math.round(perf.emaWinRate * 100)}%, ${adaptiveWeight.toFixed(2)})`);
 } else if (adaptiveWeight < 0.7) {
 insights.push(`${sig.source}: weak (with success ${Math.round(perf.emaWinRate * 100)}%, ${adaptiveWeight.toFixed(2)})`);
 }
 if (isUserOverride) {
 insights.push(`${sig.source}: custom who user (${userOverrides[sig.source].toFixed(2)})`);
 }
 }
 }
 ...
 insights.unshift(`system market: ${regimeLabelsAr[regime]}`);
