...
export function generateTradeProposal(opts: {
 ...
 revBoostDescription += ` | in order to ${cb.partnerSource} ( ${Math.round(cb.lift * 100 - 100)}%)`;
 }
 }
 ...
 revBoostDescription += ` | pattern ${pred.patternType} ${pred.completionPct}% complete`;
 }
 }
 ...
 // ── Arabic Description ──
 const dirAr = direction === 'bullish' ? '' : '';
 const signalNames = agreeingSignals.map(s => s.source).join(' + ');
 let descriptionAr = ` ${dirAr}: confluence ${agreeingSignals.length} signals (${signalNames}) | R:R = 1:${rrRatio.toFixed(1)} | confidence ${Math.round(confidence * 100)}% | ${qualityScore}`;

 if (mtfConfluence && mtfConfluence.direction === direction) {
 descriptionAr += ` | MTF: ${mtfConfluence.agreeingTFs} what`;
 }
