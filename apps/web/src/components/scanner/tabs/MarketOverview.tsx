    ...
    <div style={{ flex: 1, overflow: 'auto', direction: 'inherit', background: '#151A22', padding: 16 }}>
      ...
      <div style={{ animation: 'fadeInMO 0.4s ease' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
          ...
          <span style={{ fontSize: 'var(--text-base)', fontWeight: 800, color: '#F0F2F5', fontFamily: "var(--font-ar)" }}>{t('overview.marketOverview')}</span>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 24, padding: '20px 16px', borderRadius: 'var(--radius-lg)', background: '#0F1117', border: `0.5px solid ${'#2A313C'}`, marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              <span style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: '#00FFA3', fontFamily: "var(--font-ar)" }}>{t('bullish')}: {sentiment.bullCount}</span>
              <span style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: '#FF4757', fontFamily: "var(--font-ar)" }}>{t('bearish')}: {sentiment.bearCount}</span>
              <span style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: '#6B7280', fontFamily: "var(--font-ar)" }}>{t('neutral')}: {sentiment.neutralCount}</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 16 }}>
          ...
          <div style={{ borderRadius: 'var(--radius-md)', border: `0.5px solid ${'#2A313C'}`, background: '#0F1117', padding: 12 }}>
            <div style={{ fontSize: 'var(--text-xs)', fontWeight: 800, color: '#00D4FF', fontFamily: "var(--font-ar)", marginBottom: 10 }}>{t('overview.strongestSignals')}</div>
            {strongest.map(item => (
              <div key={item.symbol} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 0', borderBottom: `0.5px solid ${'#2A313C'}` }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  <span style={{ fontSize: 'var(--text-xs)', fontWeight: 800, color: '#F0F2F5', fontFamily: "var(--font-mono)" }}>{item.symbol}</span>
                  <DirectionTag direction={item.direction} size="sm" />
                </div>
                <div style={{ textAlign: 'left' }}>
                  <span style={{ fontSize: 'var(--text-xs)', fontWeight: 800, color: item.technicalScore >= 40 ? '#00FFA3' : '#FFB800', fontFamily: "var(--font-mono)" }}>
                    {item.technicalScore > 0 ? '+' : ''}{item.technicalScore}
                  </span>
                  <span style={{ fontSize: 'var(--text-xs)', color: '#6B7280', fontFamily: "var(--font-mono)", marginInlineEnd: 4 }}>
          ...
          {/* Top Gainers */}
          <div style={{ borderRadius: 'var(--radius-md)', border: `0.5px solid ${'#2A313C'}`, background: '#0F1117', padding: 12 }}>
            <div style={{ fontSize: 'var(--text-xs)', fontWeight: 800, color: '#00FFA3', fontFamily: "var(--font-ar)", marginBottom: 10 }}>{t('overview.topGainers')}</div>
            {gainers.map(item => (
              <div key={item.symbol} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 0', borderBottom: `0.5px solid ${'#2A313C'}` }}>
                <span style={{ fontSize: 'var(--text-xs)', fontWeight: 800, color: '#F0F2F5', fontFamily: "var(--font-mono)" }}>{item.symbol}</span>
                <div style={{ textAlign: 'left' }}>
                  <div style={{ fontSize: 'var(--text-xs)', fontWeight: 800, color: '#00FFA3', fontFamily: "var(--font-mono)" }}>
          ...
          {/* Top Losers */}
          <div style={{ borderRadius: 'var(--radius-md)', border: `0.5px solid ${'#2A313C'}`, background: '#0F1117', padding: 12 }}>
            <div style={{ fontSize: 'var(--text-xs)', fontWeight: 800, color: '#FF4757', fontFamily: "var(--font-ar)", marginBottom: 10 }}>{t('overview.topLosers')}</div>
            {losers.map(item => (
              <div key={item.symbol} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 0', borderBottom: `0.5px solid ${'#2A313C'}` }}>
                <span style={{ fontSize: 'var(--text-xs)', fontWeight: 800, color: '#F0F2F5', fontFamily: "var(--font-mono)" }}>{item.symbol}</span>
                <div style={{ textAlign: 'left' }}>
                  <div style={{ fontSize: 'var(--text-xs)', fontWeight: 800, color: '#FF4757', fontFamily: "var(--font-mono)" }}>
                    {item.changePercent.toFixed(2)}%
                  </div>
                  <div style={{ fontSize: 'var(--text-xs)', color: '#6B7280', fontFamily: "var(--font-mono)" }}>
                    ${item.price.toLocaleString()}
        ...
        <div style={{ borderRadius: 'var(--radius-md)', border: `0.5px solid ${'#2A313C'}`, background: '#0F1117', padding: 14 }}>
          <div style={{ fontSize: 'var(--text-xs)', fontWeight: 800, color: '#9CA3B5', fontFamily: "var(--font-ar)", marginBottom: 10 }}>{t('overview.sectorDistribution')}</div>
          {SECTORS.map(s => {
            ...
            return (
              <div key={s.key} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                ...
                <span style={{ width: 32, fontSize: 'var(--text-xs)', fontWeight: 800, color: avg >= 0 ? '#00FFA3' : '#FF4757', fontFamily: "var(--font-mono)", textAlign: 'left', flexShrink: 0 }}>
                  {avg > 0 ? '+' : ''}{avg}
                </span>
              </div>
            )
