...
import {
  ...
  Cpu, Activity, Shield, DollarSign, Flame, ArrowUpRight, ArrowDownRight,
  Zap, Loader2, Link2, Target, RefreshCw, Sparkles,
} from 'lucide-react'
...
function CurrencyTicker() {
  ...
  return (
    <div style={{ overflow: 'hidden', height: 36, display: 'flex', alignItems: 'center', background: 'rgba(0,212,255,0.03)', borderBottom: '0.5px solid rgba(0,212,255,0.08)', marginBottom: 8 }}>
      <div style={{ paddingInlineStart: 16, zIndex: 2, display: 'flex', alignItems: 'center', width: 80, position: 'relative' }}>
      ...
      <div style={{ display: 'flex', gap: 32, animation: 'tickerScroll 25s linear infinite', whiteSpace: 'nowrap', direction: 'ltr', width: 'max-content', paddingInlineStart: 90 }}>
        {[...displayPairs, ...displayPairs].map((pair, i) => {
          ...
          return (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{ fontSize: 11, fontWeight: 800, color: '#FFF', fontFamily: "'JetBrains Mono', monospace" }}>{pair}</span>
              <span style={{ fontSize: 11, fontWeight: 800, color: pos ? '#32D74B' : '#FF453A', fontFamily: "'JetBrains Mono', monospace" }}>
                {price ? price.toLocaleString('en', { minimumFractionDigits: price < 10 ? 4 : 2 }) : '—'}
              </span>
              <span style={{ fontSize: 9, fontWeight: 700, color: pos ? '#32D74B' : '#FF453A', fontFamily: "'JetBrains Mono', monospace" }}>
...
function Watchlist() {
  ...
  const SYMBOLS = tab === 'Crypto'
    ...
    : ['EUR/USD', 'GBP/USD', 'USD/JPY', 'XAU/USD', 'AUD/USD', 'USD/CHF']
    ...
    <IOSCard>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ width: 26, height: 26, borderRadius: 8, background: 'linear-gradient(135deg, #00D4FF, #5B21B6)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <BarChart3 size={13} color="#FFF" />
          </div>
          <span style={{ fontSize: 14, fontWeight: 800, color: '#FFF', fontFamily: "'Cairo', sans-serif" }}>قائمة المراقبة</span>
        </div>
        <button onClick={() => router.push('/mobile/markets')} style={{ fontSize: 11, color: '#00D4FF', fontWeight: 800, fontFamily: "'Cairo', sans-serif", background: 'none', border: 'none' }}>المزيد</button>
      </div>

      <div style={{ display: 'flex', gap: 0, marginBottom: 10, background: 'rgba(255,255,255,0.03)', borderRadius: 10, padding: 2 }}>
        {(['Crypto', 'Forex'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} style={{ flex: 1, padding: '5px 0', borderRadius: 8, background: tab === t ? 'rgba(0,212,255,0.12)' : 'transparent', border: 'none', color: tab === t ? '#00D4FF' : 'rgba(255,255,255,0.35)', fontSize: 10, fontWeight: 800, fontFamily: "'Cairo', sans-serif", cursor: 'pointer' }}>
      ...
      {hotMover && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 10px', borderRadius: 10, background: 'rgba(255,183,0,0.04)', border: '0.5px solid rgba(255,183,0,0.1)', marginBottom: 8 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ fontSize: 10, fontWeight: 900, color: '#FFF', fontFamily: "'JetBrains Mono', monospace" }}>{hotMover.s}</span><span style={{ fontSize: 9, fontWeight: 800, color: (hotMover.q?.changePercent ?? 0) >= 0 ? '#32D74B' : '#FF453A', fontFamily: "'JetBrains Mono', monospace" }}>{(hotMover.q?.changePercent ?? 0) >= 0 ? '+' : ''}{(hotMover.q?.changePercent ?? 0).toFixed(2)}%</span></div>
        </div>
      )}

      <div style={{ maxHeight: 240, overflowY: 'auto' }} className="m-no-scroll">
        {SYMBOLS.map(sym => {
          const q = quotes[sym]
          ...
          return (
            <div key={sym} onClick={() => { setSelectedSymbol(sym); router.push(`/mobile/chart?symbol=${sym}`) }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 6px', borderRadius: 10, background: sel ? 'rgba(0,212,255,0.06)' : 'transparent', border: sel ? '0.5px solid rgba(0,212,255,0.15)' : '0.5px solid transparent', marginBottom: 2, cursor: 'pointer' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{ width: 28, height: 28, borderRadius: 8, background: sel ? 'rgba(0,212,255,0.1)' : 'rgba(255,255,255,0.04)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 900, color: sel ? '#00D4FF' : 'rgba(255,255,255,0.4)', fontFamily: "'JetBrains Mono', monospace", border: sel ? '0.5px solid rgba(0,212,255,0.2)' : '0.5px solid rgba(255,255,255,0.06)' }}>{sym.split('/')[0].slice(0, 2)}</div>
                <span style={{ fontSize: 11, fontWeight: 800, color: '#FFF', fontFamily: "'JetBrains Mono', monospace" }}>{sym}</span>
              </div>
              <div style={{ textAlign: 'left' }}>
                {price !== null ? <div style={{ fontSize: 11, fontWeight: 900, color: '#FFF', fontFamily: "'JetBrains Mono', monospace" }}>{price > 100 ? price.toLocaleString('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : price.toFixed(price < 10 ? 4 : 2)}</div> : <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.2)' }}>—</div>}
                {q && <div style={{ fontSize: 9, fontWeight: 800, color, fontFamily: "'JetBrains Mono', monospace" }}>{isUp ? '+' : ''}{changePct.toFixed(2)}%</div>}
              </div>
            </div>
          )
        })}
...
function AgentWidget() {
  const router = useRouter()
  const { agentState, positions, loading, fetchStatus, startAgent, stopAgent, startAutoRefresh, stopAutoRefresh } = useAgentStore()
  ...
  return (
    <IOSCard onClick={() => router.push('/mobile/agent')} highlight={isRunning}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 15, fontWeight: 800, color: '#FFF', fontFamily: "'Cairo', sans-serif" }}>الوكيل المستقل</div>
            <div style={{ fontSize: 10, color: '#FF9F43', fontFamily: "'Cairo', sans-serif", fontWeight: 700 }}>تداول ذاتي بالذكاء الاصطناعي</div>
          </div>
        </div>
      ...
      <div style={{ padding: '8px 12px', borderRadius: 12, background: `${statusColor}08`, border: `0.5px solid ${statusColor}18`, marginBottom: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {isPaper && isRunning && <span style={{ fontSize: 7, fontWeight: 700, padding: '1px 5px', borderRadius: 5, background: 'rgba(0,212,255,0.1)', color: '#00D4FF', border: '0.5px solid rgba(0,212,255,0.2)', fontFamily: "'Cairo', sans-serif" }}>ورقي</span>}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ fontSize: 9, color: '#8B92A8', fontFamily: "'Cairo', sans-serif" }}>{STRATEGY_LABELS[strategy] || strategy}</span>
          <motion.button whileTap={{ scale: 0.9 }} onClick={(e) => { e.stopPropagation(); handleToggle() }} disabled={loading} style={{ padding: '4px 12px', borderRadius: 8, background: isRunning ? 'rgba(255,71,87,0.1)' : 'linear-gradient(135deg, #00FFC6, #0A84FF)', border: isRunning ? '0.5px solid rgba(255,71,87,0.2)' : 'none', color: isRunning ? '#FF4757' : '#000', fontSize: 9, fontWeight: 800, fontFamily: "'Cairo', sans-serif", cursor: 'pointer' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6 }}>
        <div style={{ padding: '8px 6px', borderRadius: 12, textAlign: 'center', background: 'rgba(255,255,255,0.02)', border: '0.5px solid rgba(255,255,255,0.05)' }}>
          ...
          <div style={{ fontSize: 13, fontWeight: 800, color: dailyPnL >= 0 ? '#00FFA3' : '#FF4757', fontFamily: "'JetBrains Mono', monospace" }}>{dailyPnL >= 0 ? '+' : ''}{dailyPnL.toFixed(2)}</div>
          <div style={{ fontSize: 8, color: '#8B92A8', fontFamily: "'Cairo', sans-serif", marginTop: 1 }}>ربح اليوم</div>
        </div>
        <div style={{ padding: '8px 6px', borderRadius: 12, textAlign: 'center', background: 'rgba(255,255,255,0.02)', border: '0.5px solid rgba(255,255,255,0.05)' }}>
          <Activity size={11} color="#00D4FF" style={{ margin: '0 auto 3px' }} />
          <div style={{ fontSize: 13, fontWeight: 800, color: '#FFF', fontFamily: "'JetBrains Mono', monospace" }}>{dailyTrades}</div>
          <div style={{ fontSize: 8, color: '#8B92A8', fontFamily: "'Cairo', sans-serif", marginTop: 1 }}>صفقات اليوم</div>
        </div>
        <div style={{ padding: '8px 6px', borderRadius: 12, textAlign: 'center', background: 'rgba(255,255,255,0.02)', border: '0.5px solid rgba(255,255,255,0.05)' }}>
          <Shield size={11} color={consecutiveLosses >= 3 ? '#FF4757' : '#B388FF'} style={{ margin: '0 auto 3px' }} />
          <div style={{ fontSize: 13, fontWeight: 800, color: consecutiveLosses >= 3 ? '#FF4757' : '#FFF', fontFamily: "'JetBrains Mono', monospace" }}>{consecutiveLosses}</div>
          <div style={{ fontSize: 8, color: '#8B92A8', fontFamily: "'Cairo', sans-serif", marginTop: 1 }}>خسائر متتالية</div>
        </div>
      </div>
    </IOSCard>
  )
    ...
    <IOSCard onClick={() => router.push('/mobile/ai')}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 15, fontWeight: 800, color: '#FFF', fontFamily: "'Cairo', sans-serif" }}>مجلس الذكاء الاصطناعي</div>
            <div style={{ fontSize: 10, color: '#B388FF', fontFamily: "'Cairo', sans-serif", fontWeight: 700 }}>رؤى من 6 نماذج AI</div>
          </div>
        </div>
        <ChevronLeft size={18} color="rgba(255,255,255,0.2)" />
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10, padding: '8px 12px', borderRadius: 12, background: `${color}08`, border: `0.5px solid ${color}18` }}>
        <div style={{ width: 44, height: 44, borderRadius: 12, background: `${color}15`, border: `1px solid ${color}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <span style={{ fontSize: 16, fontWeight: 900, color, fontFamily: "'JetBrains Mono', monospace" }}>{score}%</span>
        </div>
        <div>
          <div style={{ fontSize: 13, fontWeight: 800, color, fontFamily: "'Cairo', sans-serif" }}>توصية: {recLabel}</div>
          <div style={{ fontSize: 10, color: '#8B92A8', fontFamily: "'Cairo', sans-serif" }}>{selectedSymbol} — ثقة {score}%</div>
        </div>
      </div>
      ...
      {items.map((item) => {
        ...
        return (
          <button key={item.href} onClick={() => router.push(item.href)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, padding: '10px 4px', borderRadius: 14, background: `${item.color}08`, border: `0.5px solid ${item.color}18`, cursor: 'pointer', touchAction: 'manipulation' }}>
            ...
            <span style={{ fontSize: 9, fontWeight: 700, color: 'rgba(255,255,255,0.7)', fontFamily: "'Cairo', sans-serif", textAlign: 'center' }}>{item.label}</span>
          </button>
        )
      })}
    </div>
...
export default function MobileHomePage() {
  ...
  return (
    <div className="m-page">
      {/* Top Bar */}
      <div style={{ padding: '12px 16px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', direction: 'rtl' }}>
        <div>
          <div style={{ fontSize: 20, fontWeight: 900, color: '#FFF', fontFamily: "'Cairo', sans-serif" }}>رؤى</div>
          <div style={{ fontSize: 10, color: modeAccent, fontFamily: "'Cairo', sans-serif", fontWeight: 700 }}>منصة ربط الحسابات</div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ padding: '4px 10px', borderRadius: 8, background: `${modeAccent}12`, border: `0.5px solid ${modeAccent}25` }}>
            <span style={{ fontSize: 9, fontWeight: 800, color: modeAccent, fontFamily: "'Cairo', sans-serif" }}>{MODE_CONFIG[tradingMode]?.labelAr}</span>
          </div>
        </div>
      </div>
        ...
        <IOSCard>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 800, color: '#FFF', fontFamily: "'Cairo', sans-serif" }}>قوة الشراء</span>
            </div>
          </div>
