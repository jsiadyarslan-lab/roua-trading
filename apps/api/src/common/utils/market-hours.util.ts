// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// Roua Trading (رؤى) — Market Hours Utility (Backend)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 * Returns an object with status and human-readable reason.
 */
export function isMarketOpen(symbol: string, now: Date = new Date()): {
  ...
  switch (marketType) {
    ...
    default:
      // Unknown market type — be conservative and allow
      return { open: true, reason: 'Unknown market type — trading allowed', marketType, nextOpen: null };
  }
}

function checkForexHours(now: Date): { open: boolean; reason: string; marketType: MarketType; nextOpen: Date | null } {
...
function checkStockHours(now: Date): { open: boolean; reason: string; marketType: MarketType; nextOpen: Date | null } {
  const day = now.getUTCDay();
  const hour = now.getUTCHours();
  const minute = now.getUTCMinutes();
  const timeInMinutes = hour * 60 + minute;
...
function checkCommodityHours(now: Date): { open: boolean; reason: string; marketType: MarketType; nextOpen: Date | null } {
  const day = now.getUTCDay();
  const hour = now.getUTCHours();
  const minute = now.getUTCMinutes();
  const timeInMinutes = hour * 60 + minute;
