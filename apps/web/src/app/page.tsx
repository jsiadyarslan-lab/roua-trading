...
import {
  Shield, Globe, Brain, Newspaper, ChevronDown,
  Fingerprint, Lock, Eye, Zap, BarChart3, TrendingUp,
  CheckCircle2, Rocket, Sparkles, Key, Cpu, AlertCircle
} from 'lucide-react'
    ...
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
      ...
      {authError && (
        <div className="fixed top-16 left-0 right-0 z-[60] bg-red-500/10 border-b border-red-500/20 px-6 py-3">
          <div className="max-w-7xl mx-auto flex items-center gap-3">
            ...
            <Button variant="ghost" size="sm" className="text-red-400 hover:text-red-300 h-7" onClick={() => setAuthError(null)}>
              ✕
            </Button>
          </div>
        </div>
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-xl border-b border-border">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button size="sm" className="bg-teal-500 hover:bg-teal-600 text-background font-bold" onClick={() => setShowAuth(!showAuth)}>
      ...
      <section className="relative pt-32 pb-20 px-6">
        ...
        <div className="max-w-4xl mx-auto text-center relative">
          ...
          >
            <span className="bg-gradient-to-l from-teal-300 via-teal-400 to-blue-400 bg-clip-text text-transparent">رؤى</span>
            <span className="text-foreground"> للتداول</span>
          </motion.h1>

          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }} className="text-xl sm:text-2xl text-muted-foreground mb-2 font-light" dir="ltr">
            &ldquo;Vision into Markets&rdquo;
          </motion.p>

          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }} className="text-lg text-muted-foreground mb-10 font-light">
            ببصيرة نحو الأسواق
          </motion.p>

          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.5 }} className="text-base text-muted-foreground max-w-2xl mx-auto mb-10 leading-relaxed">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.6 }} className="flex flex-wrap justify-center gap-4">
            <Button size="lg" className="bg-teal-500 hover:bg-teal-600 text-background font-bold text-lg px-8 h-12" onClick={() => setShowAuth(true)}>
              <Fingerprint className="w-5 h-5 ml-2" />
              ابدأ الآن
            </Button>
            <Button size="lg" variant="outline" className="border-border hover:bg-secondary font-bold text-lg px-8 h-12" onClick={() => document.getElementById('pillars')?.scrollIntoView({ behavior: 'smooth' })}>
              تعرف على المزيد
              <ChevronDown className="w-5 h-5 mr-2" />
            </Button>
          </motion.div>

          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.8 }} className="grid grid-cols-3 gap-6 mt-16 max-w-lg mx-auto">
            {[
      ...
      <section id="pillars" className="py-20 px-6">
        <div className="max-w-6xl mx-auto">
          <motion.div {...fadeInUp} className="text-center mb-16">
            <Badge variant="outline" className="mb-4 border-teal-500/30 text-teal-400">الركائز الثورية</Badge>
            <h2 className="text-3xl sm:text-4xl font-black mb-4">الأعمدة الأربع لمنصة رؤى</h2>
            <p className="text-muted-foreground max-w-xl mx-auto">كل ركيزة مصممة لتحويل تجربة التداول من عشوائية إلى منهجية ذكية</p>
          </motion.div>
      ...
      <section className="py-20 px-6 bg-card/30">
        <div className="max-w-6xl mx-auto">
          <motion.div {...fadeInUp} className="text-center mb-16">
            <Badge variant="outline" className="mb-4 border-blue-500/30 text-blue-400">AI Symphony</Badge>
            <h2 className="text-3xl sm:text-4xl font-black mb-4">سيمفونية الذكاء الاصطناعي</h2>
            <p className="text-muted-foreground max-w-xl mx-auto">ستة نماذج متخصصة تعمل في تناغم — كل نموذج يؤدي دوره بدقة متناهية</p>
          </motion.div>
                ...
                <Card className={`bg-gradient-to-br ${model.color} border ${model.border} hover:scale-[1.02] transition-transform duration-200`}>
                  <CardContent className="p-5">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <Badge variant="outline" className="text-[10px] border-border/50 text-muted-foreground" dir="ltr">{model.model}</Badge>
                    </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.6 }} className="mt-12 text-center">
      ...
      <section className="py-20 px-6">
        <div className="max-w-6xl mx-auto">
          <motion.div {...fadeInUp} className="text-center mb-16">
            <Badge variant="outline" className="mb-4 border-amber-500/30 text-amber-400">الميزات الثورية</Badge>
            <h2 className="text-3xl sm:text-4xl font-black mb-4">ميزات تُعيد تعريف التداول</h2>
            <p className="text-muted-foreground max-w-xl mx-auto">أدوات لم تكن متاحة من قبل للمتداول العربي</p>
          </motion.div>
          <div className="space-y-4">
            {features.map((feature, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: i * 0.1 }}>
                <Card className="bg-card/50 border-border hover:border-teal-500/30 transition-all duration-300 cursor-pointer" onClick={() => setExpandedSection(expandedSection === feature.badge ? null : feature.badge)}>
                  <CardContent className="p-5">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-1">
                          ...
                          <Badge variant="outline" className="text-[10px] text-teal-400 border-teal-500/30" dir="ltr">{feature.badge}</Badge>
                        </div>
      ...
      <section className="py-20 px-6 bg-card/30">
        <div className="max-w-4xl mx-auto">
          <motion.div {...fadeInUp} className="text-center mb-16">
            <Badge variant="outline" className="mb-4 border-red-500/30 text-red-400">Zero-Trust Security</Badge>
            <h2 className="text-3xl sm:text-4xl font-black mb-4">الأمان الصفري</h2>
            <p className="text-muted-foreground max-w-xl mx-auto">لا ثقة افتراضية — كل طلب يجب أن يُثبت شرعيته</p>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.5 }} className="mt-8">
      ...
      <section className="py-20 px-6">
        <div className="max-w-6xl mx-auto">
          <motion.div {...fadeInUp} className="text-center mb-16">
            <Badge variant="outline" className="mb-4 border-green-500/30 text-green-400">Roadmap</Badge>
            <h2 className="text-3xl sm:text-4xl font-black mb-4">خارطة الطريق إلى الثورة</h2>
            <p className="text-muted-foreground max-w-xl mx-auto">أربع مراحل على مدى 12 شهرًا — من الأساس إلى الإطلاق</p>
          </motion.div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {roadmap.map((phase, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: i * 0.12 }}>
                <Card className="bg-card/50 border-border h-full">
      ...
      <section className="py-20 px-6 bg-card/30">
        <div className="max-w-3xl mx-auto text-center">
          <motion.div {...fadeInUp}>
            ...
            <Button size="lg" className="bg-teal-500 hover:bg-teal-600 text-background font-bold text-lg px-10 h-14" onClick={() => setShowAuth(true)}>
              <Rocket className="w-5 h-5 ml-2" />
              ابدأ مجانًا
            </Button>
            <p className="text-xs text-muted-foreground mt-4">سجّل بحساب Google أو Passkeys — بدون كلمات مرور</p>
          </motion.div>
        </div>
      </section>
      ...
      <footer className="py-10 px-6 border-t border-border">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <p className="text-xs text-muted-foreground">&copy; {new Date().getFullYear()} Roua Trading — ببصيرة نحو الأسواق</p>
