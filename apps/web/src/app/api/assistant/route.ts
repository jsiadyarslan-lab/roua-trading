...
function buildAgentPrompt(
  ...
  const systemPrompt = isAr
    ...
    : buildEnglishAgentPrompt(dbData, realtimeData, searchPlan, deepSearch, locale);
...
## قواعد أساسية:
1. استخدم الأسعار من البيانات المتاحة فقط. لا تخترع أسعارًا.
...
رتب الرد بهذا الشكل بالضبط:

**📊 ملخص المحفظة**
- عدد الصفقات + التوزيع (Short/Long)
- PnL الإجمالي
- أكبر مخاطرة

**1️⃣ جدول الصفقات**
| الأصل | النوع | الدخول | الحالي | PnL | مسافة SL | مسافة TP | أولوية |
رتب حسب الأولوية: 🔴 عاجل (قرب SL) → 🟢 جيد (قرب TP) → 🟡 راقب
...
- 🔴 صفقات خاسرة قرب SL → إغلاق فوري أو تعديل SL
...
## عند الأسئلة العامة:
كن حيًا وتفاعليًا. لا تستخدم قالبًا ثابتًا لكل سؤال.
- سؤال بسيط → رد بسيط
- سؤال تحليل → تحليل مركّز
- محادثة عادية → رد حواري قصير

## عند تحليل أصل واحد:
1️⃣ السعر + الاتجاه
2️⃣ فني (دعم/مقاومة/RSI/MACD/MA50)
3️⃣ عوامل أساسية
4️⃣ سيناريوهات (🟢/🟡/🔴)
5️⃣ توصية`);
...
4. **Write in ${(locale || 'en').toUpperCase()}**. Exception: financial asset symbols (BTC, XAU, 2222.SR). Do NOT mix languages.
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (safeRealtimeData.sources && safeRealtimeData.sources.length > 0) {
