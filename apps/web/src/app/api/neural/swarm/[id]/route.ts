...
/**
 * GET /api/neural/swarm/[id]
 * Proxies to NestJS /api/neural/swarm/:id
...
export async function GET(
  ...
  try {
    ...
    const res = await fetch(`${apiTarget}/api/neural/swarm/${encodeURIComponent(id)}`, {
      method: 'GET',
      headers,
    });
    ...
    return NextResponse.json(
      { success: false, error: `Neural swarm status proxy error: ${error.message}` },
      { status: 502 },
    );
  }
}
