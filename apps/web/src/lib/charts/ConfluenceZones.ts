...
export function detectConfluenceZones(opts: {
  ...
  for (const ob of (opts.smcData?.orderBlocks || [])) {
    ...
    allSignals.push({
      ...
      price: obPrice,
      labelAr: ob.type === 'bullish' ? 'بلوك أوامر صاعد' : 'بلوك أوامر هابط',
    });
  }
  ...
  for (const sr of (opts.srLevels || [])) {
    allSignals.push({
      ...
      price: sr.price,
      labelAr: sr.type === 'support' ? 'دعم' : 'مقاومة',
    });
  }
  ...
  for (const tl of (opts.trendLines || [])) {
    allSignals.push({
      ...
      confidence: 0.5,
      price: tl.price,
      labelAr: tl.direction === 'bullish' ? 'خط ترند صاعد' : 'خط ترند هابط',
    });
  }
  ...
  if (opts.wyckoffResult && opts.wyckoffResult.confidence > 0.3) {
    allSignals.push({
      ...
      price: currentPrice,
      labelAr: `ويكوف: ${opts.wyckoffResult.currentPhase}`,
    });
  }
...
function createZone(signals: ConfluenceSignal[], currentPrice: number): ConfluenceZone {
  ...
  const descriptionAr = `منطقة تقارب ${dirAr}: ${signals.length} إشارات من ${uniqueSources} محركات (${sourceNames}) | قوة: ${score}%`;
