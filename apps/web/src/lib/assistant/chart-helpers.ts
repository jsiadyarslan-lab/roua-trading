...
export function renderSparkline(
  ...
  const { width = 120, height = 40, color = '#00D4FF', fillColor = 'rgba(59,130,246,0.1)', strokeWidth = 1.5 } = options || {};
...
export function getTrendColor(prices: number[]): { stroke: string; fill: string } {
  if (!prices || prices.length < 2) return { stroke: '#00D4FF', fill: 'rgba(59,130,246,0.1)' };
  const isUp = prices[prices.length - 1] >= prices[0];
  return isUp
    ? { stroke: '#00FFA3', fill: 'rgba(34,197,94,0.1)' }
