  ...
  const ws = useChartWebSocket({
    ...
    onCandleUpdate: (candle) => {
      ...
      if (idx >= 0) {
        ...
        // FIX: When trimming, use setCandlesRef (NOT updateCandleRef) because
        // RouaChart's candlesRef gets a new array reference after slice(), but
        // useChart's candlesRef still holds the old (longer) array. Calling
        // updateCandleRef would push the candle to useChart's untrimmed array,
        // causing a desync between the two refs that corrupts OHLC merges.
        if (candlesRef.current.length > MAX_VISIBLE_CANDLES) {
          ...
          rebuildTimeIndexMap();
          // Use setCandlesRef to sync both RouaChart AND useChart's candlesRef
          setCandlesRef.current([...candlesRef.current], { skipIndicatorRebuild: true });
        } else {
          // No trimming needed — safe to use O(1) updateCandleRef
